...
public class InsideUtils {
	...
	/**
	 * Convierte un String delimitado por guiones bajos
	 * a su equivalente "camelizado"
	 * Ej: version_nti => versionNti
	...
	/**
	 * Convierte un String "camelizado" a su equivalente 
	 * delimitado por guiones bajos
	 * Ej versionNti => version_nti
	...
	/**
	 * Devuelve el nombre de formato que tendría un documento según el mime, si no es conocido devolvemos NULL
	 * @param mime
	 * @return 
	 */	
	public static String getNombreFormatoByMime (String mime) {
		...
		if ("application/pdf".equalsIgnoreCase(mime)) {
			...
			nombreFormato = "GIF";
		} else if ("image/tif".equalsIgnoreCase(mime) || "image/tiff".equalsIgnoreCase(mime) || "image/x-tiff".equalsIgnoreCase(mime) ) {			
			nombreFormato = "TIF";
	...
	public static String getMimeByTipoFirma (String tipoFirma) {
		...
		if ("Adobe PDF".contentEquals(tipoFirma) || "TF06".contentEquals(tipoFirma)) {
			mime = "application/pdf";
		} else if (StringUtils.hasText(tipoFirma) && 
								("TF02".contentEquals(tipoFirma) || 
								 "TF03".contentEquals(tipoFirma) || 
								 tipoFirma.toUpperCase().contains("XADES"))
				   ) {
			mime = "text/xml";
		} else if (StringUtils.hasText(tipoFirma) && (("TF04".contentEquals(tipoFirma) ||
													   "TF05".contentEquals(tipoFirma) ||
	...
	/**
	 * Devuelve una cadena formada por la concatenación de los elementos de la lista, pasados a String.
	 * Cada elemento estará separado del siguiente por la cadena que se indique en el parámetro "sep".
	 * El parámetro sepFinal indica si se quiere meter también la cadena de separación al final.
	 * El parámetro notNull tomará valor true cuando se quiera que si la lista es vacía se devuelva "" en lugar de null
	 * @param <T>
	 * @param lista
	 * @param cadenaSep
	 * @param sepFinal
	 ...
	 * @return
	 */
	public static <T> String listaToString (T[] lista, String cadenaSep, boolean sepFinal, boolean notNull) {
	...
	/**
	 * Se divide la lista en trozos (paginas), de un número de elementos concreto (elementosPorPagina).
	 * Se devuelve el "trozo" correspondiente al número de página indicado en el parámetro "pagina".
	 ...
	 * @return Si la lista es vacía, se devuelve la lista vacía. Si la página pedida es mayor
	 * que el número real de página se devuelve la lista vacía.
	 */
	public static <T> List<T> getSublista (List<T> lista, int elementosPorPagina,  int pagina) {
		// numero de páginas que habrá en total
		int numeroPaginas;
		
		// si el resto de la division no es 0, habra una pagina mas que el resultado devuelto por la division entera.
	 ...
	 * Si no lo encuentra lo cameliza. Si no lo encuentra compara sin tener en cuenta mayúsculas y minúsculas.
	 * @param objeto
	 * @param fieldName
	 * @return Devuelve el propertyDescriptor asociado al objeto y el atributo. Null si ninguno coincide con el nombre.
	 */
	public static PropertyDescriptor findPropertyDescriptor (Object objeto, String fieldName) {
		...
		if (pd == null) {
			...
			while (i < pds.length && !found) {
				...
				if (pdesc.getName().equalsIgnoreCase(fieldName) || pdesc.getName().equalsIgnoreCase(fieldNameCam)) {
					pd = pdesc;
					found = true;
				}
				i++;
