...
public class InsideConverterBusqueda {
	...
	public static ConsultaInside consultaXmlToInside(ConsultaWsInside consultaXml) throws InsideConverterException{
		ConsultaInside consulta = null;
		if(consultaXml.getMetadato() != null){
			consulta = new ConsultaMetadatoInside(InsideConverterBusquedaMetadato.metadatoXmlToInside(consultaXml.getMetadato()));
	...
	static class InsideConverterBusquedaMetadato{
		...
		public static MetadatoBusquedaInside metadatoXmlToInside(MetadatoBusquedaWsInside metadatoXml) throws InsideConverterException{
			...
			if(StringUtils.isNotBlank(valorXml.getEqual())) {
				if (InsideConverterUtils.isBooleanValue(valorXml.getEqual())) {
					valor = new MetadatoValorSimpleInside(MetadatoValorSimpleInside.TipoBusqueda.equal, Boolean.valueOf(valorXml.getEqual()));
				} else {
					valor = new MetadatoValorSimpleInside(MetadatoValorSimpleInside.TipoBusqueda.equal, valorXml.getEqual());
				}
			}else if(StringUtils.isNotBlank(valorXml.getLike())){
				valor = new MetadatoValorSimpleInside(MetadatoValorSimpleInside.TipoBusqueda.like, valorXml.getLike());
			}else if(valorXml.getMinusThan() != null){
				valor = new MetadatoValorSimpleInside(MetadatoValorSimpleInside.TipoBusqueda.lessThan, valorXml.getMinusThan());
			}else if(valorXml.getMoreThan() != null){
				valor = new MetadatoValorSimpleInside(MetadatoValorSimpleInside.TipoBusqueda.greaterThan, valorXml.getMoreThan());
			}else if(valorXml.getBetween() != null){
				valor = new MetadatoValorBetweenInside(valorXml.getBetween().getInfRange(),valorXml.getBetween().getSupRange());
			}else if(valorXml.getDateRange() != null){
				valor = new MetadatoValorDateRange(	InsideConverterXmlGregorianCalendar.calendarToXmlCalendar(valorXml.getDateRange().getFrom()),
													InsideConverterXmlGregorianCalendar.calendarToXmlCalendar(valorXml.getDateRange().getTo()));
			}else{
	...
	static class InsideConverterSubConsulta{
			...
			throws InsideConverterException
		{
			...
			for(MetadatoBusquedaWsInside consultaMetadato :  subconsultaXml.getMetadato()){
				subconsultasInside.add(new ConsultaMetadatoInside(InsideConverterBusquedaMetadato.metadatoXmlToInside(consultaMetadato)));
			}
		...
		public static ConsultaSubInside.TipoSubConsulta tipoSubConsultaXmlToInside(TipoSubconsulta tipoSubConsultaXml) 
			throws InsideConverterException
		{
			...
			case AND:
				...
				break;
			default:
				throw new InsideConverterException("Tipo de subconsulta no admitido: " + tipoSubConsultaXml.value(),true);
			}
			return tipo;
		}
	}
