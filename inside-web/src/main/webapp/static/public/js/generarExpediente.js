...
function generateExpedient(operation) {
	...
	try {
		...
		$("#tipoMensaje").val(4);
		$("#valorMensaje").val('Incongruencia de datos. Se modifico el identificador del expediente después de generar el indice y no coinciden sus identificadores, para solucionarlo realice una modificacion en el indice.');
		showMessage();
		$('#expedienteVeil').addClass('hidden');
	}
}
		...
		success : function(data) {
			if (data.mensajeUsuario != null
				...
				if (operation === '/generarExpediente') {
					...
					$("#descargarExpedienteEniForm #identificador").val($("#expedienteForm #identificador").val());
					
					$("#descargarExpedienteEniAdicionalesForm #contenido").val(data.expedienteEniConvertconAdicionales);
					$("#descargarExpedienteEniAdicionalesForm #identificador").val($("#expedienteForm #identificador").val());
					
					if (data.conAdicionales) {
						$("#button_descargarExpAdicionales").show();
						dataFileExp = data.expedienteEniConvertconAdicionales;
						$("#descargarExpedienteEniCompletoForm #contenido").val(data.expedienteEniConvertconAdicionales);
					} else {
						$("#button_descargarExpAdicionales").hide();
					...
					$("#descargarExpedienteEniCompletoForm #identificador").val($("#expedienteForm #identificador").val());
...
function removeInteresadoExp(position) {
	...
	while(interesadosAdded-1 > position) {
		var aux = position + 1;
		document.getElementById("expediente.metadatosExp.interesado" + aux).setAttribute("name", "expediente.metadatosExp.interesado[" + position + "]");
		document.getElementById("expediente.metadatosExp.interesado" + aux).setAttribute("id", "expediente.metadatosExp.interesado" + position);
...
function removeOrganoExp(position) {
	...
	while(organosAdded-1 > position) {
		var aux = position + 1;
		document.getElementById("expediente.metadatosExp.organo" + aux).setAttribute("name", "expediente.metadatosExp.organo[" + position + "]");
		document.getElementById("expediente.metadatosExp.organo" + aux).setAttribute("id", "expediente.metadatosExp.organo" + position);
...
function removeMetadatoDoc (position){
	$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + position + ".nombre']").remove();
	$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + position + ".valor']").remove();
	...
	while(metadatosDocAdded-1 > position) {
		var aux = position + 1;
		$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + aux + ".nombre']").attr("name", "metadatosAdicionalesDoc.metadatoAdicional[" + position + "].nombre");
		$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + aux + ".nombre']").attr("id", "metadatosAdicionalesDoc.metadatoAdicional" + position + ".nombre");
		$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + aux + ".valor']").attr("name", "metadatosAdicionalesDoc.metadatoAdicional[" + position + "].valor");
		$("#js-gallery-createDocument input[id='metadatosAdicionalesDoc.metadatoAdicional" + aux + ".valor']").attr("id", "metadatosAdicionalesDoc.metadatoAdicional" + position + ".valor");
		$("#js-gallery-createDocument #removeMetadatoButton_" + aux).attr("onclick", "removeMetadatoDoc(" + position + ")");
		$("#js-gallery-createDocument #removeMetadatoButton_" + aux).attr("id", "removeMetadatoDoc_" + position);
		$("#js-gallery-createDocument #divMetadatoAdicionalDoc_" + aux).attr("id", "divMetadatoAdicionalDoc_" + position);
		position++;
	}
	metadatosDocAdded--;
}
			...
			if(data.datosDeEnvio != null)
				{
					...
					$('.mf-dialog #visorResguardo').attr('src', 'data:application/pdf;base64,' + data.dataResguardoDocBase64);
					...
					$(".mf-dialog #claseProcedimientoConsulta").val(data.datosDeEnvio.procedimiento.claseProcedimiento);
					$(".mf-dialog #procedimientoanioConsulta").val(data.datosDeEnvio.procedimiento.anioProcedimiento);
					$(".mf-dialog #procedimientonumeroConsulta").val(data.datosDeEnvio.procedimiento.numeroProcedimiento);
					...
					$(".mf-dialog #piezanumeroProcedimientoConsulta").val(data.datosDeEnvio.pieza.numeroProcedimientoPieza);
					
					$(".mf-dialog #presentadorcodigoOrganoRemitenteConsulta").val(data.presentadorcodigoOrganoRemitente);
					$(".mf-dialog #presentadordescripcionPresentadorExpedienteConsulta").val(data.datosDeEnvio.descripcionExpediente);
				}
				else
				{
					...
					$(".mf-dialog #comentario").val("Los datos recibidos sobre el env\u00edo no aparecen informados.");
