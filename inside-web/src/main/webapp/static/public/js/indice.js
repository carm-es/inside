...
function configurarNuevo() {
	...
	$('#js-gallery-ver')
		.on('click','.js-gallery-ver--lista-cancel', function (e) {
...
function crearEspaciosDragDrop() {
	...
	function crearEspacioDragDrop(orden) {
		...
		return $('<li class="js-arbol-espacio" data-orden="' + ordenfinal + '">')
					.attr('ondrop', "drop2(event)")
					.attr('ondragover', "allowDrop2(event)")
...
function crearItemLista(par) {
	...
	var item = $('<li id="' + generateNodeId() + '" class="js-arbol-lista-item" ' + (par.isDocument ? 'data-id="' + par.id + '"' : '') + ' data-nombre="' + par.texto + '" data-tipo="' + (par.isDocument ? 'documento' : 'carpeta') + '"><div class="js-arbol-lista-item-cont g-arbol-lista-item-cont' + (!par.isDocument ? ' is-open' : '') + '">' + (par.isDocument ? '<span class="mf-icon mf-icon-doc"/> ' : '<span class="mf-icon mf-icon-folder js-arbol-lista-item-folder"/>') + '<span class="name' + (par.isDocument ? '' : ' js-arbol-lista-item-text')+ '" ' + ' title="' + par.texto + '">' + (par.isDocument ? getNameDocument(par.texto, par.natural) : par.texto) + '</span><div class="actions">' + (par.isDocument ? '<i class="mf-icon mf-icon-eye js-arbol-lista-item-ver" title="ver"></i>' : '') + '<i class="mf-icon mf-icon-cancel js-arbol-lista-item-delete" title="borrar"> </i></div></div></li>');
	...
	item
		...
		.on('click', '.js-arbol-lista-item-text', onClickText)
		.on('click', '.js-arbol-lista-item-ver', onClickVer)
		.on('click', '.js-arbol-lista-item-delete', onClickDelete)
		.on('click', '.js-arbol-lista-item-folder', onClickFolder)
		.attr('ondrop', "drop2(event)")
		.attr('ondragover', "allowDrop2(event)")
		.attr('ondragleave', "dragleave(event)")
		.attr('draggable', "true")
...
function getLista() {
	...
	return {lista: lista, par: par, item: item};
}
...
function crearEstructuraCarpetas(e) {
	...
	if (elementoSeleccionadoNoEsDocumento('crear carpeta')) {
		...
		enviarAjax2(arbol.data('url_estructura_carpeta'), lista.par, 
		function (data) {
			if (data.mensajeUsuario != null){
...
function showInput(item) {
	if ($('#js-arbol-input').is(":hidden")){
		...
		$('#js-arbol-input')
			...
			.val(item.data('nombre'))
			.select()
			.on('blur', null, item, closeInput)
			.on('keypress', null, item, onKeyDownInput)
			.css('top', item.position().top + distanciaRaiz);
...
function hideInput(item) {
	...
	if (!newName.match(caracteresValidos)){
		$("#tipoMensaje").val(4);
		$("#valorMensaje").val("Nombre de carpeta incorrecto: " + newName + ". Caracteres permitidos: letras, digitos, y los caracteres .-_");
		showMessage();
	} else {
		var idExpediente = $("#expedienteForm #identificador").val();
		if (item && item.data('nombre') != newName) {
			enviarAjax2($('#js-arbol').data('url_carpeta_renombrar'), {
	    		...
	    		if (data.mensajeUsuario != null){
					...
					item
						.attr('data-nombre', newName)
						.data('nombre', newName)
						.find('.js-arbol-lista-item-text').first().html(newName).attr('title', newName);
	    		}
			});
		}
	}
...
function activateActions(){
	$(".item-actions").each(function(){
		...
		$(this).parent().find('>span').mouseenter(
				function(){
...
function updateTree(data, parent) {
	...
	var createNode = function (par) {
		...
		if (par.isDocument) {
			item = $('<li id="' + generateNodeId() + '" class="js-arbol-lista-item" data-id="' 
					+ par.id + '"' + ' data-nombre="' + par.texto + '" data-tipo="documento"><div class="js-arbol-lista-item-cont g-arbol-lista-item-cont"> '
					+ ' <span class="mf-icon mf-icon-doc"/><span class="name" title="' + par.texto + '">' 
					+ par.natural + '</span><div class="actions"><i class="mf-icon mf-icon-eye js-arbol-lista-item-ver" title="ver"></i>' 
					+ '<i class="mf-icon mf-icon-cancel js-arbol-lista-item-delete" title="borrar"> </i></div></div></li>');
		} else {
			item = $('<li id="' + generateNodeId() + '" class="js-arbol-lista-item" data-id="' 
					+ par.id + '"' + ' data-nombre="' + par.texto + '" data-tipo="carpeta"><div class="js-arbol-lista-item-cont g-arbol-lista-item-cont is-open"> ' 
					+ '<span class="mf-icon mf-icon-folder js-arbol-lista-item-folder"/><span class="name js-arbol-lista-item-text" title="' + par.texto + '">' 
					+  getNameFolder(par) + '</span><div class="actions">'
					+ '<i class="mf-icon mf-icon-cancel js-arbol-lista-item-delete" title="borrar"> </i></div></div></li>');
		}
