...
$(document)
		.ready(
				function() {
					...
					tableExpedientes = $('#expedientList')
							.dataTable(
									{
										"language" : {
...
function removeInteresadoExp(position) {
	...
	while (interesadosAdded - 1 > position) {
		var aux = position + 1;
		document.getElementById("interesadoslista" + aux).setAttribute("name",
				"interesadoslista[" + position + "]");
		document.getElementById("interesadoslista" + aux).setAttribute("id",
				"interesadoslista" + position);
		$("#removeInteresadoButton_" + aux).attr("onclick",
				"removeInteresadoExp(" + position + ")");
		$("#removeInteresadoButton_" + aux).attr("id",
				"removeInteresadoButton_" + position);
			...
			for (var i = 0; i < listVersiones.length; i++) {
				...
				$('.mf-dialog #versiones_expediente_MJU').append(
						$('<option>', {
							...
							text : 'Versión: ' + listVersiones[i].version
	...
	$(".mf-dialog #presentadorcodigoOrganoRemitente").autocomplete(
			{
				...
				// },
				select : function(event, ui) {
					$(".mf-dialog #presentadorcodigoOrganoRemitente").val(
							ui.item.key + " - " + ui.item.value);
					event.preventDefault();
				}
			}).bind('focus', function() {
...
function buscarOrganos() {
	if ($(".mf-dialog #provinciasModal").val().trim() != "") {
		var codigoProvinciaYDescripcion = $(".mf-dialog #provinciasModal")
				.val();

		var splitcodigoProvinciaYDescripcion = codigoProvinciaYDescripcion
				.split("-");
		...
		$(".mf-dialog #organos")
				.autocomplete(
						{
							source : $("#context").val()
									+ "/justicia/getOrganosByProvincia?codigoprovincia="
									+ codigoProvincia,
							minLength : 0,
							autoFocus : true,
							// focus : function(event, ui) {
			...
			return $("<li>").append(
					"<a>" + item.key + " - " + item.value + " - "
							+ item.tipoOrgano + " - " + item.codigoPoblacion
							+ "</a>").appendTo(ul);
		};
	}
}
...
function buscarOrdenByOrgano() {
	if ($(".mf-dialog #organos").val() != "") {
		...
		var codigoPoblacion = $(".mf-dialog #organos").val().split("-")[3]
				.trim();
		...
		$(".mf-dialog #ordenModal").autocomplete(
				{
					source : $("#context").val()
							+ "/justicia/getOrdenByOrgano?numOrgano="
							+ numOrgano + "&codigoPoblacion=" + codigoPoblacion
...
function buscarClaseProcedimientoByOrdenAndOrgano() {
	...
	if ($(".mf-dialog #dir3Justicia").val() != "" && $(".mf-dialog #ordenModal").val() != "") {
		...
		// Autocompletado del campo orden.
		$(".mf-dialog #claseProcedimiento")
				.autocomplete(
						{
							source : $("#context").val()
									+ "/justicia/getClaseProcedimientoByOrdenAndOrgano?numOrgano="
									+ numOrgano + "&codigoPoblacion="
									+ codigoPoblacion + "&tipoOrgano="
									+ tipoOrgano + "&ordenNum=" + ordenNum,
							minLength : 0,
							autoFocus : true,
							// focus : function(event, ui) {
							// $(".mf-dialog
		...
		{
			setTimeout(function(){mensajeNoOrdenYOcultarOrden()},3000);//pasan 3 segundos y oculta el orden
...
function remitirMJU() {
	$('#almacenadosVeil').removeAttr('style').removeClass('hidden');
	$(".mf-dialog #almacenadosVeilGeneraToken").removeAttr('style')
			.removeClass('hidden');
	$.ajax({
...
function signError(errorType, errorMsg) {
	if (errorType == 'es.gob.afirma.core.AOCancelledOperationException') {
		...
		console
				.log("No se ha encontrado certificado en el almacen");
