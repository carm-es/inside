...
public class CmisAdapter implements CmisAdapterInterface{
	...
	public void setNumberRetriesGetFolderWithHalfSecondDelayToAvoidConcurrentCreation(
		...
		this.numberRetriesGetFolderWithHalfSecondDelayToAvoidConcurrentCreation = numberRetriesGetFolderWithHalfSecondDelayToAvoidConcurrentCreation;
	}
	// CARM 2.0.7.2 ###	
	 ...
	 * @param objectTypeId	Tipo del objeto que va a crearse. Si es nulo se le da el genérico de carpeta de CMIS.
	 * @param metadatos 	Metadatos de la carpeta. Puede ser nulo si la carpeta no los tiene.
	 * @return
	 * @throws RepositoryCmisException
	 */
	public Folder altaCarpeta(final String[] path,final String objectTypeId, final Map<String, Object> metadatos)
			throws RepositoryCmisException 
	{
		...
		if(cleanedPath.length == 1){
			...
			return this.altaCarpeta(cleanedPath[cleanedPath.length - 1],Arrays.copyOf(cleanedPath, cleanedPath.length - 1), objectTypeId, metadatos, true);
		}
	 ...
	 * @param objectTypeId 	Tipo del objeto que va a crearse. Si es nulo se le da el genérico de carpeta de CMIS.
	 * @param metadatos 	Metadatos de la carpeta. Puede ser nulo si la carpeta no los tiene.
	 * @param crearRuta 	Indica si debe crearse la estructura de directorios en caso de que ésta no exista. 
	 * @throws RepositoryCmisException cuando no pueda darse de alta la carpeta.
	 */
	public Folder altaCarpeta(final String folderName, 
										   final String[] ruta,
										   final String objectTypeId,
										   final Map<String, Object> metadatos,
										   boolean crearRuta)
			throws RepositoryCmisException {
		...
		try {
			...
			logger.debug(String.format("No existe la carpeta %s en el path %s , la crearé",folderName,StringUtils.join(cleanedRuta, pathSeparator)));
		}	
		...
		try {
			...
			logger.debug(String.format("Creando carpeta '%s' en  '%s'", folderName,StringUtils.join(cleanedRuta, pathSeparator)));
			...
			throw new RepositoryCmisException ("Error al dar de alta la carpeta [" + folderName + "]:" + GestionErroresUtils.getMessageByException(e) + " " + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado al dar de alta la carpeta [" + folderName + "]: "+ e.getMessage(), e);
			throw new RepositoryCmisException ("Error al dar de alta la carpeta [" + folderName + "]:" + GestionErroresUtils.getMessageByException(e) + " " + e.getMessage(), e);			
		}
	}
	...
	public Folder modificaCarpeta(final String folderId, final Map<String, Object> metadatos)
		...
		try {
			...
			throw new RepositoryCmisException ("Error al modificar la carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado al modificar la carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado al modificar la carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);			
		}
	}
	...
	public Folder getInfoCarpeta(final String folderId) throws RepositoryCmisException {
		try {
			...
			logger.error("Error en getInfoCarpeta, carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);
			throw new RepositoryCmisException ("Error en getInfoCarpeta, carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado en getInfoCarpeta, carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en getInfoCarpeta, carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);			
		}
	}
	
	@CmisSession
	public List<Map<String, Object>> getDocsCarpeta(final String folderId) throws RepositoryCmisException 
	{
		return getObjetosCarpeta(folderId,BaseTypeId.CMIS_DOCUMENT);
	}
	
	@CmisSession
	public List<Map<String, Object>> getFoldersCarpeta(final String folderId) throws RepositoryCmisException 
	{
		return getObjetosCarpeta(folderId,BaseTypeId.CMIS_FOLDER);
	}
	
	@CmisSession
	private List<Map<String, Object>> getObjetosCarpeta(final String folderId, BaseTypeId restriccionTipo) 
		throws RepositoryCmisException 
	{
		try {
			...
			for (final CmisObject children : folder.getChildren()) {
				if (restriccionTipo != null && !children.getType().getBaseTypeId().equals(restriccionTipo)) {
					continue;
				}
			...
			throw new RepositoryCmisException ("Error no controlado en getDocsCarpeta, carpeta con Id [" + folderId + "]: "+ e.getMessage(), e);			
		}
	}
	
	
	@CmisSession
	public Document altaDocumento(final String documentName,
											 final String[] ruta,
											 final String objectTypeId,
		...
		try {
			...
			return this.altaDocumento(documentName, folder, objectTypeId, contenido, tipoMIME, metadatos, crearRuta);
			
		} catch (final RepositoryCmisException rce) {
			throw rce;
		}  catch (final Exception e) {
			logger.error("Error no controlado en altaDocumento, documentName [" + documentName + "]: "+ e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en altaDocumento, documentName [" + documentName + "]: "+ e.getMessage(), e);			
		}
	}
		...
		try {
			...
			if (contenido != null) {
				...
				content = sessionManager.getSession().getObjectFactory().createContentStream(documentName, 
																		 length,
																		 ((tipoMIME == null || tipoMIME.length() == 0) ? CmisUtils.getMimeType(contenido) : tipoMIME),
																		 bis);
	...
	@CmisSession
	public Map<String, Object> getInfoDocumento(final String documentId) throws RepositoryCmisException {
	...
	public Map<String, Object> getInfoDocumento(final String documentId, final String version)
		...
		try {
			...
			logger.error("Error en getInfoDocumento, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);
			throw new RepositoryCmisException ("Error en getInfoDocumento, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no getInfoDocumento en getInfoDocumento, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en getInfoDocumento, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);			
		}
	}
	...
	@CmisSession
	public Collection<String> getVersionesDocumento(final String documentId) throws RepositoryCmisException {
		try {
			...
			logger.error("Error en getVersionesDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error en getVersionesDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado en getVersionesDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en getVersionesDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		}
	}
	...
	public ContenidoDocumento getContenidoDoc(final String documentId, final String version)
		...
		try {
			...
			logger.error("Error en getContenidoDoc, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);
			throw new RepositoryCmisException ("Error en getContenidoDoc, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado en getContenidoDoc, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en getContenidoDoc, documentId [" + documentId + "], version ["+ version  +"]:" + e.getMessage(), e);			
		}
	}
	
	
	@CmisSession
	public Map<String, Object> modificarDocumento(final String documentId, final Map<String, Object> metadatos,
			final ContenidoDocumento contenido) throws RepositoryCmisException {
		try {
			final Document document = (Document) getCmisObjectById(documentId);
			...
			if (contenido != null && contenido.getContenido() != null) {
				...
				content = session.getObjectFactory().createContentStream(fileName, length, contenido.getTipoMIME(),
						bis);
				bis.close();
			}
			...
			logger.error("Error en modificarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error en modificarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado en modificarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en modificarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		}
	}
	
	
	@CmisSession
	public void borrarDocumento(final String documentId, final boolean allVersions) throws RepositoryCmisException {
		try {
			final Document document = (Document) getCmisObjectById(documentId);
			if (allVersions) {
			...
			logger.error("Error en borrarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error en borrarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		} catch (final Exception e) {
			logger.error("Error no controlado en borrarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);
			throw new RepositoryCmisException ("Error no controlado en borrarDocumento, documentId [" + documentId + "]: " + e.getMessage(), e);			
		}
	}
	 ...
	 * @throws RepositoryCmisException
	 */
	public CmisObject getCmisObjectById(final String objectId) throws CmisBaseException, RepositoryCmisException {
	...
	@CmisSession
	/**
	 * Devuelve el objeto que representa el Folder indicado en ruta
	 * Si el segundo parámetro es true, si la carpeta no existía, se crea
	 * @param ruta
	 * @param crearRuta
	 * @return
	 * @throws RepositoryCmisException
	...
	public Folder getFolder (final String[] ruta, boolean crearRuta) throws RepositoryCmisException
	{
		...
		String path = sessionManager.getRootFolder().getPath() + pathSeparator + StringUtils.join(ruta, pathSeparator);
	...
	/*private static String buildMessageException (final Exception e) {
		StringBuffer sb = new StringBuffer ("");
		
		if (e instanceof CmisBaseException e) {
			CmisBaseException cmisExc = (CmisBaseException) e;
			cmisExc.getCode()
		}
	}*/
