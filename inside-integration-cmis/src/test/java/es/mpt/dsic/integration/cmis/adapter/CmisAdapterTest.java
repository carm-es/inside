...
public class CmisAdapterTest extends CmisTestBase {
	...
	public void configure() throws RepositoryCmisException, URISyntaxException{
		...
		if(rutaDocDisco == null){
			...
			Assert.assertTrue("La ruta de los ficheros de prueba no contiene ningún fichero" + rutaDocsPruebasDir, files.length > 0);
	...
	public void testAltaCarpeta () throws RepositoryCmisException {
		...
		metadatos1 = CmisUtils.getMetadatos(integration.altaCarpeta(nombreFolder + t2, ruta, folderType, metadatosCarpeta, true));
		...
		try {			
			metadatos1 = CmisUtils.getMetadatos(integration.altaCarpeta(nombreFolder + t4, ruta, folderType, metadatosCarpeta, false));
	...
	public void testModificaCarpeta () throws RepositoryCmisException {
		...
		Map<String, Object> metadatos1 = CmisUtils.getMetadatos(integration.modificaCarpeta(folderId, metadatosCarpetaModif));
	...
	public void testAltaDocumento () throws RepositoryCmisException  {
		...
		for(File docprueba : rutaDocsPruebasDir.listFiles()){
			...
			Map<String, Object> meta = CmisUtils.getMetadatos(altaDocSinMetaRutaNoExisteSinTipoCrearTrue (docprueba.getAbsolutePath()));
			pintarPropiedades(meta);
			logger.info("Fin Test sin metadatos para documento " + docprueba.getAbsolutePath() + ", ruta no existe, sin tipo, crear true");
			...
			pintarPropiedades(meta);
			//Assert.assertNotNull(meta.get("IdUnico"));
			logger.info("Fin Test sin metadatos para documento " + docprueba.getAbsolutePath() + ", ruta existe, sin tipo, crear false");
			docsBorrar.add((String) meta.get("cmis:objectId"));
			
			meta = CmisUtils.getMetadatos(altaDocConMetaRutaExisteConTipo (docprueba.getAbsolutePath(),false));
			pintarPropiedades(meta);
			docsBorrar.add((String) meta.get("cmis:objectId"));
			...
			//Assert.assertNotNull(meta.get("IdUnico"));
			logger.info("Fin Test con metadatos para documento " + docprueba.getAbsolutePath() + ", ruta existe, con tipo, crear false");
		}
	...
	public void testModificaDocumento() throws RepositoryCmisException {
		...
		final Map<String, Object> document = integration.modificarDocumento(documentId, metadatosDocumentoModif, null);
	...
	public void testGetDocsCarpeta() throws RepositoryCmisException {
		...
		metadatos1 = CmisUtils.getMetadatos(integration.altaCarpeta("s2", new String[] {"s1"}, folderType, metadatosCarpeta, true));
		String folderId = (String) metadatos1.get("cmis:objectId");
		
		byte[] bytes = fileToByteArray(rutaDocDisco);
		
		Map<String, Object> metadatos2 = CmisUtils.getMetadatos(integration.altaDocumento("parla1", new String[]{"s1", "s2"}, documentType, bytes , CmisUtils.getMimeType(rutaDocDisco, bytes), metadatosDocumento, false));
		docsBorrar.add((String)metadatos2.get("cmis:objectId"));
		
		metadatos2 = CmisUtils.getMetadatos(integration.altaDocumento("parla2", new String[]{"s1", "s2"}, documentType, bytes , CmisUtils.getMimeType(rutaDocDisco, bytes), metadatosDocumento, false));
		docsBorrar.add((String)metadatos2.get("cmis:objectId"));
	...
	{
		String[] ruta = new String[] {"f1"+System.currentTimeMillis(),"f2"+System.currentTimeMillis()};
		return this.altaDocumento(rutaDocDisco,String.valueOf(System.nanoTime()), ruta, null, null, false);
	}
	...
	private  Document altaDocSinMetaRutaNoExisteSinTipoCrearTrue (String rutaDocDisco)
		throws RepositoryCmisException
	{
		return this.altaDocumento(rutaDocDisco,String.valueOf(System.nanoTime()), rutaFolderDocs, null, null, true);
	}
		...
		throws RepositoryCmisException 
	{
		...
		return this.altaDocumento(rutaDocDisco,nombreDocumento, rutaFolderDocs, documentType, metadatosDocumento, false);
	}
	
	private  Document altaDocSinMetaRutaExisteSinTipo (String rutaDocDisco) throws RepositoryCmisException {
		byte[] bytes = fileToByteArray(rutaDocDisco);
		return integration.altaDocumento(String.valueOf(System.nanoTime()), rutaFolderDocs, null, bytes, CmisUtils.getMimeType(rutaDocDisco, bytes), null, false);
	}
	
	/**
	 * Método que llama al altaDocumento del CmisAdapter
	 * Extrae el contenido del fichero en base a una ruta absoluta
	 * Obtiene el tipo Mime del fichero
	 * Lleva la cuenta de los documentos subidos
	 * @param rutaDocDisco
	 * @param documentName
	 * @param ruta
	 * @param objectTypeId
	 ...
	 * @throws RepositoryCmisException
	 */
	private Document altaDocumento(String rutaDocDisco,final String documentName,final String[] ruta,final String objectTypeId,final Map<String, Object> metadatos,boolean crearRuta)
		throws RepositoryCmisException
	{
		logger.info(String.format("Voy a dar el alta del documento %s desde el fichero %s",documentName,rutaDocDisco));
		byte[] bytes = fileToByteArray(rutaDocDisco);
		Document documento = integration.altaDocumento(documentName, ruta, objectTypeId, bytes,  CmisUtils.getMimeType(rutaDocDisco, bytes), metadatos, crearRuta);
		documentosAlta++;
		return documento;
	}
	...
	/*@Test
	public void testAltaCarpeta() throws RepoIntegrationException {
		logger.info("Iniciando el test del método altaCarpeta");

		nombreFolder = String.valueOf(System.nanoTime());
		final Map<String, Metadato> metadatos = new HashMap<String, Metadato>();

		final String tipo = context.getCmisExpedienteTipo();
		metadatos.put(tipo, createMetadatoSimple(tipo, "tipo tramite"));
		final String interesado = context.getCmisExpedienteInteresado();
		metadatos.put(interesado, createMetadatoMulti(interesado, "id1", "id2"));
		final String asociado = context.getCmisExpedienteAsociado();
		metadatos.put(asociado, createMetadatoMulti(asociado, "asociado1", "asociado2"));

		final Map<String, Metadato> carpeta = integration.altaCarpeta(nombreFolder, metadatos);
		Assert.assertNotNull(carpeta);
		Assert.assertNotNull(carpeta.get(tipo));
		Assert.assertNotNull(carpeta.get(interesado));
		Assert.assertNotNull(carpeta.get(asociado));
	...
	@Test
	public void testAltaDocumento() throws RepoIntegrationException {
		logger.info("Iniciando el test del método altaDocumento");

		final String nombreFichero = String.valueOf(System.nanoTime());
		final Map<String, Metadato> metadatos = new HashMap<String, Metadato>();

		final String nombre = context.getCmisDocumentoNombre();
		metadatos.put(nombre, createMetadatoSimple(nombre, nombreFichero));
		final String tipo = context.getCmisDocumentoTipo();
		metadatos.put(tipo, createMetadatoSimple(tipo, "tipo documento"));
		final String id = context.getCmisDocumentoIdUnico();
		metadatos.put(id, createMetadatoSimple(id, "id único"));
		final String visibilidad = context.getCmisDocumentoVisibilidad();
		metadatos.put(visibilidad, createMetadatoSimple(visibilidad, "visibilidad"));
		final String ids = context.getCmisDocumentoIdImplicados();
		metadatos.put(ids, createMetadatoMulti(ids, "id1", "id2", "id3"));
		final String tipoFirma = context.getCmisDocumentoTipoFirma();
		metadatos.put(tipoFirma, createMetadatoSimple(tipoFirma, "tipo firma"));
		final String expediente = context.getCmisDocumentoExpediente();
		metadatos.put(expediente, createMetadatoMulti(expediente, nombreFolder));

		final Map<String, Metadato> document = integration.altaDocumento(metadatos, nombreFichero, null);
		Assert.assertNotNull(document);
		Assert.assertNotNull(document.get(tipo));
		Assert.assertNotNull(document.get(id));
		Assert.assertNotNull(document.get(expediente));
	...
	@Test
	public void testGetContenidoDoc() throws RepoIntegrationException {
		logger.info("Iniciando el test del método getContenidoDoc");
	...
	@Test
	public void testGetContenidoDocVersion() throws RepoIntegrationException {
		logger.info("Iniciando el test del método getContenidoDocVersion");

		final String version = null;
		final ContenidoDocumento contenido = integration.getContenidoDoc(documentId, version);
		Assert.assertNotNull(contenido);
		Assert.assertNull(contenido.getContenido());
	...
	@Test
	public void testGetDocsCarpeta() throws RepoIntegrationException {
		logger.info("Iniciando el test del método getDocsCarpeta");

		final List<Map<String, Metadato>> documentos = integration.getDocsCarpeta(folderId);
		Assert.assertNotNull(documentos);
		Assert.assertTrue(documentos.size() == 1);

		logger.info("Finalizando el test del método getDocsCarpeta");
	}

	@Test
	public void testGetVersionesDocumento() throws RepoIntegrationException {
		...
		Assert.assertNotNull(versiones);
		Assert.assertTrue(versiones.size() == 1);

		logger.info("Finalizando el test del método getVersionesDocumento");
	}

	@Test
	public void testModificaCarpeta() throws RepoIntegrationException {
		logger.info("Iniciando el test del método modificaCarpeta");

		final Map<String, Metadato> metadatos = new HashMap<String, Metadato>();

		final String tipo = context.getCmisExpedienteTipo();
		metadatos.put(tipo, createMetadatoSimple(tipo, "mi tipo...."));

		final Map<String, Metadato> folder = integration.modificaCarpeta(folderId, metadatos);
		Assert.assertNotNull(folder);
		Assert.assertNotNull(folder.get(tipo));

		logger.info("Finalizando el test del método modificaCarpeta");
	}

	@Test
	public void testModificaDocumento() throws RepoIntegrationException {
		logger.info("Iniciando el test del método modificaDocumento");

		final Map<String, Metadato> metadatos = new HashMap<String, Metadato>();
		final String estado = context.getCmisDocumentoEstado();
		metadatos.put(estado, createMetadatoSimple(estado, "ABIERTO"));
		final String csv = context.getCmisDocumentoCSV();
		metadatos.put(csv, createMetadatoSimple(csv, "mi csv...."));

		final Map<String, Metadato> document = integration.modificaDocumento(documentId, metadatos, null);
		Assert.assertNotNull(document);
		Assert.assertNotNull(document.get(estado));
	...
	@Test
	public void testBorrarDocumento() throws RepoIntegrationException {
	...
	private Metadato createMetadatoSimple(final String clave, final Object valor) {
		final Metadato metadato = new MetadatoSimple();
	...
	private Metadato createMetadatoMulti(final String clave, final Object... valor) {
		final Metadato metadato = new MetadatoSimple();
		metadato.setClave(clave);
		metadato.setMultivalor(true);
		metadato.setValor(Arrays.asList(valor));
		return metadato;
	}*/
}
