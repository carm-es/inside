...
public class VisualizacionUtils {
	...
	public void visualizarDocumento(ModelAndView retorno, HttpSession session, HttpServletRequest request) throws Exception {
		...
		if (StringUtils.isBlank(request.getParameter("plantilla"))
			...
			resultado = getVisualizacionPlantilla(session.getId(), request.getParameter(DOCUMENT_ID),
					request.getParameter("metadatosAdicionalesVisualizar"), request.getParameter("plantilla"));
		}

		retorno.addObject("IDENTIFICADORDOCUMENTO", resultado.getObjetoDocumentoInside().getIdentificador());
		...
		Item itemDocumento = insideServiceVisualizacion.documentoInsideToItemVisualizacion(documentoInside, null);
	...
	/**
	 * Recorre la estructura de Ã¡rbol en preorden para obtener una lista
	 * ordenada de Items de Visualizacion. *
	 * 
	 * @param nodo
	 *            Estructura en Ã¡rbol que contiene la informaciÃ³n que
	 *            quiere foliarse.
	 * 
	 * @return Lista ordenada de VisualizacionItem en el orden en que se quieren
	 *         imprimir.
	 * @throws Exception
	 */
	 ...
	 * @param lista
	 */
	private void addVisualizacionItemToList(final Item nodo, int prof, List<VisualizacionItem> lista, int[] marcadores) {
		...
		if (nodo.getHijos() != null) {
			...
			for (Item hijo : nodo.getHijos().getItems().getItems()) {
				...
				nuevosMarcadores[nuevosMarcadores.length - 1] = nodo.getHijos().getItems().getItems().indexOf(hijo) + 1;
	...
	private List<Entry<String, String>> listaPropiedadesToEntryList(final Item nodo) {
		...
		if (nodo.getPropiedadesItem() != null) {
			...
			for (Propiedad propiedad : nodo.getPropiedadesItem().getPropiedades().getPropiedades()) {
				Entry<String, String> entry = new AbstractMap.SimpleEntry<String, String>(propiedad.getClave(),
						propiedad.getValor());
				entryList.add(entry);
			}
		}
	...
	private TipoResultadoVisualizacion getDefaultVisualizacion(String idSession, String documentId, String metadatosAdicionales)
			throws Exception {
		...
		List<MetadatoAdicional> listaAdiconales = visualizar.getMetadatosAdicionales() != null
				? visualizar.getMetadatosAdicionales().getMetadatoAdicional() : null;
		return new TipoResultadoVisualizacion(insideUtilService.visualizarDocumentoEni(visualizar), listaAdiconales,
				InsideConverterDocumento.documentoVisualizacionToInside(visualizar));
	}

	private void setAdicionalesToVisualizar(String adicionales, TipoDocumentoVisualizacionInside visualizar)
		...
		if (StringUtils.isNotEmpty(adicionales)) {
			...
			listAdicionales = mapper.readValue(adicionales, new TypeReference<List<MetadatoAdicional>>() {
			});
		} else {
			listAdicionales = getMetadatosAdicionales(visualizar.getDocumentoEni().getDocumentoEniBinario());
		}
	...
	private TipoResultadoVisualizacion getVisualizacionPlantilla(String idSession, String documentId, String metadatosAdicionales,
		...
		TipoResultadoVisualizacionDocumentoInside result = insideServiceVisualizacion.visualizacionConPlantilla(plantilla,
				visualizar);

		return new TipoResultadoVisualizacion(result, visualizar.getMetadatosAdicionales().getMetadatoAdicional(),
				InsideConverterDocumento.documentoVisualizacionToInside(visualizar));
	}
}
