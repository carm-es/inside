...
public class UsuarioManageController {
	...
	public ModelAndView altaUsuarioWebInside(Locale locale, HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			logger.error("UsuarioManageController.altaUsuarioWebInside --> Error alta usuario web inside.", e);
	...
	public List<ComboItem> cargaComboRoles(Locale locale) throws InsideServiceStoreException {
		...
		for (ObjetoInsideRol insideRol : insideRoles) {
			if (insideRol.getId() != 1) {
				listaReturn.add(new ComboItem(insideRol.getId()+"", messageSource.getMessage(insideRol.getDescripcionLarga(), null, locale), null));
			}
		}
	...
	public List<ComboItem> cargaNumeroProcedimientos(Locale locale) throws InsideServiceStoreException {

		logger.info("[INI] Entramos en cargaNumeroProcedimientos. ");

		List<ObjetoNumeroProcedimiento> listaNumeroProcedimiento = insideService.getNumeroProcedimientoList();

		logger.info("Transformamos en tipo SelectIntemVO para mostrar lista de autocompletado...");
		List<ComboItem> listaReturn = new ArrayList<ComboItem>();
		listaReturn.add(new ComboItem("0", "", null));
		for (ObjetoNumeroProcedimiento objetoNumProc : listaNumeroProcedimiento) {
			listaReturn.add(new ComboItem(objetoNumProc.getId()+"", objetoNumProc.getNumeroProcedimiento(), null));
		}

		logger.info("[FIN] Salimos de cargaNumeroProcedimientos. Total a mostrar: " + listaReturn.size());

		return listaReturn;
	}
	
	@RequestMapping(value = "/altaUsuarioExiteNif", method = RequestMethod.POST)
	@ResponseBody
	public  Map<String, Object> existeNifAlta(@RequestParam("nif") final String nif) 
	{
		...
		try 
		{
			...
			existeUsuario = true;
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_INFO, "El NIF " + nif + " ya existe en Inside. ");
	...
	public ModelAndView altaUsuarioWebInsidePost(ObjetoInsideUsuario usuarioAlta) {
		...
		{	
			...
			{
				logger.debug("ERROR en altaUsuarioWebInsidePost: El NIF no viene informado y es obligatorio.");
				throw new Exception();
			}			
			...
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_INFO, "El NIF: " + usuarioDadoDeAlta.getNif() + " ha sido dado de alta correctamente.");
			retorno.addObject(MENSAJE_USU, mensaje);
		...
		{
			logger.debug(e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, "Error en el alta. Compruebe que los valores obligatorios son los correctos y están informados.");
			retorno.addObject(MENSAJE_USU, mensaje);
			usuarioAlta.setNif("");
			usuarioAlta.setNumeroProcedimiento("");
			retorno.addObject("rolesDisponibles", this.getListadoComboRoles());
	...
	public void setListadoNumeroProcedimientos(
			List<ComboItem> listadoNumeroProcedimientos) {
	...
	public Map<String, Object> getUnidadesOrganicas(HttpServletRequest request) {
		...
		try {
			List<ObjetoInsideUnidad> unidades = insideService.getUnidadesUsuario(identificadorUsuario, true);
	...
	public ModelAndView updateUnidadActiva(HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			ObjetoInsideUnidad unidadUsuarioActiva = insideService.getUnidadesUsuario(usuario.getNif(), false).get(0);
						
			UserAuthentication auth = (UserAuthentication) SecurityContextHolder.getContext().getAuthentication();
			String unidadOrganica = unidadUsuarioActiva.getCodigo() + " - " + unidadUsuarioActiva.getNombre();
			...
			ObjetoInsideRol objetoInsideRol = insideService.getRolUnidadUsuario(codigoUnidad, usuario.getNif());
			...
			List<GrantedAuthority> roles = RolesUsuarioAsignacionUtils.asignarRolesUsuario(objetoInsideRol.getDescripcion());
...
//			}else if ("ALTA_USUARIOS_ROLE".equals(objetoInsideRol.getDescripcion()) || ("REDACTOR_ROLE".equals(objetoInsideRol.getDescripcion())) ) {
//				roles.add(new SimpleGrantedAuthority("USER_ROLE"));
//			} 
			
			UserAuthentication authActualizado =new UserAuthentication(auth.getNif(), null, roles, null, auth.getNif(), null);
	...
	public ModelAndView listaUsuariosEnUnidadOrganica(HttpServletRequest request, Locale locale) {
		...
		ObjetoInsideUsuario usuarioEnSesion = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
		...
		try 
		{
			...
			List<ObjetoInsideUsuario> usuariosEnUnidadOrganica = insideService.getUsuariosUnidadOrganica(usuarioEnSesion, locale);
			...
			retorno.addObject("titulo", messageSource.getMessage("listaUsuarios.title", option, locale)); //listaUsuarios.title=Usuarios en la Unidad Organica {0}
	...
	public ModelAndView borrarUsuario(HttpServletRequest request, Locale locale) {
		ModelAndView retorno = new ModelAndView("listaUsuariosEnUnidadOrganica");
			...
			try 
			{
				...
				insideService.eliminarUnidadUsuario(unidadOrganicaActivaDelUsuarioLogadoCodigo, numeroProcedimiento, idUsuario);
	...
	public ModelAndView modificarUsuarioWebInside(Locale locale, HttpSession session, HttpServletRequest request) {
	...
	public ModelAndView guardarModificacionUsuarioWebInsidePost(ObjetoInsideUsuario usuarioAlta, HttpServletRequest request,Locale locale) {
		...
		{			
			//borrar relacion unidad Usuario
			insideService.eliminarUnidadUsuario(usuarioAlta.getUnidadOrganicaActiva(), usuarioAlta.getNumeroProcedimiento(), usuarioAlta.getNif());
						
			ObjetoInsideUsuario usuarioDadoDeAlta = insideService.altaUsuario(usuarioAlta);						
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_INFO, "El NIF: " + usuarioDadoDeAlta.getNif() + " ha sido modificado correctamente.");
			retorno.addObject(MENSAJE_USU, mensaje);
