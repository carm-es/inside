...
public class SignatureUtils {
	...
	public Firmas setSignaturesExp(byte[] signature, String ref) throws XPathExpressionException, ParserConfigurationException, SAXException,
			IOException, TransformerException, JAXBException, TransformerFactoryConfigurationError {
		...
		//String nodofirma = es.mpt.dsic.inside.service.util.XMLUtils.updateSignature("AFIRMA/Signature", signature, "#EXP_INDICE_CONTENIDO" + ref, false);
		//firmaConCertificado.setSignature(marshaller.unmarshallDataSignature(nodofirma.getBytes()));
		
		//pasamos todo el eni para hacer la firma y guardo la firma(el nodo ds:signature) original en base64 en firmaBase64
		//para al descargar el eni decodificarla y quitar el nodo firmabase64 y poner el ds:signature
//		String nodofirma = es.mpt.dsic.inside.service.util.XMLUtils.signatureString("expediente/indice/Signature", signature);
		String nodofirma = es.mpt.dsic.inside.service.util.XMLUtils.signatureString("//*[local-name()='Signature']", signature);
		
		firmaConCertificado.setFirmaBase64(Base64.encode(nodofirma.getBytes(es.mpt.dsic.inside.service.util.XMLUtils.UTF8_CHARSET)));
