...
public class PdfUtils {
	 ...
	 */
	public Map<String,byte[]> createPdfReceivedExpedient(ObjetoInsideUsuario user, ObjetoExpedienteToken token , String basePath) throws Exception {
		...
		try {
			...
			FileOutputStream fileOutputStream = new FileOutputStream(basePath + token.getIdentificador() + 
																	 WebConstants.ACTA_DE_INGRESO_SUFIJO + 
			...
			doc.newPage();
			printTitle(doc,	context.getMessage("pdf.remisionEnLaNube.titulo", null, WebConstants.LOCALE_DEFAULT));
			...
			if (registrar) {
				...
				// Ingreso
				JustificanteType justificanteType = recUtils.doRegistroElectronico(token.getIdentificador() + 
																				   WebConstants.ACTA_DE_INGRESO_SUFIJO + 
																				   WebConstants.FORMAT_PDF,
				...
				InsideWSUtils.writeFile(new File(basePath),
										basePath + token.getIdentificador() + WebConstants.JUSTIFICANTE_REGISTRO_ELECTRONICO_SUFIJO
										+ WebConstants.FORMAT_PDF,
				...
				mapaActaANDJustificante.put(WebConstants.JUSTIFICANTE_REGISTRO_ELECTRONICO_SUFIJO, justificante);
	 ...
	 */
	private PdfPTable fillPdf(LinkedHashMap<String, String> dataPDF, Map<String, String> data, PdfPTable generalData) {
		for (Map.Entry<String, String> entry : dataPDF.entrySet()) {
			generalData
					.addCell(createCellClave(context.getMessage(entry.getKey(), null, WebConstants.LOCALE_DEFAULT),
							Element.ALIGN_LEFT, PdfPCell.NO_BORDER, DEFAULT_FONT_BOLD, -1));
			generalData.addCell(createCellValue(data.get(entry.getValue()), Element.ALIGN_LEFT, PdfPCell.NO_BORDER,
					DEFAULT_FONT_BASE, -1));
	 ...
	 */
	private PdfPCell createCellClave(String texto, int alignment, int border, Font font, int profundidad) {
	 ...
	 * @return the pdf p cell
	 */
	private PdfPCell createCellValue(String texto, int alignment, int border, Font font, int profundidad) {
	...
	public byte[] addCabecera(byte[] pdfEntrada, Document doc) throws Exception {
		...
		try {
			...
			for (int i = 1; i <= n; i++) {
				...
				cabecera.writeSelectedRows(0, -1, doc.leftMargin(), stamp.getReader().getPageSize(i).getHeight() - 15,	stamp.getOverContent(i));
			}
