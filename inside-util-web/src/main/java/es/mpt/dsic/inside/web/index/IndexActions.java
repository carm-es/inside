...
public class IndexActions {
	...
	public static TipoExpediente createFolder(TipoExpediente exp, String path, String name) {
		...
		if (!"".equals(path)) {
			...
			TipoCarpetaIndizada carpetaPadre = getPath(
					retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
					splitPath);
			carpetaPadre.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().add(carpeta);
		} else {
			retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().add(carpeta);
		}
		return retorno;
	}

	public static TipoExpediente createFolderComplete(TipoExpediente exp, TipoIndiceContenido indice, String path) {
		TipoExpediente retorno = exp;
		if (!"".equals(path)) {
			String[] splitPath = path.split("/");
			TipoCarpetaIndizada carpetaPadre = getPath(
			...
			for (Object objt : indice.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada())
				retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().add(objt);
		}
		return retorno;
	}

	public static TipoExpediente createDocument(TipoExpediente exp, String path, String id, String valorHuella,
			String funcionResumen, XMLGregorianCalendar fechaIncorpExp, String ordenDocExp) {
		TipoExpediente retorno = exp;
		...
		if (StringUtils.isNotBlank(path)) {
			...
			if (carpetaPadre != null) {
				document.setOrdenDocumentoExpediente(calculateOrdenExpediente(
						retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada()));
			...
			document.setOrdenDocumentoExpediente(calculateOrdenExpediente(
					retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada()));
			retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().add(document);
		}
		return retorno;
	}
			...
			if (obj instanceof TipoCarpetaIndizada) {
				if (((TipoCarpetaIndizada) obj).getIdentificadorCarpeta().equals(actualPath)) {
					if (path.length == 1) {
						...
						retorno = getPath(((TipoCarpetaIndizada) obj).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
	...
	public static TipoExpediente moveElement(TipoExpediente exp, String pathOrigen, String identificador, String pathDestino,
			boolean onlyDeleteElement, Integer orden, HttpSession session) {
		TipoExpediente retorno = exp;
		...
		if (onlyDeleteElement || !vPath.equals(pathDestino)) {
			if (!pathOrigen.equals("")) {
				...
				objetosOrigen = getPath(
						retorno.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
						splitPath).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada();
			} else {
			...
			if (CollectionUtils.isNotEmpty(objetosOrigen)) {
				...
				if (!onlyDeleteElement) {
					...
					if (orden != null) {
						int ordenFinal = calculateOrdenFinal(ordenOriginal, orden, pathOrigen.equalsIgnoreCase(pathDestino));
	...
	private static int calculateOrdenFinal(int ordenOriginal, Integer orden, boolean movimientoAlMismoNivel) {
	...
	public static void modificarDocumentoSession(HttpSession session, Object elementoeliminar, boolean eliminar) {
	...
	private static void removeDocCascade(TipoCarpetaIndizada carpeta, HttpSession session) {
		if (CollectionUtils.isNotEmpty(carpeta.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada())) {
	...
	public static void deteleElement(HttpSession session, String dataDoc) {
		if (session != null && session.getAttribute("docs") != null && session.getAttribute("newDocs") != null) {
			((List<String>) session.getAttribute("docs")).remove(dataDoc);
			((HashMap<String, String>) session.getAttribute("newDocs")).remove(dataDoc);
		}
	}

	public static TipoExpediente renameElement(TipoExpediente exp, String path, String actualName, String newName) {
		...
		TipoCarpetaIndizada carpeta = getPath(
				expedientType.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
				splitPath);
	...
	public static boolean checkMoveElementNoRepeatInLevel(TipoExpediente expedientType, String identificador, String path) {
		...
		findElementsSameLevel(listElementSameLevel, splitPath,
				expedientType.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(), 0);
	...
	public static boolean checkNewNameNoRepeatInLevel(TipoExpediente expedientType, String path, String actualName,
			String newName) {
		...
		List<Object> listElementSameLevel = new ArrayList<Object>();
		findElementsSameLevel(listElementSameLevel, splitPath,
				expedientType.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(), 1);
		boolean checkNewNameNoRepeatInLevel = true;

		for (Object obj : listElementSameLevel) {
			if (obj instanceof TipoCarpetaIndizada && ((TipoCarpetaIndizada) obj).getIdentificadorCarpeta().equals(newName)) {
	...
	private static List<Object> findElementsSameLevel(List<Object> listElementSameLevel, String[] splitPath, List<Object> list,
			int level) {
		if (splitPath.length == level) {
			...
			for (Object obj : list) {
				if (obj instanceof TipoCarpetaIndizada
					...
					findElementsSameLevel(listElementSameLevel, splitPathNew,
							((TipoCarpetaIndizada) obj).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(), level);
				}
			}
		}
