...
public class InsideValidationDataServiceImpl implements InsideValidationDataService {
	...
	private static String VERSION_NTI_EXP = "http://administracionelectronica.gob.es/ENI/XSD/v1.0/expediente-e";
	private static String VERSION_NTI_DOC = "http://administracionelectronica.gob.es/ENI/XSD/v1.0/documento-e";
	
	@Override
	public TipoDocumentoConversionInside validaTipoDocumentoConversionInside (TipoDocumentoConversionInside documento, 
															                  boolean fillEmptyFields) throws InsideValidationDataException {
		// VALIDACIÓN
		...
		if (documento.getMetadatosEni().getEstadoElaboracion().getValorEstadoElaboracion() == null) {
			throw new InsideValidationDataException ("El valor del metadato Estado Elaboración es incorrecto");
		}
		
		if (documento.getMetadatosEni().getOrgano() == null || documento.getMetadatosEni().getOrgano().size() == 0) {
		...
		if (documento.getMetadatosEni().getTipoDocumental() == null) {
			throw new InsideValidationDataException ("El valor del metadato Tipo Documental es incorrecto");
		}
		...
		if (fillEmptyFields) {
			if (documento.getMetadatosEni().getFechaCaptura() == null) {
				try {				
					documento.getMetadatosEni().setFechaCaptura(InsideConverterXmlGregorianCalendar.calendarToXmlCalendar(new GregorianCalendar()));
	...
	public TipoExpedienteConversionInside validaTipoExpedienteConversionInside (TipoExpedienteConversionInside expediente, boolean fillEmptyFields) throws InsideValidationDataException {
		...
		if (expediente.getOpcionesVisualizacion() == null) {
			throw new InsideValidationDataException ("No se han enviado las Opciones de Visualización del índice del expediente");			
		}
	...
	private void validaTipoIndiceConversion (Object obj, boolean fillEmptyFields) throws InsideValidationDataException {
		
		if (obj instanceof TipoIndiceConversion) {
			TipoIndiceConversion indiceConversion = (TipoIndiceConversion) obj;
			
			List<Object> hijos = indiceConversion.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada();
			...
			if (fillEmptyFields) {
				if (indiceConversion.getFechaIndiceElectronico() == null) {
					try {
						indiceConversion.setFechaIndiceElectronico(InsideConverterXmlGregorianCalendar.calendarToXmlCalendar(new GregorianCalendar()));
					} catch (InsideConverterException e) {
						//
					}
				}
			...
			if (documentoConversion.getFuncionResumen() == null) {
				throw new InsideValidationDataException ("No se ha enviado la funcion resumen del documento " + documentoConversion.getIdentificadorDocumento());
			}
			...
			try {
				...
				throw new InsideValidationDataException ("El orden del documento debe ser un numero. Documento:  " + documentoConversion.getIdentificadorDocumento());
			}
	...
	public TipoDocumentoValidacionInside validaTipoDocumentoValidacionInside (TipoDocumentoValidacionInside documento, boolean fillEmptFileds) throws InsideValidationDataException {
		...
		if (documento.getOpcionesValidacionDocumento() == null) {
			throw new InsideValidationDataException ("No se han enviado las opciones de validación del documento");
		}
	...
	public TipoExpedienteValidacionInside validaTipoExpedienteValidacionInside (TipoExpedienteValidacionInside expediente, boolean fillEmptFileds) throws InsideValidationDataException {
		...
		if (expediente.getOpcionesValidacionExpediente() == null) {
			throw new InsideValidationDataException ("No se han enviado las opciones de validación del expediente");
		}
	...
	public TipoDocumentoVisualizacionInside validaTipoDocumentoVisualizacionInside (TipoDocumentoVisualizacionInside documento, boolean fillEmptyFields) throws InsideValidationDataException {
		...
		if (documento.getDocumentoEni().getDocumentoEniBinario() == null && documento.getDocumentoEni().getDocumentoEniTipo() == null) {
			throw new InsideValidationDataException ("No se ha enviado el documento ENI ni como binario ni como tipo");
		}
		if (documento.getOpcionesVisualizacionDocumento() == null) {
			throw new InsideValidationDataException ("No se han enviado las Opciones de Visualización del documento");			
		}
		return documento;
	}
}
