...
public class InSideAuditoriaServiceImpl implements InSideAuditoriaService {
	...
	@Override
	public void auditoriaRemisionNube(ObjetoInsideUsuario user, ObjetoExpedienteToken token, String sessionId) throws Exception {
		logger.debug("InSideAuditoriaServiceImpl.auditoriaRemisionNube: Inicio registro de auditoria remision en la nube.");
		...
		Map<String, byte[]> mapaActaANDJustificante = pdfUtils.createPdfReceivedExpedient(user, token, temporalDir);
		...
		ObjetoAuditoriaToken objectAuditoriaToken = new ObjetoAuditoriaToken(user, token,
				...
				mapaActaANDJustificante.get(WebConstants.JUSTIFICANTE_REGISTRO_ELECTRONICO_SUFIJO), new Date(),
				user.getUnidadOrganicaActiva() != null
						? user.getUnidadOrganicaActiva().substring(0, user.getUnidadOrganicaActiva().lastIndexOf(" -")) : null,
				user.getUnidadOrganicaActiva() != null ? user.getUnidadOrganicaActiva().substring(
						user.getUnidadOrganicaActiva().lastIndexOf("- ") + 2, user.getUnidadOrganicaActiva().length()) : null,
		...
		logger.debug("InSideAuditoriaServiceImpl.auditoriaRemisionNube: Fin de registro de auditoria remision en la nube.");
	}
