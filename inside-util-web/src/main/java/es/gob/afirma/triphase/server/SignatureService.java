...
public final class SignatureService extends HttpServlet {
	...
	/**
	 * Separador que debe usarse para incluir varios certificados dentro del
	 * mismo par&aacute;metro.
	 */
	private static final String PARAM_NAME_CERT_SEPARATOR = ","; //$NON-NLS-1$

	/**
	 * Nombre del par&aacute;metro que identifica los nodos que deben
	 * contrafirmarse.
	 */
	...
	/**
	 * Or&iacute;genes permitidos por defecto desde los que se pueden realizar
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			final InputStream configIs = new FileInputStream(System.getProperty("config.path")
		...
		try {
			...
			throw new RuntimeException(
					"La clase DocumentManager indicada no existe (" + configData.getProperty(CONFIG_PARAM_DOCUMENT_MANAGER_CLASS) + "): " + e, e //$NON-NLS-1$ //$NON-NLS-2$
			);
		}

		try {
			final Constructor<?> docManagerConstructor = docManagerClass.getConstructor(String.class, String.class, String.class);
			...
			DOC_MANAGER = (DocumentManager) docManagerConstructor.newInstance(indir, outdir, configData.getProperty(OVERWRITE_PARAM));
		} catch (final Exception e) {
			try {
	...
	protected void service(final HttpServletRequest request, final HttpServletResponse response) {
		...
		final String allowOrigin = configData.getProperty(CONFIG_PARAM_ALLOW_ORIGIN, ALL_ORIGINS_ALLOWED);
		...
		try {
			if (parameters.containsKey(PARAM_NAME_EXTRA_PARAM)) {
				extraParams.load(new ByteArrayInputStream(Base64.decode(parameters.get(PARAM_NAME_EXTRA_PARAM).trim(), true)));
			}
		} catch (final Exception e) {
		...
		if (docId != null) {
			try {
				...
				docBytes = DOC_MANAGER.getDocument(docId, signerCertChain, extraParams, request.getSession());
		...
		if (AOSignConstants.SIGN_FORMAT_PADES.equalsIgnoreCase(format)
			...
			final boolean installXmlDSig = Boolean.parseBoolean(configData.getProperty(CONFIG_PARAM_INSTALL_XMLDSIG,
		...
		if (PARAM_VALUE_OPERATION_PRESIGN.equalsIgnoreCase(operation)) {
			...
			try {
				if (PARAM_VALUE_SUB_OPERATION_SIGN.equalsIgnoreCase(subOperation)) {
					...
					preRes = prep.preProcessPreCounterSign(docBytes, algorithm, signerCertChain, extraParams, target);
				} else {
			...
			try {
				if (PARAM_VALUE_SUB_OPERATION_SIGN.equals(subOperation)) {
					signedDoc = prep.preProcessPostSign(docBytes, algorithm, signerCertChain, extraParams, sessionData);
				} else if (PARAM_VALUE_SUB_OPERATION_COSIGN.equals(subOperation)) {
					signedDoc = prep.preProcessPostCoSign(docBytes, algorithm, signerCertChain, extraParams, sessionData);
					...
					signedDoc = prep.preProcessPostCounterSign(docBytes, algorithm, signerCertChain, extraParams, sessionData,
							target);
				} else {
					throw new AOException("No se reconoce el codigo de sub-operacion: " + subOperation); //$NON-NLS-1$
				}
			...
			try {
				newDocId = DOC_MANAGER.storeDocument(docId, signerCertChain, signedDoc, extraParams, request.getSession());
			} catch (final IOException e) {
			...
			out.println(new StringBuilder(newDocId.length() + SUCCESS.length()).append(SUCCESS).append(newDocId).toString());
			out.flush();
