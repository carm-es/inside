...
public class MailServiceImpl implements MailService {
	...
	public String sendToken(ObjetoExpedienteToken objetoExpedienteToken, String idSesion, String URLDeEntornoEjecucion) throws Exception {
		...
		if (!active) return noActivoEnvioCorreo;
	...
	private void construirMensaje(ObjetoExpedienteToken objetoExpedienteToken, String to, MimeMessageHelper helper)throws MessagingException
	{
		...
		try 
		{
			...
			if(objetoExpedienteToken.getFechaCaducidad()!=null)
			{
				...
				texto = texto + "\n\nEl Credencial de Acceso tiene fecha de caducidad en: " + fechaQueCaducaToken + ".";
			}
			else
			{
			...
			textoXML = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"
									+"<token>\n"
										...
										+"\t<UUID>" + objetoExpedienteToken.getUuid() + "</UUID>\n"
										+"\t<FechaCaducidad>" + fechaQueCaducaToken + "</FechaCaducidad>\n"
									+"</token>";
			
			texto = texto
						+"\nIdentificador ... "+objetoExpedienteToken.getIdentificador()
						+"\nCSV ............. "+objetoExpedienteToken.getCsv()
						+"\nUUID ............ "+objetoExpedienteToken.getUuid();
			
			

			
			if(objetoExpedienteToken.getAsunto()!=null && !objetoExpedienteToken.getAsunto().trim().equals(""))
			{
			...
			e.printStackTrace();
			throw new MessagingException("Error al construir el mensaje del correo Credenciales de Acceso"+e);
		}
	...
	private void adjuntarFichero(ObjetoExpedienteToken objetoExpedienteToken, String idSesion, MimeMessageHelper helper) throws IOException
	{
		...
		try 
		{
			...
			if(pathTemp!=null && !pathTemp.trim().equals(""))
			{
				...
				texto=texto+"\n\n\nSe ha adjuntado un fichero con los Credenciales de Acceso llamado: "+filename;
			}
	...
	private String escribir(byte[] data, String folder, String filename, boolean create) throws IOException {
