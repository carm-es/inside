...
public class InsideUsersServiceImpl implements InsideUsersService {
	...
	@Override
	public ObjetoInsideAplicacion getAplicacionBySerialNumber(String serialNumberCertificado) throws InsideUsersServiceException {
	...
	public ObjetoInsideUsuario getUsuario(String nif) throws InsideUsersServiceException {
		...
		if (usuario != null	&& usuario.getActivo() && CollectionUtils.isNotEmpty(usuario.getListaUnidades())) {
			...
			InsideRol insideRolEntity = (InsideRol) insideServiceJta.findObjeto(InsideRol.class, criteriasRol);
			ObjetoInsideRol objetoInsideRol = InsideServiceStoreHibernateConverterInsideRol.toInside(insideRolEntity);			
			usuarioInside.setRol(objetoInsideRol);			
			
			criteriasUnidadOrganica.add(Restrictions.eq("id", unidadUsuarioActiva.getUnidad().getId()));
			UnidadOrganica unidadOrganicaActiva = (UnidadOrganica) insideServiceJta.findObjeto(UnidadOrganica.class, criteriasUnidadOrganica);
			usuarioInside.setUnidadOrganicaActiva(unidadOrganicaActiva.getCodigoUnidadOrganica()+" - "+unidadOrganicaActiva.getNombreUnidadOrganica());
		}
