...
public class MetadatoMetadatosMap extends Metadato<Object> {
	...
	public MetadatoMetadatosMap() throws InsideServiceMetadatoValueExtractionException {
		InsideServiceMetadatosMapValidator metadatosValidator = new InsideServiceMetadatosMapValidator(this);
	...
	private class InsideServiceMetadatosMapValidator implements MetatadatoValidator<Object>{
			...
			throws InSideServiceValidationException,InsideServiceMetadatoValueExtractionException
		{
			...
			for (Metadato<?> metadato : this.metadatoMap.getMetadatosDef().values()) {
				logger.debug("Validando metadato " + metadato.getNombre() + " dentro del metadato " + this.metadatoMap.getNombre());
				
				cleanMetadato(metadato, objeto);
				
				logger.debug("Validado correctamente el metadato " + metadato.getNombre() + " dentro del metadato " + this.metadatoMap.getNombre());	
			}
	
			return objeto;
		}
		
		private void cleanMetadato(Metadato<?> metadato,Object objeto) throws InSideServiceMetadatoValidationException, InsideServiceMetadatoValueExtractionException
		{
			PropertyDescriptor pd = BeanUtils.getPropertyDescriptor(objeto.getClass(),metadato.getNombre());
			if(pd == null){
				throw new InsideServiceMetadatoValueExtractionException("No existe un PropertyDescriptor para el metadato " + metadato.getNombre() + " en el objeto " + objeto.getClass());
			}
			Method readMethod = pd.getReadMethod();
			Method writeMethod = pd.getWriteMethod();
			
			if(readMethod == null || writeMethod == null){
				throw new InsideServiceMetadatoValueExtractionException("No se puede acceder en lectura o en escritura al metadato " + metadato.getNombre() + " en el objeto " + objeto.getClass());
			}
			...
			try {
				...
				throw new InsideServiceMetadatoValueExtractionException("Error leyendo metadato " + metadato.getNombre() + " en el objeto " + objeto.getClass() + " : " + e.getMessage(),e);
			}
			...
			try {
				...
				throw new InsideServiceMetadatoValueExtractionException("Error estableciendo valor de metadato " + metadato.getNombre() + " en el objeto " + objeto.getClass() + " : " + e.getMessage(),e);
			}
