...
public class InsideServiceIdentifierGenerator {
	...
	public static String generateObjetoInsideIdentificador(ObjetoInside<?> objeto)
			throws InSideServiceException 
		{
			...
			try {
				//Añadimos las partes H (Hash del host) M (Tiempo en milisegundos de la máquina) + R (aleatorio alfanumérico hasta completar 30)
				secuencia+= "H" + java.net.InetAddress.getLocalHost().hashCode() + "M" + System.currentTimeMillis() + "R";
			...
			secuencia+= new BigInteger(130, new SecureRandom()).toString(32).toUpperCase().substring(0, 30 - secuencia.length());
