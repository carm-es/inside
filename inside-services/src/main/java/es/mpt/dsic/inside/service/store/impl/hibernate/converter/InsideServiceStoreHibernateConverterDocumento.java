...
public class InsideServiceStoreHibernateConverterDocumento
	...
	public DocumentoInside toEntity(ObjetoDocumentoInside documento, Session session)
		...
		if (documento.getMetadatos() != null) {
			...
			entity.setOrigenCiudadanoAdministracion(documento.getMetadatos().isOrigenCiudadanoAdministracion());
			...
			if (documento.getFirmas() != null) {
				...
				for (FirmaInside firmaInside : documento.getFirmas()) {
					...
					try {
						firmaEntity.setFirmaInside(InsideServiceStoreHibernateConverterFirma.toEntity(firmaInside));
					} catch (InsideServiceStoreHibernateConverterFirmaException e) {
						throw new InsideServiceStoreException("Error convirtiendo FirmaInside a Entidad de Hibernate", e);
					}
	...
	public void setInsideMetadatosAdicionales(ObjetoDocumentoInside documento, DocumentoInside entity) {
		Set<DocumentoInsideMetadatosAdicionales> documentoInsideMetadatosAdicionaleses = new HashSet<DocumentoInsideMetadatosAdicionales>();
		for (ObjetoInsideMetadatoAdicional metadatoAdicional : documento.getMetadatos().getMetadatosAdicionales()) {
			DocumentoInsideMetadatosAdicionales metadatoAaditionaEntity = new DocumentoInsideMetadatosAdicionales();
	...
	@Override
	public ObjetoDocumentoInside toInside(DocumentoInside entity, Session session) throws InsideServiceStoreException {
		...
		documento.setVersion(new ObjetoInsideVersion(entity.getVersion(), (GregorianCalendar) fechaVersion));
		...
		if (entity.getEstadoElaboracion() != null) {
			estadoElaboracion.setValorEstadoElaboracion(
					ObjetoDocumentoInsideMetadatosEstadoElaboracionEnumeracion.fromValue(entity.getEstadoElaboracion()));
		}
		...
		documento.getMetadatos().setOrigenCiudadanoAdministracion(
				entity.getOrigenCiudadanoAdministracion() != null && entity.getOrigenCiudadanoAdministracion());

		List<ObjetoInsideMetadatoAdicional> metadatosAdicionales = new ArrayList<ObjetoInsideMetadatoAdicional>();
		for (DocumentoInsideMetadatosAdicionales metEntry : entity.getDocumentoInsideMetadatosAdicionaleses()) {
		...
		if (entity.getTipoDocumental() != null) {
			documento.getMetadatos()
					.setTipoDocumental(ObjetoDocumentoInsideMetadatosTipoDocumental.fromValue(entity.getTipoDocumental()));
		}
	...
	public List<ObjetoExpedienteInside> getExpedientesAsociados(String identificador, Session session)
		...
		InsideServiceStoreHibernateConverterExpediente converter = new InsideServiceStoreHibernateConverterExpediente();
		...
		for (ExpedienteInsideIndiceDocumentoIndizado entity : objetos) {
			...
			if (entity.getExpedienteInsideIndice() != null) {
				...
				retorno = converter.getExpediente(entity.getExpedienteInsideIndice().getId(), session, retorno);

			} else {
				// si el documento esta vinculado a una carpeta dentro del
				// expediente
				retorno = converter.getExpedienteCarpeta(entity.getExpedienteInsideIndiceCarpetaIndizada(), session, retorno);
			}
		}
