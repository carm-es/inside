...
//        bp.getRequestContext().put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, endpointURL); 
	...
	public List<ResultadoValidacionDocumento> validaDocumentoENI(byte[] docuemnto, Validaciones validaciones) throws InsideServiceValidacionException 
	{
		// CARM ### v2.0.7.1
		if (!this.isActivo()) {
		...
		RespuestaValidacionENI respuesta = scMtom.validarDocumentoENI(aplicacionInfo, docEntrada, versionSchemas, validaciones);		
	...
	public List<ResultadoValidacionDocumento> validaFirmaDocumentoENI(byte[] docuemnto) throws InsideServiceValidacionException 
	{
		// CARM ### v2.0.7.1
		if (!this.isActivo()) {
			throw new InsideServiceValidacionException("El WS de VALIDACION no se encuentra activo");
	...
	public List<ResultadoValidacionExpediente> validaExpedienteENI(byte[] expediente, Validaciones validaciones) throws InsideServiceValidacionException 
	{
		// CARM ### v2.0.7.1
		if (!this.isActivo()) {
			throw new InsideServiceValidacionException("El WS de VALIDACION no se encuentra activo");
		...
		RespuestaValidacionENI respuesta = scMtom.validarExpedienteENI(aplicacionInfo, expEntrada, versionSchemas, validaciones);
	...
	public static TipoResultadoValidacionExpedienteInside listResultadoValidacionExpedienteToTipoResultadoValidacionExpedienteInside (List<Detalle> resultados) {
		
		TipoResultadoValidacionExpedienteInside res = new TipoResultadoValidacionExpedienteInside ();
				
		if (resultados != null) {
			for (Detalle elemento : resultados) {
				TipoResultadoValidacionDetalleExpedienteInside detalle = new TipoResultadoValidacionDetalleExpedienteInside ();
				detalle.setResultadoValidacion(elemento.getCodigoRespuesta().contains("000]")?true:false);
	...
	private List<ResultadoValidacionExpediente> formatValidacionENIExpedienteInside(RespuestaValidacionENI respuestaWSValidacionEN)
	{
		List<ResultadoValidacionExpediente> resultadosValidacion = new ArrayList<ResultadoValidacionExpediente>();	
		...
		if (respuestaWSValidacionEN != null) {
			for (Detalle elemento : respuestaWSValidacionEN.getDetalle()) {
				...
				{
					rve.setTipoValidacion(OpcionValidacionExpediente.TOVE_01);// Validacion esquemas espedientes
				}
	...
	private List<ResultadoValidacionDocumento> formatValidacionENIDocumentoInside(RespuestaValidacionENI respuestaWSValidacionEN)
	{
		List<ResultadoValidacionDocumento> resultadosValidacion = new ArrayList<ResultadoValidacionDocumento>();			
