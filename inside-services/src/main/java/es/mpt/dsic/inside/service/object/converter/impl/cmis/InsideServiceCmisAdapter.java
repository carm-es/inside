...
public class InsideServiceCmisAdapter implements InsideServiceAdapter {
	...
	/**
	 * Almacena una firma en el repositorio CMIS un
	 * conjunto de firmas 
		...
		for (FirmaInside firma : firmas) {
			if(firma.getContenidoFirma() != null && firma.getContenidoFirma() != null){
				...
				if(contenidoFirma instanceof ContenidoFirmaCertificadoAlmacenableInside){
					...
					try {
						String firmaName = idENI + "_" + firma.getIdentificadorEnDocumento() + "_" + System.currentTimeMillis();
						
						if (StringUtils.isNotEmpty(documentoIdENI)) {	
							metadatosRepository.put(documentoIdENI, firmaName);
						}
						Document documentoFirmaCmis = cmisAdapter.altaDocumento(firmaName,
															                    folder,
															                    documentoObjectTypeId,
																				contenidoFirmaAlmacenado.getValorBinario(),
																				contenidoFirmaAlmacenado.getMime(),
																				metadatosRepository,
																				false);
						contenidoFirmaAlmacenado.setValorBinario(null);
						contenidoFirmaAlmacenado.setIdentificadorRepositorio((String) documentoFirmaCmis.getId());							
	...
	public ObjetoExpedienteInside almacenaObjetoExpedienteInside (ObjetoExpedienteInside expediente, byte[] bytesContenido) throws InsideServiceAdapterException{
	...
	/**
	 * Almacena un documento electrónico en el repositorio CMIS, siempre que éste tenga contenido.
	 * Si no estaba almacenado crea su carpeta	 
	 * En caso contrario modifica sus metadatos y almacena firmas nuevas
	 * @param documento
	 * @return
	 * @throws InsideServiceAdapterException
	 */
	public ObjetoDocumentoInside almacenaObjetoDocumentoInside(ObjetoDocumentoInside documento) throws InsideServiceAdapterException{
		ObjetoDocumentoInsideMetadatos metadatosEntrada = documento.getMetadatos();

		if (metadatosEntrada == null) {
			throw new InsideServiceAdapterException ("Los metadatos del documento de entrada no pueden ser nulos");
		}
		...
		try {
			...
			try {
				ObjetoDocumentoInside objetoDocumentoInsideBbdd = insideService.getDocumento(documento.getIdentificador());
				if(objetoDocumentoInsideBbdd != null) {
					documento.setIdentificadorRepositorio(objetoDocumentoInsideBbdd.getIdentificadorRepositorio());
				}
			...
			if(documento.getIdentificadorRepositorio() != null){
				...
				cmisAdapter.modificarDocumento(documento.getIdentificadorRepositorio(), metadatosRepository, contenidoDocumento);
				...
				documentoCmis = cmisAdapter.altaDocumento(docName,
						rutaDocumentoAlta, 
						documentoObjectTypeId, 
						contentMarshal, 
						mimeType, 
			...
			String mensajeError = String.format("No ha podido almacenar el documento con identificador %s en el repositorio " + rce.getMessage(), documento.getIdentificador());
			logger.error(mensajeError);
			throw new InsideServiceAdapterException (mensajeError, rce);
		} catch (Exception rce) {
			String mensajeError = String.format("No ha podido insertarse el documento con identificador %s en el repositorio: " + rce.getMessage(), documento.getIdentificador());
			logger.error(mensajeError);
			throw new InsideServiceAdapterException (mensajeError, rce);
		}
	...
	public ObjetoDocumentoInside recuperaDocumentoInside(ObjetoDocumentoInside documento)
		...
		if (documento.getIdentificadorRepositorio() != null) {
			...
			try {		
				...
				if (contenidoDocumento != null 
						&& contenidoDocumento.getContenido().length > 0) {					
					convertDocumentResponseToObjetoDocumentoInside(
							contenidoDocumento.getContenido(), documento);
				}
				
			} catch (Exception rce) {
					String mensajeError = String.format("No ha podido obtenerse el documento con identificador %s " + rce.getMessage(), documento.getIdentificador());
					logger.error(mensajeError);
					throw new InsideServiceAdapterException (mensajeError, rce);
			}
		...
		logger.debug("convertFromRepository: documento con identificador de Repositorio " + documento.getIdentificadorRepositorio() + " convertido correctamente");
		return documento;		
	...
	public ObjetoExpedienteInside recuperaExpedienteInside(ObjetoExpedienteInside expediente)
		...
		try {		
			...
			String mensajeError = String.format("No ha podido obtenerse el objeto del repositorio cuando intentaba consultarse el expediente con identificador %s " + rce.getMessage(), expediente.getIdentificador());
			logger.error(mensajeError);
			throw new InsideServiceAdapterException (mensajeError, rce);
		}
		return expediente;
	...
	@Override
	public ObjetoDocumentoInside eliminaDocumentoInside(
	...
	@Override
	public ObjetoExpedienteInside eliminaExpedienteInside(
			...
			if (expediente.getIndice().getFirmas() != null) {
				...
				for (FirmaInside firma : expediente.getIndice().getFirmas()) {
					...
					if( contenidoFirma != null && contenidoFirma instanceof ContenidoFirmaCertificadoAlmacenableInside)
					{
						...
						cmisAdapter.borrarDocumento(contenidoFirmaAlmacenado.getIdentificadorRepositorio(), true);
