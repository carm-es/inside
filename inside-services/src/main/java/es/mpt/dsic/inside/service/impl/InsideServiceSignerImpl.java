...
public class InsideServiceSignerImpl implements InsideServiceSigner {
	...
	public void configureSigner() {
		...
		if (!ACTIVO.contentEquals(firmaActivo)) {
			...
			logger.debug(String.format("Utilizando idaplicacion/password : %s/%s", properties.getProperty("firma.idaplicacion"),
	...
	/*
	 * @Override public String firmarCadena (String cadena, String
	 * algoritmoFirma, String formatoFirma, String modoFirma) throws
	 * InSideServiceSignerException {
	 * 
	 ...
	 * String firma = null;
	 * 
	 * DatosEntrada de = new DatosEntrada (); de.setContenido(cadena);
	 * de.setConvertirB64(true); de.setAlgoritmoFirma(algoritmoFirma);
	 * de.setFormatoFirma(formatoFirma); de.setModoFirma(modoFirma);
	 * 
	 * DatosSalida salida = null; try { salida =
	 * port.firmaContenido(applicationLogin, de); } catch (Exception e) { throw
	 * new InSideServiceSignerException (
	 * "Se ha producido un error en la llamada al servicio de firma remota " ,
	 * e); }
	 * 
	 * ContenidoSalida cont = salida.getDatosResultado();
	 * 
	 * if (salida.getEstado().equalsIgnoreCase(properties.getProperty(
	 * "firma.cadena.ok" ))) { logger.debug ("Cadena firmada correctamente");
	 * ResultadoFirma resultado = (ResultadoFirma) cont; firma =
	 * resultado.getContenidoFirma(); } else {
	 * es.mpt.dsic.firma.cliente.model.Error error =
	 * (es.mpt.dsic.firma.cliente.model.Error) cont; throw new
	 * InSideServiceSignerException ("No se ha firmado correctamente la cadena "
	...
	@Override
	public byte[] firmarFichero(byte[] bytesFichero, String algoritmoFirma, String formatoFirma, String modoFirma,
			WSCredentialInside info) throws InSideServiceSignerException {
		...
		try {
			...
			throw new InSideServiceSignerException("Se ha producido un error en la llamada al servicio de firma remota ", e);
		}
		...
		if (salida.getEstado().equalsIgnoreCase(properties.getProperty("firma.cadena.ok"))) {
			...
			throw new InSideServiceSignerException(
					"No se ha firmado correctamente el fichero: " + error.getMensaje() + " " + error.getCausa());
		}
		return firma;
	}
	...
	public byte[] firmarFicheroWhitPropertie(byte[] bytesFichero, String algoritmoFirma,
			String formatoFirma, String modoFirma, String nodeToSign,
			WSCredentialInside info) throws InSideServiceSignerException {
