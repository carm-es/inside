...
public class InsideMetadatoValidatorDataBase {
	...
	public void validateEqual(Object dato) throws InSideServiceValidationException,
		...
		try {
			if (dato instanceof List<?>) {
				for (Object datoAux : (List<?>) dato) {
					...
					if (value == null) {
						if (table.equalsIgnoreCase("UnidadORganica")) {
							throw new InSideServiceValidationException("El dato '" + datoAux +"' no encuentra correspondencia en las unidades organicas del DIR3'");
						} else {
							throw new InSideServiceValidationException("El dato '" + datoAux +"' no se encuentra en la tabla '"+ table + "' columna '" + column + "'");
						}
					}
				}
			} else {
				...
				if (value == null) {
					if (table.equalsIgnoreCase("UnidadORganica")) {
						throw new InSideServiceValidationException("El dato '" + dato +"' no encuentra correspondencia en las unidades organicas del DIR3'");
					} else {
						throw new InSideServiceValidationException("El dato '" + dato +"' no se encuentra en la tabla '"+ table + "' columna '" + column + "'");
					}
				}
			}
	...
	private Criteria getCriteriaEqual(Object dato, Session session) {
		Criteria retorno = session
				.createCriteria(getTable(table));
		if (dato instanceof List<?>) {
			for (Object datoAux : ((List<?>) dato)) {
