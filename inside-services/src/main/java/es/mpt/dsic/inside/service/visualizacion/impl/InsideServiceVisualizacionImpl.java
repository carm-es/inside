...
public class InsideServiceVisualizacionImpl implements InsideServiceVisualizacion {
	...
	public void configure() {
		...
		if (!activoCadena.contentEquals(visualizacionActivo)) {
			...
			try {
				...
				logger.error("No se puede crear la URL del servicio de visualizacion " + urlVisualizacion, me);
			}

			EeUtilServiceImplService ss = new EeUtilServiceImplService(
					url);
			...
			logger.debug(String.format("Utilizando para visualizacion idaplicacion/password : %s/%s",
					properties.getProperty("visualizacion.idaplicacion"), properties.getProperty("visualizacion.password")));

			filasNombreOrganismo = Arrays.asList(properties.getProperty("visualizacion.nombre.organismo").split(";"));
			...
			logger.debug(String.format("Utilizando estamparpie: %s", properties.getProperty("visualizacion.estamparpie")));

			textoPie = properties.getProperty("visualizacion.pie");

			logger.debug(String.format("Utilizando pie: %s", properties.getProperty("visualizacion.pie")));
	...
	@Override
	/**
	 * Devuelve un expediente Inside con la visualización del índice con las
	 * opciones indicadas en el parámetro.
	 */
	public ObjetoExpedienteInside expedienteConVisualizacionIndice(ObjetoExpedienteInside expediente,
		...
		if (!activo) {
			throw new InsideServiceVisualizacionException("El WS de VISUALIZACION no se encuentra activo");
		}
		...
		try {
			...
			throw new InsideServiceVisualizacionException("Error al obtener la visualización del índice", t);
		}
		...
		String nombreFormato = InsideUtils.getNombreFormatoByMime(salida.getDocumentoContenido().getMimeDocumento());
	...
	@Override
	/**
	 * Devuelve la visualización de un documento inside, con opciones de
	 * visualización.
	 */
	public ResultadoVisualizacionDocumento visualizacionDocumento(ObjetoDocumentoInside documento,
			OpcionesVisualizacionIndice opciones, byte[] bytesContenido) throws InsideServiceVisualizacionException {

		if (!activo) {
			throw new InsideServiceVisualizacionException("El WS de VISUALIZACION no se encuentra activo");
		}

		if (opciones.getLineasNombreOrganismo() == null || CollectionUtils.isEmpty(opciones.getLineasNombreOrganismo())) {
		...
		try {
			...
			String identificador = itemDocumento.getIdentificador() != null ? itemDocumento.getIdentificador() : "";
			throw new InsideServiceVisualizacionException("Error al obtener la visualización del documento " + identificador, t);
		}
	...
	@Override
	/**
	 * Devuelve la visualización de un documento inside, sin opciones, por lo
	 * que se usarán las de por defecto
	 */
	public ResultadoVisualizacionDocumento visualizacionDocumento(ObjetoDocumentoInside documento, byte[] bytesContenido)
			throws InsideServiceVisualizacionException {
	...
	/**
	 * Construcción de la estructura de datos jerárquica que le pasamos al WS de
	 * obtención de visualización del índice.
	 * 
	 * @param expediente
	 * @return
	 */
	private Item expedienteInsideToItemVisualizacion(ObjetoExpedienteInside expediente, OpcionesVisualizacionIndice opciones)
			throws InsideServiceVisualizacionException {
		Item padre = new Item();
		...
		fillItemWithIndice(padre, expediente.getIndice().getIndiceContenido().getElementosIndizados(), opciones);
	 ...
	 * 
	 * @param item
	 * @param elementosIndice
	 *            Elementos que serán hijos de "item"
	 * @param AtomicInteger
	 *            Entero que se va incrementando.
	 */
	private void fillItemWithIndice(Item item, List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> elementosIndice,
			OpcionesVisualizacionIndice opciones) throws InsideServiceVisualizacionException {
		...
		for (ObjetoExpedienteInsideIndiceContenidoElementoIndizado elementoHijo : elementosIndice) {
			...
			if (elementoHijo instanceof ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado) {
				ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado docIndizado = (ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado) elementoHijo;

				itemHijo.setNombre(getNombreDocumentoIndizado(docIndizado, opciones.isExterno()));
				itemHijo.setPropiedadesItem(getPropiedadesDocumentoIndizado(docIndizado, docIndizado.getOrdenDocumentoExpediente()));

			} else if (elementoHijo instanceof ObjetoExpedienteInsideIndiceContenido) {
				ObjetoExpedienteInsideIndiceContenido indiceContenidoIndizado = (ObjetoExpedienteInsideIndiceContenido) elementoHijo;
				itemHijo.setNombre("Expediente Electrónico: " + indiceContenidoIndizado.getIdentificadorExpedienteAsociado());
				...
				ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada carpetaIndizada = (ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) elementoHijo;
	...
	private void fillItemWithInfoDocumento(Item item, ObjetoDocumentoInside documento, byte[] bytesContenido)
			throws InsideServiceVisualizacionException {
		...
		if (documento.getContenido().getReferencia() == null) {
			...
			List<FirmaInside> firmasRef = InsideObjectsUtils
					.getFirmasReferencianNodo(documento.getContenido().getIdentificadorEnDocumento(), documento.getFirmas());
			...
			if (CollectionUtils.isNotEmpty(firmasRef)) {
				...
				try {
					...
					for (FirmaInside firma : firmasRef) {
						...
						Item itemFirma = getItemFirmaNoCSV(item, infoFirma.getTipoFirma().getTipoFirma(), infoFirmante);
					...
					throw new InsideServiceVisualizacionException(
							"Error al obtener información de la firma a partir del contenido del documento: " + identificador, e);
				...
				if (CollectionUtils.isNotEmpty(firmasNoCSV)) {
					...
					for (FirmaInside firma : firmasNoCSV) {
						if (firma.getContenidoFirma() instanceof ContenidoFirmaCertificadoAlmacenableInside) {
							ContenidoFirmaCertificadoAlmacenableInside contAlm = (ContenidoFirmaCertificadoAlmacenableInside) firma
									.getContenidoFirma();
							try {
			...
			ContenidoFirmaCertificadoAlmacenableInside contAlm = (ContenidoFirmaCertificadoAlmacenableInside) firmaImplicita
					.getContenidoFirma();

			// Llamamos a infofirma para obtener el contenido firmado y los
			// firmantes.
			try {
				infoFirma = llamadaInfoFirmaService(contAlm.getValorBinario(), true, true, true, bytesContenido);
				contenidoDocumento = infoFirma.getContenidoFirmado();
				mimeContenidoDocumento = infoFirma.getMimeContenidoFirmado();
	...
	@Override
	public InfoFirmaElectronica llamadaInfoFirmaService(byte[] firma, boolean obtenerDatosFirmados, boolean obtenerFirmantes,
			boolean obtenerTipoFirma, byte[] bytesContenido) throws InfoFirmaServiceException {
	...
	@Override
	public ResultadoValidacionInfo llamadaValidacionFirmaService(byte[] firma, String datosFirmadosBase64)
	...
	/**
	 * Construye la lista de propiedades de un expediente que aparecerán en la
	...
	private ListaPropiedades getPropiedadesExpediente(ObjetoExpedienteInside expediente) {
		...
		lista.getPropiedades().getPropiedades().add(createPropiedad(EXPEDIENTE, "identificador", expediente.getIdentificador()));
		lista.getPropiedades().getPropiedades().add(createPropiedad(EXPEDIENTE, "organos",
				InsideUtils.listaToString(expediente.getMetadatos().getOrgano().toArray(new String[] {}), "; ", false, true)));
		lista.getPropiedades().getPropiedades()
				.add(createPropiedad(EXPEDIENTE, "clasificacion", expediente.getMetadatos().getClasificacion()));
		lista.getPropiedades().getPropiedades().add(createPropiedad(EXPEDIENTE, "fechaAperturaExpediente",
				InsideUtils.calendarToStringISO8601(expediente.getMetadatos().getFechaAperturaExpediente())));

		lista.getPropiedades().getPropiedades()
				.add(createPropiedad(EXPEDIENTE, "estado", expediente.getMetadatos().getEstado().value()));

		lista.getPropiedades().getPropiedades().add(createPropiedad(EXPEDIENTE, "interesados", InsideUtils
				.listaToString(expediente.getMetadatos().getInteresado().toArray(new String[] {}), "; ", false, true)));

		// listado de metadatos adicionales
		if (CollectionUtils.isNotEmpty(expediente.getMetadatos().getMetadatosAdicionales())) {
			for (ObjetoInsideMetadatoAdicional data : expediente.getMetadatos().getMetadatosAdicionales()) {
				lista.getPropiedades().getPropiedades()
						.add(createPropiedad(EXP_METADATO_ADICIONAL, data.getNombre(), data.getValor().toString()));
			}
		}
	...
	private ListaPropiedades getPropiedadesDocumento(ObjetoDocumentoInside documento) {
		...
		lista.getPropiedades().getPropiedades().add(createPropiedad(DOCUMENTO, "identificador", documento.getIdentificador()));
		lista.getPropiedades().getPropiedades().add(createPropiedad(DOCUMENTO, "organo",
				InsideUtils.listaToString(documento.getMetadatos().getOrgano().toArray(new String[] {}), "; ", false, true)));
		...
		lista.getPropiedades().getPropiedades().add(createPropiedad(DOCUMENTO, "origenCiudadanoAdministracion",
		...
		lista.getPropiedades().getPropiedades()
				.add(createPropiedad(DOCUMENTO, "tipoDocumental", documento.getMetadatos().getTipoDocumental().value()));

		// listado de metadatos adicionales
		if (CollectionUtils.isNotEmpty(documento.getMetadatos().getMetadatosAdicionales())) {
			for (ObjetoInsideMetadatoAdicional data : documento.getMetadatos().getMetadatosAdicionales()) {
				lista.getPropiedades().getPropiedades()
						.add(createPropiedad(DOC_METADATO_ADICIONAL, data.getNombre(), data.getValor().toString()));
			}
		}
	...
	private Item getItemFirmaCSV(Item padre, FirmaInside firmaInside) {
		...
		ContenidoFirmaCSVInside contenidoFirma = (ContenidoFirmaCSVInside) firmaInside.getContenidoFirma();
		lista.getPropiedades().getPropiedades().add(createPropiedad(FIRMA_CSV, "valorCSV", contenidoFirma.getValorCSV()));
		lista.getPropiedades().getPropiedades()
				.add(createPropiedad(FIRMA_CSV, "regulacionCSV", contenidoFirma.getRegulacionGeneracionCSV()));
	...
	private Item getItemFirmaNoCSV(Item padre, String tipoFirma, InfoFirmante infoFirmante) {
		...
		lista.getPropiedades().getPropiedades().add(createPropiedad(FIRMA_NO_CSV, "descripcion", infoFirmante.getDescripcion()));
		lista.getPropiedades().getPropiedades().add(createPropiedad(FIRMA_NO_CSV, "fecha", infoFirmante.getFecha()));
	...
	/**
	 * Construye la lista de propiedades de un documento que aparecerán en la
	 * visualización del índice.
	 * 
	...
	private ListaPropiedades getPropiedadesDocumentoIndizado(
		...
		lista.getPropiedades().getPropiedades().add(createPropiedad(DOCUMENTO_INDIZADO, "orden", Integer.toString(orden)));
		lista.getPropiedades().getPropiedades()
	...
	/**
	 * Obtiene el nombre que se mostrará de un documento Indizado. Si el
	 * expediente es externo a InSide (no lo tenemos almacenado), se meterá el
	 * identificador del documento. Si el expediente es interno (lo tenemos
	 * almacenado): si el documento tiene el Metadato Adicional
	 * "DescripcionDocumento" se enviará ese, en caso contrario se enviará el
	 * identificador del documento indizado.
	 */
	private String getNombreDocumentoIndizado(ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado documentoIndizado,
			boolean externo) throws InsideServiceVisualizacionException {
		...
		if (externo) {
			...
			try {
				...
				throw new InsideServiceVisualizacionException("Error al obtener la descripcion del documento con identificador "
						+ documentoIndizado.getIdentificadorDocumento(), e);
			} catch (InsideServiceStoreException e) {
				throw new InsideServiceVisualizacionException("Error al obtener la descripcion del documento con identificador "
						+ documentoIndizado.getIdentificadorDocumento(), e);
			}

			nombreDocumento = InsideObjectsUtils.getMetadatoAdicionalNotNull(metadatosDocumento, "DescripcionDocumento");
			nombreDocumento = nombreDocumento + " [" + documentoIndizado.getIdentificadorDocumento() + "]";
		}
	...
	private Propiedad createPropiedad(String tipo, String key, String value) {
		...
		if (StringUtils.equalsIgnoreCase(tipo, DOC_METADATO_ADICIONAL)
				&& StringUtils.equalsIgnoreCase(getEnviarVisualizacion(DOCUMENTO, ADD_ALL_METADATOS), VALUE_SI)) {
			addALLMetadatos = true;
		}
		if (StringUtils.equalsIgnoreCase(tipo, EXP_METADATO_ADICIONAL)
				&& StringUtils.equalsIgnoreCase(getEnviarVisualizacion(EXPEDIENTE, ADD_ALL_METADATOS), VALUE_SI)) {
			addALLMetadatos = true;
		}
		if (addALLMetadatos) {
			p = new Propiedad();
	...
	/**
	 * Crea las opciones de visualización del WS a partir de las opciones de
	 * visualizacion de InSide.
	 * 
	 * @return
	 */
	private OpcionesVisualizacion getOpcionesVisualizacionWS(OpcionesVisualizacionIndice opciones, String modelo)
			throws InsideServiceVisualizacionException {
		...
		if (opciones.isEstamparNombreOrganismo()) {
			if (opciones.getLineasNombreOrganismo() != null && CollectionUtils.isNotEmpty(opciones.getLineasNombreOrganismo())) {
	...
	@Override
	public Item documentoInsideToItemVisualizacion(ObjetoDocumentoInside documento, byte[] bytesContenido)
			throws InsideServiceVisualizacionException {
		Item padre = new Item();
	...
	@Override
	public byte[] visualizarContenidoOriginal(ObjetoDocumentoInside documento) throws InsideServiceVisualizacionException {
	...
	@Override
	public TipoResultadoVisualizacionDocumentoInside visualizacionConPlantilla(String plantilla, TipoDocumentoVisualizacionInside tipoDocumentoVisualizacionInside)
			throws InsideServiceVisualizacionException {
		try {
			TipoResultadoVisualizacionDocumentoInside retorno = new TipoResultadoVisualizacionDocumentoInside();
			
			logger.debug("Convirtiendo a documento Inside");
			ObjetoDocumentoInside documentoInside = InsideConverterDocumento.documentoVisualizacionToInside(tipoDocumentoVisualizacionInside);
			
			DocumentoEniConMAdicionales docEniAdicionales = InsideConverterDocumento.convertToEeutil(documentoInside);
			
			SalidaVisualizacion salidaVisualizacion = port.visualizarDocumentoConPlantilla(applicationLogin, docEniAdicionales, plantilla);
