...
public class InsideServiceStoreHibernateConverterFirma {
	...
	protected static final Log logger = LogFactory.getLog(InsideServiceStoreHibernateConverterFirma.class);
	private static final String EXP_REF = "EXP_INDICE";
	
	public static es.mpt.dsic.inside.store.hibernate.entity.FirmaInside toEntity(
			FirmaInside firma) throws InsideServiceStoreHibernateConverterFirmaException 
	{
		...
		if(contenidoFirma == null){
			throw new InsideServiceStoreHibernateConverterFirmaException("El contenido de la firma está vacío");
		}
		
		if(contenidoFirma instanceof ContenidoFirmaCertificadoAlmacenableInside){
			ContenidoFirmaCertificadoAlmacenableInside contenidofirmaAlmacenable = (ContenidoFirmaCertificadoAlmacenableInside) contenidoFirma;
			...
			ContenidoFirmaCertificadoReferenciaInside contenidofirmaReferencia = (ContenidoFirmaCertificadoReferenciaInside) contenidoFirma;
			...
			if(firma.getTipoFirma() != FirmaInsideTipoFirmaEnum.TF_01){
				throw new InsideServiceStoreHibernateConverterFirmaException("El contenido de la firma es de tipo ContenidoFirmaCSVInside pero el tipo no es TF_01");
			}
			...
			throw new InsideServiceStoreHibernateConverterFirmaException("El tipo de firma es TF_01 pero el contenido no es de tipo ContenidoFirmaCSVInside");
		}
	...
	public static FirmaInside toInside(es.mpt.dsic.inside.store.hibernate.entity.FirmaInside entity)
	{
		...
		if(firma.getTipoFirma() == FirmaInsideTipoFirmaEnum.TF_01){
			...
			if(isDocReference(entity.getReferencia())){
				ContenidoFirmaCertificadoReferenciaInside contenidofirmaReferencia = new ContenidoFirmaCertificadoReferenciaInside();
				...
				if (entity.getContenido() != null) {
					try {
						contenidofirmaAlmacenable.setValorBinario(Base64.decode(new String(entity.getContenido(), "UTF-8")));
