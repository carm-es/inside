...
public class InsideServiceStoreHibernateConverterSolicitudAccesoExpediente {
	...
	public static SolicitudAccesoExpediente toEntity(ObjetoSolicitudAccesoExpediente objetoSolicitudAccesoExpediente) throws InsideServiceStoreException {
		...
		entity.setDir3ExpedienteSolicitado(objetoSolicitudAccesoExpediente.getDir3ExpedienteSolicitado());
	...
	public static ObjetoSolicitudAccesoExpediente toInside(SolicitudAccesoExpediente entity) throws InsideServiceStoreException, JAXBException {
		
		ObjetoSolicitudAccesoExpediente objetoSolicitudAccesoExpediente = new ObjetoSolicitudAccesoExpediente();
		...
		try {
			if (entity.getMetadatos() != null) {
				...
				JAXBContext jc = JAXBContext.newInstance(es.mpt.dsic.inside.xml.inside.TipoMetadatosAdicionales.class);
				Unmarshaller unMarshaller = jc.createUnmarshaller();
				Reader reader = new StringReader(objetoSolicitudAccesoExpediente.getMetadatos());
				XMLInputFactory factory = XMLInputFactory.newInstance(); // Or newFactory()
				XMLStreamReader xmlReader;
				xmlReader = factory.createXMLStreamReader(reader);
			    JAXBElement<TipoMetadatosAdicionales> root2 = unMarshaller.unmarshal(xmlReader, TipoMetadatosAdicionales.class);
			    ...
			    if (!car.getMetadatoAdicional().isEmpty()) {
					for (MetadatoAdicional iterable_element : car.getMetadatoAdicional()) {
						...
						pintarMetadatos += "<p><b>" + iterable_element.getNombre() + "</b> : " + e.getTextContent() + "</p>";
						pintarMetadatosTitle += "" + iterable_element.getNombre() + " : " + e.getTextContent() + "\n";
		...
		objetoSolicitudAccesoExpediente.setDir3ExpedienteSolicitado(entity.getDir3ExpedienteSolicitado());
