...
public class InsideServiceStoreHibernateImpl<I extends ObjetoInside<?>, E extends Object> implements InsideServiceStore {
	...
	public <T extends ObjetoInside<?>> T storeObject(T object, String usuario, boolean online)
		...
		try {
			...
			logger.debug(
					"Almacenando objeto " + object.getClass() + " con identificador proporcionado " + object.getIdentificador(),
					e);
		}
	...
	private <T extends ObjetoInside<?>> Class<?> getEntityClass(Class<T> tipo) throws InsideServiceStoreException {
		if (tipo == ObjetoExpedienteInside.class) {
			...
			throw new InsideServiceStoreException("No se corresponden ningun Entity de BBDD con el tipo " + tipo);
		}
	}

	@Override
	public <T extends ObjetoInside<?>> T getObjectVersion(Class<T> tipo, String Identifier, int version)
			throws InsideServiceStoreException, InsideStoreObjectNotFoundException {
		SortedMap<ObjetoInsideVersion, T> versionesObjeto = getVersionesObjeto(tipo, Identifier, version);
	...
	@Override
	public <T extends ObjetoInside<K>, K extends ObjetoInsideMetadatos> K getObjectMetadatos(Class<T> tipo, String Identifier)
			throws InsideServiceStoreException, InsideStoreObjectNotFoundException {
	...
	public <T extends ObjetoInside<?>> T updateObject(T object, String usuario, boolean online)
		...
		if (object instanceof ObjetoInsideVersionable) {
			...
			((ObjetoInsideVersionable) object)
					.setVersion(ObjetoInsideVersion.createNextVersion(((ObjetoInsideVersionable) stored).getVersion()));
			logger.debug("Subida la versión para objeto de tipo " + object.getClass() + " con identificador "
					+ object.getIdentificador());
		} else if (object instanceof ObjetoDocumentoInside) {
			ObjetoInside<?> stored = this.getObject(object.getClass(), object.getIdentificador());
			((ObjetoDocumentoInside) object).setVersion(
					new ObjetoInsideVersion(((ObjetoDocumentoInside) stored).getVersion().getVersion(), new GregorianCalendar()));
		}
		Object objectUnidad = asociarObjetoUnidad(object, usuario, online);
		persister.persistExpedienteOrDocumento(object, objectUnidad);
	...
	@Override
	public <T extends ObjetoInside<?>> List<ObjetoInsideVersion> getObjectVersions(Class<T> tipo, String Identifier)
			throws InsideServiceStoreException, InsideStoreObjectNotFoundException {
		SortedMap<ObjetoInsideVersion, T> versionesObjeto = getVersionesObjeto(tipo, Identifier, null);
	...
	@Override
	public <T extends ObjetoInside<?>> SortedMap<ObjetoInsideVersion, T> getVersionesObjeto(Class<T> tipo, String Identifier,
			Integer idVersion) throws InsideServiceStoreException, InsideStoreObjectNotFoundException {
		Session session = sessionFactory.openSession();

		try {
			SortedMap<ObjetoInsideVersion, T> versionesObjeto = new TreeMap<ObjetoInsideVersion, T>(
					new ObjetoInsideVersionComparator());
	...
	public <T extends ObjetoInside<K>, K extends ObjetoInsideMetadatos> ResultadoBusquedaInside<K> queryObjects(
		...
		try {
			...
			InsideServiceHibernateConverterBusqueda converterBusqueda = new InsideServiceHibernateConverterBusqueda(
					objectDefinitions);
			...
			try {
				...
				throw new InsideServiceStoreQueryNotValidException(
						"Alguno de los datos proporcionados no tenía el tipo de datos adecuado para su columna", e);
			}
			...
			List<ItemResultadoBusquedaInside<K>> resultados = new ArrayList<ItemResultadoBusquedaInside<K>>();
			...
			// resultados.
			List<?> subLista = (limite == -1) ? objetos : InsideUtils.getSublista(objetos, limite, pagina);
	...
	/**
	 * Devuelve el objeto Inside asociado al nombre de una aplicación. Si la
	 * consulta devuelve más de un resultado dará un error Si la consulta no
	 * devuelve nada devolverá NULL.
	 */
	@Override
	public ObjetoInsideAplicacion getAplicacion(String nombreAplicacion) throws InsideServiceStoreException {
		...
		try {
			...
			throw new InsideServiceStoreException("Excepción obteniendo la aplicacion con nombre " + nombreAplicacion, e);
		} finally {
			session.close();
		}
	...
	public InsideWsAplicacion getAplicacionEntity(String nombreAplicacion, Session session) throws InsideServiceStoreException {
		...
		if (CollectionUtils.isNotEmpty(aplicaciones)) {
			...
			throw new InsideServiceStoreException("No se ha obtenido resultado para la aplicación " + nombreAplicacion);
		}
		return retorno;
	}
	...
	public ObjetoInsideUsuario getUsuario(String nifUsuario) throws InsideServiceStoreException {
		...
		try {
			...
			throw new InsideServiceStoreException("Excepción obteniendo el usuario con nif " + nifUsuario, e);
		} finally {
			session.close();
		}
	...
	public UsuarioInside getUsuarioEntity(String nifUsuario, Session session) throws InsideServiceStoreException {
		...
		if (usuarios != null) {
			if (usuarios.isEmpty())
				throw new InsideServiceStoreException("No se ha obtenido resultado para el usuario: " + nifUsuario);
			if (usuarios.size() > 1)
				throw new InsideServiceStoreException("Se ha obtenido más de un resultado para el usuario: " + nifUsuario);
		} else {
			throw new InsideServiceStoreException("No se ha obtenido resultado para el usuario: " + nifUsuario);
		}
	...
	public void setConverterExpedienteToken(InsideServiceStoreHibernateConverterExpedienteToken converterExpedienteToken) {
	...
	public void setConverterExpedienteInsideRespuestaEnvioJusticia(
		...
		this.converterExpedienteInsideRespuestaEnvioJusticia = converterExpedienteInsideRespuestaEnvioJusticia;
	}
	...
	public void setConverterExpedienteNoInsideRespuestaEnvioJusticia(
		...
		this.converterExpedienteNoInsideRespuestaEnvioJusticia = converterExpedienteNoInsideRespuestaEnvioJusticia;
	}

	@Override
	public <T extends ObjetoInside<?>> List<ObjetoExpedienteInside> getExpedientesAsociados(Class<T> objeto, String identificador)
			throws InsideServiceStoreException, InsideStoreObjectNotFoundException {
		Session session = sessionFactory.openSession();
		...
		try {
			if (objeto.isAssignableFrom(ObjetoExpedienteInside.class)) {
				retorno = new InsideServiceStoreHibernateConverterExpediente().getExpedientesAsociados(identificador, session);
			}
			if (objeto.isAssignableFrom(ObjetoDocumentoInside.class)) {
				retorno = new InsideServiceStoreHibernateConverterDocumento().getExpedientesAsociados(identificador, session);
			}
		} catch (InsideServiceStoreException e) {
			throw e;
		} finally {
	...
	@Override
	public Object asociarObjetoUnidad(ObjetoInside<?> obj, String usuario, boolean online) throws InsideServiceStoreException {
		...
		try {
			...
			if (online) {
				...
				UnidadWsAplicacion unidadAplicacion = getAplicacionEntity(usuario, session).getUnidadActiva();
	...
	/**
	 * Relacionamos un objeto de tipo documento/expediente con una unidad
	 * orgánica
	 * 
	 * @param obj
	 * @param usuario
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Object calculateObjetoUnidad(Object obj, Integer unidad, Integer procedimiento, Session session)
			throws InsideServiceStoreException {
		Object dato = null;
	...
	public Object generarObjetoDocumentoUnidad(Object obj, Integer unidad, Integer procedimiento, Session session) {
		Object dato;
		Calendar time = GregorianCalendar.getInstance();
		Criteria documentoUnidad = session.createCriteria(DocumentoUnidad.class);
		documentoUnidad.add(Restrictions.eq(IDENTIFICADOR, ((ObjetoDocumentoInside) obj).getIdentificador()));
	...
	public Object generarObjetoExpedienteUnidad(Object obj, Integer unidad, Integer procedimiento, Session session) {
		Object dato;
		Calendar time = GregorianCalendar.getInstance();
		Criteria expedienteUnidad = session.createCriteria(ExpedienteUnidad.class);
		expedienteUnidad.add(Restrictions.eq(IDENTIFICADOR, ((ObjetoExpedienteInside) obj).getIdentificador()));
		...
		if (expedienteUnidadEntity != null) {
			...
			expedienteUnidadEntity.setEstadoExpediente(((ObjetoExpedienteInside) obj).getMetadatos().getEstado().value());
			...
			((ExpedienteUnidad) dato).setEstadoExpediente(((ObjetoExpedienteInside) obj).getMetadatos().getEstado().value());
		}
		return dato;
	}

	/**
	 * Desactivar el documento de la unidad es poner la fecha de versión a nulo.
	 * Eso nos indica que el documento en si está dado de baja.
	 */
	@Override
	...
	@Override
	public boolean validarObjetoUnidad(ObjetoInside<?> obj, String identificadorUsuario, boolean online)
			throws InsideServiceStoreException {
	...
	public boolean validarDocumentoUnidad(ObjetoInside<?> obj, Session session, List<ObjetoInsideUnidadUsuario> unidad) {
		boolean validar = false;
		Criteria documentoUnidad = session.createCriteria(DocumentoUnidad.class);
		...
		if (docuemntoUnidadEntity != null) {
			for (DocumentoUnidad docUnidad : docuemntoUnidadEntity) {
				validar = validarUnidadesCompatibles(unidad, docUnidad.getIdUnidad(), docUnidad.getIdProcedimiento());
				if (validar)
					break;
			}
		}
	...
	public boolean validarExpedienteUnidad(ObjetoInside<?> obj, Session session, List<ObjetoInsideUnidadUsuario> unidad) {
		boolean validar = false;
		Criteria expedienteUnidad = session.createCriteria(ExpedienteUnidad.class);
	...
	private List<ObjetoInsideUnidadUsuario> getUnidadesOrganicasAsociadas(String identificadorUsuario, boolean online,
			boolean soloUnidadActiva, Session session) throws InsideServiceStoreException {
		...
		if (online) {
			...
			if (soloUnidadActiva) {
				lista.add(InsideServiceStoreHibernateConverterUnidadUsuario.toInside(usu.getUnidadActiva()));
			} else {
	 ...
	 * @return
	 */
	private boolean validarUnidadesCompatibles(List<ObjetoInsideUnidadUsuario> unidades, Integer idUnidad,
		...
		for (ObjetoInsideUnidadUsuario unidad : unidades) {
			...
			if (unidad.getIdUnidad().intValue() == idUnidad.intValue()) {
				if (unidad.getIdProcedimiento() == null) {
					validar = true;
				} else if (idProcedimiento != null && unidad.getIdProcedimiento().intValue() == idProcedimiento.intValue()) {
					validar = true;
				}
			}
		}
	...
	@Override
	public List<ObjetoInsideExpedienteUnidad> getExpedientesUnidad(String nif, boolean soloUnidadActiva)
			throws InsideServiceStoreException {
	...
	public List<ExpedienteUnidad> getListExpedientesUnidad(String nif, boolean soloUnidadActiva, Session session)
			throws InsideServiceStoreException {
		List<ExpedienteUnidad> datos = new ArrayList<ExpedienteUnidad>();
		List<ObjetoInsideUnidadUsuario> unidades = getUnidadesOrganicasAsociadas(nif, true, soloUnidadActiva, session);
	...
	@Override
	public List<ObjetoInsideExpedienteUnidad> getExpedientesUnidadAutocompleter(String nif, boolean soloUnidadActiva)
			throws InsideServiceStoreException {
		List<ObjetoInsideExpedienteUnidad> retorno = null;
		Session session = sessionFactory.openSession();
	...
	private List<ObjetoInsideExpedienteUnidad> obtenerExpedientesUnidad(List<ExpedienteUnidad> datos)
		...
		for (ExpedienteUnidad dato : datos) {
			...
			expedienteUnidad
					.setEstadoExpediente(ObjetoExpedienteInsideMetadatosEnumeracionEstados.fromValue(dato.getEstadoExpediente()));
	...
	private List<ObjetoInsideExpedienteUnidad> obtenerExpedientesUnidadInformandoRemisionMJU(List<ExpedienteUnidad> datos)
			throws InsideServiceStoreException {
		List<ObjetoInsideExpedienteUnidad> retorno = new ArrayList<ObjetoInsideExpedienteUnidad>();
		for (ExpedienteUnidad dato : datos) {
			ObjetoInsideExpedienteUnidad expedienteUnidad = new ObjetoInsideExpedienteUnidad();
	...
	private String estaRemitidoMJUAlgunaVersion(String identificadorExp) throws InsideServiceStoreException {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<ObjetoInsideDocumentoUnidad> getDocumentosUnidad(String nif, boolean soloUnidadActiva) throws InsideServiceStoreException {
	...
	public List<ObjetoInsideDocumentoUnidad> obtenerDocumento(List<DocumentoUnidad> datos) throws InsideServiceStoreException {
	...
	@Override
	public ObjetoExpedienteToken getTokenByUsuarioExpediente(ObjetoExpedienteToken data) throws InsideServiceStoreException {
	...
	@Override
	public void saveAuditoriaToken(ObjetoAuditoriaToken objectAuditoriaToken) throws InsideServiceStoreException {
	...
	@Override
	public void saveComunicacionTokenExpediente(ObjetoComunicacionTokenExpediente objetoComunicacionTokenExpediente)
			throws InsideServiceStoreException {
		logger.debug("Inicio saveComunicacionTokenExpediente");
		Session session = sessionFactory.openSession();
		try {
			persister.saveComunicacionTokenExpediente(
					InsideServiceStoreHibernateConverterComunicacionTokenExpediente.toEntity(objetoComunicacionTokenExpediente),
					session);
		} finally {
			session.close();
		}
	...
	public void saveAuditoriaAccesoDocumento(ObjetoAuditoriaAccesoDocumento objetoAuditoriaAccesoDocumento)
		...
		Session session = sessionFactory.openSession();
		try {
			persister.saveAuditoriaAccesoDocumento(
					InsideServiceStoreHibernateConverterAuditoriaAccesoDocumento.toEntity(objetoAuditoriaAccesoDocumento),
					session);
		} finally {
			session.close();
		}
	...
	@Override
	public void saveSolicitudAccesoExpediente(ObjetoSolicitudAccesoExpediente objetoSolicitudAccesoExpediente)
			throws InsideServiceStoreException {
		logger.debug("Inicio saveSolicitudAccesoExpediente");
		Session session = sessionFactory.openSession();
		try {
			persister.saveSolicitudAccesoExpediente(
					InsideServiceStoreHibernateConverterSolicitudAccesoExpediente.toEntity(objetoSolicitudAccesoExpediente),
					session);
		} finally {
			session.close();
		}
	...
	/**
	 * Se recuperan las peticiones que: - No han sido correctas o tienen el
	 * campo correcto a nulo, el job no las ha tratado todavía. - La fecha del
	 * próximo intento es mayor al momento de lanzarse el job. - Los intentos no
	 * han pasado del máximo establecido. Un máximo de maximoResultados. Primero
	 * las más antiguas.
	 */
	@Override
	public List<ObjetoComunicacionTokenExpediente> getComunicacionesTokenExpedienteActivas(int maximoResultados,
			int numeroMaximoIntentos) throws InsideServiceStoreException {
		logger.debug("Inicio getComunicacionesTokenExpedienteActivas");
		Session session = sessionFactory.openSession();
		List<ObjetoComunicacionTokenExpediente> retorno = new ArrayList<ObjetoComunicacionTokenExpediente>();
		try {
			...
			crit.add(Restrictions.or(Restrictions.lt("NIntentos", numeroMaximoIntentos), Restrictions.isNull("NIntentos")));
	 ...
	 * @throws InsideServiceStoreException 
	 */
	public List<ObjetoComunicacionTokenExpediente> getComunicacionesTokenExpedienteUnidadOrganicaUsuario(ObjetoInsideUsuario objetoInsideUsuario) throws InsideServiceStoreException {
		logger.debug("Inicio getComunicacionesTokenExpedienteUnidadOrganicaUsuario");
		List<ObjetoComunicacionTokenExpediente> retorno = getComunicacionesTokenExpedienteUnidadOrganicaDestino(objetoInsideUsuario.getUnidadOrganicaActiva().substring(0, objetoInsideUsuario.getUnidadOrganicaActiva().indexOf(" -")));
	 ...
	 * @throws InsideServiceStoreException 
	 */
	public List<ObjetoComunicacionTokenExpediente> getComunicacionesTokenExpedienteUnidadOrganicaDestino(String dir3Destino) throws InsideServiceStoreException {
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(datos)) {
				for (ComunicacionTokenExpediente comunicacionTokenExpediente : datos) {
					retorno.add(
							InsideServiceStoreHibernateConverterComunicacionTokenExpediente.toInside(comunicacionTokenExpediente));
				}
			}
		} finally {
			session.close();
	...
	/**
	 * Se recuperan las peticiones que tienen el resultado nulo. Un máximo de
	 * maximoResultados. Primero las más antiguas. Si el campo resultado
	 * contiene información quiere decir que se envió (o intentó) a su
	 * destinatario.
	 * 
	 ...
	 */
	@Override
	public List<ObjetoSolicitudAccesoExpediente> getSolicitudesAccesoExpediente(ObjetoInsideUsuario objetoInsideUsuario)
		...
		Session session = sessionFactory.openSession();
		List<ObjetoSolicitudAccesoExpediente> retorno = new ArrayList<ObjetoSolicitudAccesoExpediente>();
		List<ObjetoInsideExpedienteUnidad> expedientesUnidadUsuario = getExpedientesUnidad(objetoInsideUsuario.getNif(), true);
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(datos)) {
				for (SolicitudAccesoExpediente solicitudAccesoExpediente : datos) {
					retorno.add(
							InsideServiceStoreHibernateConverterSolicitudAccesoExpediente.toInside(solicitudAccesoExpediente));
				}
			}
		} finally {
			session.close();
	...
	public List<ObjetoAuditoriaAcceso> getAuditoriaAccesoDocumento(ObjetoInsideUsuario objetoInsideUsuario)
		...
		List<ObjetoInsideExpedienteUnidad> expedientesUnidadUsuario = getExpedientesUnidad(objetoInsideUsuario.getNif(), false);
		List<ObjetoInsideDocumentoUnidad> documentosUnidadUsuario = getDocumentosUnidad(objetoInsideUsuario.getNif(), false);
...
//			crit.add(Restrictions.in("identificador", arrayDocumentos));
			...
			if (CollectionUtils.isNotEmpty(datos)) {
				for (VAuditoriaAccesoNube vAuditoriaAccesoNube : datos) {
					retorno.add(InsideServiceStoreHibernateConverterVAuditoriaAccesoNube.toInside(vAuditoriaAccesoNube));
				}
			}
	...
	@Override
	public ObjetoSolicitudAccesoExpediente getSolicitudAccesoExpedientePorIdPeticion(String idPeticion)
			throws JAXBException {
	...
	@Override
	public ObjetoExpedienteToken getTokenByData(ObjetoExpedienteToken usuarioToken) throws InsideServiceStoreException {
		ObjetoExpedienteToken retorno = null;
		Session session = sessionFactory.openSession();
		try {
	...
	public ResultadoBusquedaUsuario getUsuarios(ObjetoFilterPageRequest data)
		...
		try {
				...
				while (token.hasMoreTokens()) {
					String filter = token.nextToken();
					 if (filter.equalsIgnoreCase("true")
							...
							disjunction.add(Restrictions.eq("usuario.activo", 
									Boolean.parseBoolean(filter.toUpperCase())));
					 }
			...
			List<?> subLista = (data.getPageSize() == -1) ? objetos : InsideUtils.getSublista(objetos, data.getPageSize(), data.getPageNumber());
			...
			for (Object entity : subLista) {
				...
				if (usuarioInside.contieneUnidadActiva()) {
					...
					numeroProcedimiento = unidadUsuario.getIdProcedimiento() != null
							? unidadUsuario.getProcedimiento().getNumProcedimiento()	 : null;
							
				}
				
				ObjetoInsideUsuario res = 
						InsideServiceStoreHibernateConverterUsuario.toInsideUnidad(
								usuarioInside, numeroProcedimiento);
				resultados.add(res);
				//}
	...
	@Override
	public ObjetoInsideUsuario altaUsuario(ObjetoInsideUsuario data) throws InsideServiceStoreException {
		...
		try {
			...
			if (data.getObjNumeroProcedimiento()!=null && data.getObjNumeroProcedimiento().getId()>0) {
				numeroProcedimiento = getNumeroProcesoEntity(data.getObjNumeroProcedimiento().getId(), session);
				idProcedimiento = numeroProcedimiento.getId();
			}
	...
	@Override
	public ObjetoInsideUsuario bajaUsuario(ObjetoInsideUsuario data) throws InsideServiceStoreException {
	...
	private void deleteUnidadUsuario(UsuarioInside usuario, Integer idUnidad, Integer idProcedimiento, Session session)
			throws InsideServiceStoreException {
	...
	@Override
	public ObjetoInsideAplicacion altaAplicacion(ObjetoInsideAplicacion data) throws InsideServiceStoreException {
	...
	@Override
	public ObjetoInsideAplicacion bajaAplicacion(ObjetoInsideAplicacion data) throws InsideServiceStoreException {
	...
	private void deleteUnidadWSAplicacion(Integer idAplicacion, Session session) throws InsideServiceStoreException {
	...
	@Override
	public ObjetoInsideUnidad altaUnidadAplicacion(String idUnidad, String numeroProcedimiento, String idAplicacion)
			throws InsideServiceStoreException {
		logger.debug("Inicio altaUnidadAplicacion");
		Session session = sessionFactory.openSession();
		try {
			InsideWsAplicacion aplicacion = cambiarUnidadAplicacion(idUnidad, numeroProcedimiento, idAplicacion, session);
	...
	@Override
	public void deleteUnidadAplicacion(String codigoUnidad, String numeroProcedimiento, String idAplicacion)
			throws InsideServiceStoreException {
		logger.debug("Inicio altaUnidadAplicacion");
		Session session = sessionFactory.openSession();
		try {
			...
			if (StringUtils.isNotEmpty(codigoUnidad)) {
				...
				if (StringUtils.isNotEmpty(numeroProcedimiento)) {
					NumeroProcedimiento numProcedimiento = getNumeroProcesoEntity(numeroProcedimiento, session);
	...
	private void deleteUnidadAplicacion(Integer idAplicacion, Integer idUnidad, Integer idProcedimiento, Session session)
			throws InsideServiceStoreException {
	...
	@Override
	public ObjetoInsideUnidad altaUnidadUsuario(String idUnidad, String numeroProcedimiento, String idUsuario)
			throws InsideServiceStoreException {
		logger.debug("Inicio asociarUnidadUsuario");
		Session session = sessionFactory.openSession();
		try {
			UsuarioInside usuario = cambiarUnidadUsuario(idUnidad, numeroProcedimiento, idUsuario, session);
			persister.saveUser(usuario, session);
			UnidadOrganica unidad = getUnidadEntity(usuario.getUnidadActiva().getUnidad().getId(), session);
			return InsideServiceStoreHibernateConverterUnidad.toInside(unidad, numeroProcedimiento, true);
		} finally {
			session.close();
		}
	...
	public UsuarioInside cambiarUnidadUsuario(String idUnidad, String numeroProcedimiento, String idUsuario, Session session)
			throws InsideServiceStoreException {
	...
	public InsideWsAplicacion cambiarUnidadAplicacion(String idUnidad, String numeroProcedimiento, String idAplicacion,
		...
		UnidadWsAplicacion unidadAplicacion = aplicacion.getUnidad(unidad.getId(), idProcedimiento, aplicacion.getId());
	...
	public UnidadOrganica getUnidadEntity(Object idUnidad, Session session) throws InsideServiceStoreException {
		UnidadOrganica unidad;
		...
		if (idUnidad instanceof String)
			...
			throw new InsideServiceStoreException("Proceso incompleto. No se pudo obtener unidad orgánica no existe en BBDD");
		unidad = (UnidadOrganica) critUnidad.uniqueResult();

		if (unidad == null)
			throw new InsideServiceStoreException("Proceso incompleto. La unidad orgánica no existe en BBDD");

		return unidad;
	}
	
	public InsideRol getInsideRolEntity(Integer idRol, Session session) throws InsideServiceStoreException {
		InsideRol rolEntity;
		// Recuperamos la unidad orgánica
	...
	public NumeroProcedimiento getNumeroProcesoEntity(Object numeroProc, Session session) throws InsideServiceStoreException {
		...
		if (numeroProcedimiento == null)
			throw new InsideServiceStoreException("Proceso incompleto. El número de procedimiento no existe en BBDD");
	...
	@Override
	public List<ObjetoInsideUnidad> getUnidadesAplicacion(String idAplicacion, boolean todas) throws InsideServiceStoreException {
	...
	private List<ObjetoInsideUnidad> getTodasUnidadesAplicacion(Session session) throws InsideServiceStoreException {
		...
		for (ObjetoInsideAplicacion aplicacion : aplicaciones) {
			InsideWsAplicacion aplicacionEntity = getAplicacionEntity(aplicacion.getAplicacion(), session);
	...
	private List<ObjetoInsideUnidad> getUnidadesAplicacion(String idAplicacion, boolean todas, Session session)
			throws InsideServiceStoreException {
		List<ObjetoInsideUnidad> retorno = new ArrayList<ObjetoInsideUnidad>();
	...
	public void getUnidadProcedimiento(Session session, List<ObjetoInsideUnidad> retorno, UnidadWsAplicacion unidad)
			throws InsideServiceStoreException {
		if (unidad != null) {
			String numeroProcedimiento = unidad.getIdProcedimiento() != null
					? getNumeroProcesoEntity(unidad.getIdProcedimiento(), session).getNumProcedimiento() : null;
			retorno.add(InsideServiceStoreHibernateConverterUnidad
					.toInside(getUnidadOrganicaEntity(session, unidad.getIdUnidad()), numeroProcedimiento, unidad.isActivo()));
		}
	}
	...
	@Override
	public List<ObjetoInsideUnidad> getUnidadesUsuario(String idUsuario, boolean todas) throws InsideServiceStoreException {
	...
	private List<ObjetoInsideUnidad> getTodasUnidadesUsuario(Session session) throws InsideServiceStoreException {
		List<ObjetoInsideUnidad> retorno = new ArrayList<ObjetoInsideUnidad>();
	...
	private List<ObjetoInsideUnidad> getUnidadesUsuario(String idUsuario, boolean todas, Session session)
			throws InsideServiceStoreException {
		List<ObjetoInsideUnidad> retorno = new ArrayList<ObjetoInsideUnidad>();
	...
	public void getUnidadProcedimiento(Session session, List<ObjetoInsideUnidad> retorno, UnidadUsuario unidad)
			throws InsideServiceStoreException {
		if (unidad != null) {
			String numeroProcedimiento = unidad.getIdProcedimiento() != null
					? getNumeroProcesoEntity(unidad.getIdProcedimiento(), session).getNumProcedimiento() : null;
			retorno.add(InsideServiceStoreHibernateConverterUnidad
					.toInside(getUnidadOrganicaEntity(session, unidad.getUnidad().getId()), numeroProcedimiento, unidad.isActivo()));
		}
	}
	...
	@Override
	public boolean comprobarUnidadesOrganicasActivasUsuario(Object usuario) throws InsideServiceStoreException {
		logger.debug("Inicio comprobarUnidadesOrganicasActivasUsuario");
		boolean validar = false;
		Session session = sessionFactory.openSession();
		try {
			if (usuario instanceof ObjetoInsideUsuario) {
				UsuarioInside usuEntity = getUsuarioEntity(((ObjetoInsideUsuario) usuario).getNif(), session);
				if (usuEntity.contieneUnidadActiva())
					validar = true;
			} else if (usuario instanceof ObjetoInsideAplicacion) {
				InsideWsAplicacion usuEntity = getAplicacionEntity(((ObjetoInsideAplicacion) usuario).getAplicacion(), session);
				if (usuEntity.contieneUnidadActiva())
					validar = true;
			}
		} finally {
	...
	public List<ExpedienteInsideIndice> indicesAsociadosExpediente(String identificadorExpediente, Session session) {
	...
	public List<ObjetoInsideExpedienteUnidad> getViewsExpedient(String identificadorExpediente)
		...
		Session session = sessionFactory.openSession();
		try {
			List<ExpedienteInsideIndice> expedienteIndice = indicesAsociadosExpediente(identificadorExpediente, session);
			...
			if (!CollectionUtils.isEmpty(expedienteIndice)) {
				...
				for (ExpedienteInsideIndice indice : expedienteIndice) {
					Criteria crit = session.createCriteria(ExpedienteInside.class).createAlias("expedienteInsideIndice",
							"indice");
	...
	@Override
	public ObjetoInsideAplicacion actualizarCredencialesEeetuilApp(String app, ObjetoCredencialEeutil credential)
			throws InsideServiceStoreException {
		logger.debug("Inicio altaAplicacion");
		Session session = sessionFactory.openSession();
		try {
	...
	@Override
	public void insertRespuestaRemisionJusticia(ObjectInsideRespuestaEnvioJusticia objectInsideRespuestaEnvioJusticia,
			String identificadorExpediente, String version) throws InsideServiceStoreException {

		persister.persistRespuestaRemisionJusticiaExpediente(objectInsideRespuestaEnvioJusticia, identificadorExpediente,
				version);
	...
	public void insertRespuestaRemisionJusticiaExpedienteNoInside(
			ObjectInsideRespuestaEnvioJusticia objectInsideRespuestaEnvioJusticia, String identificadorExpediente, String version)
					throws InsideServiceStoreException {

		persister.persistRespuestaRemisionJusticiaExpedienteNoInside(objectInsideRespuestaEnvioJusticia, identificadorExpediente,
				version);

	}

	@Override
	public ObjetoInsideAplicacion getAplicacionBySerialNumber(String serialNumberCertificado) throws InsideServiceStoreException {
		Session session = sessionFactory.openSession();
		ObjetoInsideAplicacion aplicacionInside = null;
	...
	public ObjetoInsideUnidadAplicacionEeutil getApplicationEeutilByUser(ObjetoInsideUsuario user)
		...
		try {
			...
			retorno = InsideServiceStoreHibernateConverterUnidadAplicacionEeutil.toInside(unidadApp, null);
		} finally {
			session.close();
		}
		return retorno;
	...
	public ObjetoInsideUnidadAplicacionEeutil crearActualizarUnidadAplicacionEeutil(String codigoUnidad, String aplicacion,
			String password) throws InsideServiceStoreException {
		Session session = sessionFactory.openSession();
		...
		try {
			objetoUnidadAplicacionEeutil = persister.saveUnidadAplicacionEeutil(unidadAplicacionEeutil, session);
	...
	@Override
	public ObjetoInsideUnidadAplicacionEeutil getUnidadAplicacionEeutil(String codigoUnidad) throws InsideServiceStoreException {
		ObjetoInsideUnidadAplicacionEeutil objetoInsideUnidadAplicacionEeutil = null;
		List<UnidadAplicacionEeutil> listaUnidadAplicacionEeutil = getListUnidadAplicacionEeutilEntity(codigoUnidad);
		Session session = sessionFactory.openSession();
		try {
			if (CollectionUtils.isNotEmpty(listaUnidadAplicacionEeutil)) {
				UnidadOrganica unidad = getUnidadEntity(listaUnidadAplicacionEeutil.get(0).getIdUnidad(), session);
				objetoInsideUnidadAplicacionEeutil = InsideServiceStoreHibernateConverterUnidadAplicacionEeutil
	...
	@Override
	public List<ObjetoInsideUnidadAplicacionEeutil> getListUnidadAplicacionEeutil() throws InsideServiceStoreException {
		List<UnidadAplicacionEeutil> listUnidadAplicacionEeutil = getListUnidadAplicacionEeutilEntity(null);
		List<ObjetoInsideUnidadAplicacionEeutil> listObjetoInsideUnidadAplicacionEeutil = new ArrayList<ObjetoInsideUnidadAplicacionEeutil>();
		Session session = sessionFactory.openSession();
		try {
			for (UnidadAplicacionEeutil unidadAplicacionEeutil : listUnidadAplicacionEeutil) {
				...
				listObjetoInsideUnidadAplicacionEeutil.add(InsideServiceStoreHibernateConverterUnidadAplicacionEeutil
	...
	public UnidadAplicacionEeutil getUnidadAplicacionEeutilEntity(String codigoUnidad) throws InsideServiceStoreException {
		UnidadAplicacionEeutil unidadAplicacionEeutil = null;
		List<UnidadAplicacionEeutil> listUnidadAplicacionEeutil = getListUnidadAplicacionEeutilEntity(codigoUnidad);
	...
	public String altaNumeroProcedimiento(String numeroProcedimiento) throws InsideServiceStoreException {
		...
		try {
			...
			if (lista != null && !lista.isEmpty()) {
				throw new InsideServiceStoreException("Ya existe el número de procedimiento: " + numeroProcedimiento);
			}
	...
	public List<NumeroProcedimiento> getNumeroProcedimientoEntity(String numeroProcedimiento, Session session) throws InsideServiceStoreException {
	...
	@Override
	public ObjetoEstructuraCarpetaInside getEstructuraCarpeta(ObjetoInsideUsuario user) throws InsideServiceStoreException {
		...
		try {
			...
			if (usuario != null) {
				...
				if (CollectionUtils.isNotEmpty(datosEstructura)) {
					estructuraCarpeta = InsideServiceStoreHibernateConverterEstructuraCarpeta.toInside(datosEstructura.get(0));
			...
			if (obtenerEstructuraGenerica) {
				...
				estructuraCarpeta = InsideServiceStoreHibernateConverterEstructuraCarpeta.toInside(estructuraEntity);
			}
		} finally {
			session.close();
		}
	...
	public List<ObjetoEstructuraCarpetaInside> listaEstructuraCarpeta(String identificadorEstructura)
		...
		List<ObjetoEstructuraCarpetaInside> listaEstructuraCarpeta = new ArrayList<ObjetoEstructuraCarpetaInside>();
		Session session = sessionFactory.openSession();
		try {
			List<EstructuraCarpetaInside> datos = getEstructuraCarpetaEntity(identificadorEstructura, session);
			for (EstructuraCarpetaInside es : datos) {
				listaEstructuraCarpeta.add(InsideServiceStoreHibernateConverterEstructuraCarpeta.toInside(es));
			}
		} finally {
			session.close();
		}
	...
	public List<EstructuraCarpetaInside> getEstructuraCarpetaEntity(String identificadorEstructura, Session session) {
		Criteria crit = session.createCriteria(EstructuraCarpetaInside.class);
	...
	public ObjetoEstructuraCarpetaInside altaEstructuraCarpeta(ObjetoEstructuraCarpetaInside objetoEstructuraCarpetaInside)
		...
		Session session = sessionFactory.openSession();
		try {
			estructuraCarpeta = InsideServiceStoreHibernateConverterEstructuraCarpeta.toEntity(objetoEstructuraCarpetaInside);
			if (StringUtils.isNotBlank(objetoEstructuraCarpetaInside.getCodigoUnidadOrganica())) {
				estructuraCarpeta
						.setUnidad(persister.getUnidadEntity(objetoEstructuraCarpetaInside.getCodigoUnidadOrganica(), session));
			}
			if (StringUtils.isNotBlank(objetoEstructuraCarpetaInside.getNumeroProcedimiento())) {
				estructuraCarpeta.setNumeroProcedimiento(
						persister.getNumeroProcesoEntity(objetoEstructuraCarpetaInside.getNumeroProcedimiento(), session));
			}
	...
	public ObjetoSolicitudAccesoExpAppUrl saveSolicitudAccesoExpAppUrl(
		...
		try {
			respuesta = persister.saveSolicitudAccesoExpAppUrl(
					InsideServiceStoreHibernateConverterSolicitudAccesoExpAppUrl.toEntity(objetoSolicitudAccesoExpAppUrl),
					session);
		} finally {
			session.close();
		}
	...
	public ObjectInsideRespuestaEnvioJusticia getRespuestaEvioJusticaByCodigoEnvio(
		...
		try {
			...
			ExpedienteInsideRespuestaEnvioJusticia datosRecogidos = (ExpedienteInsideRespuestaEnvioJusticia) crit.uniqueResult();

			if (datosRecogidos != null) {
				retorno = converterExpedienteInsideRespuestaEnvioJusticia.toInside(datosRecogidos);
			} else {
				ObjectInsideRespuestaEnvioJusticia ningunResultado = new ObjectInsideRespuestaEnvioJusticia();
	...
	public ObjectInsideRespuestaEnvioJusticia getRespuestaEnvioJusticiaByCodigoEnvio(String codigoEnvio)
		...
		try {
			...
			if (datosRecogidos != null) {
				...
				crit.add(Restrictions.eq("codigoEnvio", codigoEnvio));
				ExpedienteNoInsideRespuestaEnvioJusticia datosNoInsideRecogidos = (ExpedienteNoInsideRespuestaEnvioJusticia) crit
						.uniqueResult();
				if (datosNoInsideRecogidos != null) {
					retorno = converterExpedienteNoInsideRespuestaEnvioJusticia.toInside(datosNoInsideRecogidos);
				}
			}
	...
	private List<ObjetoElastic> getExpedientesByOrgano(String organo) throws InsideServiceStoreException {
	...
	private List<ObjetoElastic> getExpedientesInsideOrgano(String organo, Session session) {
		...
		for (ExpedienteInsideOrgano expedienteInsideOrgano : datos) {
			...
			objetoElastic.setIdentificador(expedienteInsideOrgano.getExpedienteInside().getIdentificador());
			objetoElastic.setMetadatos(new HashMap<String, String>());
			objetoElastic.getMetadatos().put("VersionNTI", expedienteInsideOrgano.getExpedienteInside().getVersionNti());
			objetoElastic.getMetadatos().put("Identificador", expedienteInsideOrgano.getExpedienteInside().getIdentificador());
			...
			objetoElastic.getMetadatos().put("Clasificacion", expedienteInsideOrgano.getExpedienteInside().getClasificacion());
			objetoElastic.getMetadatos().put("Estado", expedienteInsideOrgano.getExpedienteInside().getEstado());
			objetosElastic.add(objetoElastic);

			objetoElastic.setMetadatosAdicionales(new HashMap<String, String>());
			for (ExpedienteInsideMetadatosAdicionales metadatosAdicional : expedienteInsideOrgano.getExpedienteInside()
					.getExpedienteInsideMetadatosAdicionaleses()) {
				objetoElastic.getMetadatosAdicionales().put(metadatosAdicional.getNombre(), metadatosAdicional.getValor());
			}
	...
	private List<ObjetoElastic> getDocumentosInsideOrgano(String organo, Session session) {
		...
		for (DocumentoInsideOrgano documentoInsideOrgano : datos) {
			...
			objetoElastic.setMetadatos(new HashMap<String, String>());
			objetoElastic.getMetadatos().put("VersionNTI", documentoInsideOrgano.getDocumentoInside().getVersionNti());
			objetoElastic.getMetadatos().put("Identificador", documentoInsideOrgano.getDocumentoInside().getIdentificador());
			objetoElastic.getMetadatos().put("Organo", documentoInsideOrgano.getOrgano());
			objetoElastic.getMetadatos().put("FechaCaptura",
					new SimpleDateFormat("dd/MM/yyyy").format(documentoInsideOrgano.getDocumentoInside().getFechaCaptura()));
			objetoElastic.getMetadatos().put("OrigenCiudadanoAdministracion",
					documentoInsideOrgano.getDocumentoInside().getOrigenCiudadanoAdministracion() ? "Ciudadano" : "Admon");
			objetoElastic.getMetadatos().put("EstadoElaboracion",
					documentoInsideOrgano.getDocumentoInside().getEstadoElaboracion());
			objetosElastic.add(objetoElastic);

			objetoElastic.setMetadatosAdicionales(new HashMap<String, String>());
			for (DocumentoInsideMetadatosAdicionales metadatosAdicional : documentoInsideOrgano.getDocumentoInside()
	...
	@Override
	public int consultAndIncreaseDefaultIdByDir3(String unidadOrganica, String elemento) throws InsideServiceStoreException {
		Session session = sessionFactory.openSession();
		int numeroId;
		try {
	...
	public void saveAuditoriaFirmaServidor(ObjetoAuditoriaFirmaServidor objetoAuditoriaFirmaServidor) {
		...
		Session session = sessionFactory.openSession();
		try {
			persister.saveAuditoriaFirmaServidor(
					InsideServiceStoreHibernateConverterAuditoriaFirmaServidor.toEntity(objetoAuditoriaFirmaServidor), session);
		} catch (InsideServiceStoreException e) {
			logger.error("Error en saveAuditoriaFirmaServidor al almacenar: " + objetoAuditoriaFirmaServidor.toString(), e);
		} finally {
			session.close();
		}
	}
	...
	public void setConverterInsideRol(InsideServiceStoreHibernateConverterInsideRol converterInsideRol) {
...
//	}
	...
	public List<ObjetoInsideUsuario> getUsuariosUnidadOrganica(	ObjetoInsideUsuario usuarioEnSesion, Locale locale) throws InsideServiceStoreException {
		...
		String unidadOrganicaActivaConDescripcionDelUsuarioLogado = usuarioEnSesion.getUnidadOrganicaActiva();
		String unidadOrganicaActivaDelUsuarioLogadoCodigo = unidadOrganicaActivaConDescripcionDelUsuarioLogado.split("-")[0].trim();
		String unidadOrganicaActivaDelUsuarioLogadoDescripcion = unidadOrganicaActivaConDescripcionDelUsuarioLogado.split("-")[1].trim();
		...
		try {
			...
			if (CollectionUtils.isNotEmpty(datos)) {
				for (UsuarioInside user : datos) 
				{
					...
					if( unidadBuscada!=null && !usuarioEnSesion.getNif().trim().equalsIgnoreCase(user.getNif().trim()))
					{
						...
						{
							NumeroProcedimiento numProc = getNumeroProcesoEntity(unidadBuscada.getIdProcedimiento(),session);
						...
						obj.getRol().setDescripcionLargaTraducida(messageSource.getMessage(insideRol.getDescripcionLarga(), null, locale));
