...
public class InsideServiceCsvStorageAdapter implements InsideServiceAdapter {
	 ...
	 */
	public ObjetoExpedienteInside almacenaObjetoExpedienteInside(ObjetoExpedienteInside expediente, byte[] bytesContenido)
		...
		try {
			...
			if (expediente.getIndice() != null && expediente.getIndice().getFirmas() != null
				...
				this.almacenaFirmasInside(expediente.getIndice().getFirmas(), expediente.getIdentificador(), dirusuario, true);
			}
			logger.info("Sale de almacenaExpedienteEni");

		} catch (InsideServiceAdapterException e) {
			logger.error("Error almacenando el cml del ExpedinteInside: " + expediente != null ? expediente.getIdentificador()
					: "" + " Usuario: " + dirusuario);
	...
	public ObjetoExpedienteInside recuperaExpedienteInside(ObjetoExpedienteInside expediente)
		...
		if (StringUtils.isEmpty(expediente.getIdentificadorRepositorio())) {
			throw new InsideServiceAdapterException ("El identificador del repositorio del expediente no puede ser nulo");
		}
		
		try {
			logger.debug("IdentificadorRepositorio: " + expediente.getIdentificadorRepositorio());
			// En el almacenamiento en Alfresco no se está recuperando el solo se están recuperando las firmas
			if (expediente.getIndice().getFirmas() != null) {
				...
				for (FirmaInside firma : expediente.getIndice().getFirmas()) {
					...
					if( contenidoFirma != null && contenidoFirma instanceof ContenidoFirmaCertificadoAlmacenableInside)
					{
						...
						logger.debug("contenidoFirmaAlmacenado IdentificadorRepositorio: " + contenidoFirmaAlmacenado.getIdentificadorRepositorio());
						...
						if (contenidoFirmaAlmacenado.getIdentificadorRepositorio() != null
							...
							String[] nombreSplit = contenidoFirmaAlmacenado.getIdentificadorRepositorio().split("/");
							...
							DocumentoMtomResponse documentoResponse = insideCsvStorageMtomService.obtenerFirmaCsvStorage(id, dir3);
							
							if (documentoResponse != null && documentoResponse.getCodigo()
									.equals(Constantes.csvStorage.GUARDADOCORRECTO)) {
	 ...
	 *            the expediente
	 * @throws InsideServiceAdapterException
	 *             the inside service cmis adapter exception
	 */
	private void almacenaFirmasInside(List<FirmaInside> firmas, String idENI, String dir3, boolean guardar) throws InsideServiceAdapterException {
		...
		for (FirmaInside firma : firmas) {
			if (firma.getContenidoFirma() != null) {
				...
				if (contenidoFirma instanceof ContenidoFirmaCertificadoAlmacenableInside) {
					ContenidoFirmaCertificadoAlmacenableInside contenidoFirmaAlmacenado = (ContenidoFirmaCertificadoAlmacenableInside) contenidoFirma;
					try {
						if (contenidoFirmaAlmacenado.getMime() != null && contenidoFirmaAlmacenado.getMime().equals("application/octect-stream") ) {
							contenidoFirmaAlmacenado.setMime("application/octet-stream");
						} else if (contenidoFirmaAlmacenado.getMime() != null && contenidoFirmaAlmacenado.getMime().equals("application/xml") ) {
							contenidoFirmaAlmacenado.setMime("text/xml");
						}
						// El idENI de la firma que almacenamos en el csvstorage lo componemos con el idENI del expediente +  IdentificadorEnDocumento (FIRMA_0, FIRMA_1, etc...)
						...
						if (contenidoFirmaAlmacenado.getValorBinario() != null) {
							...
							if (guardar) {
								GuardarDocumentoResponse response = insideCsvStorageMtomService
										.guardarFirmaCsvStorage(dir3, firmaName, contenidoFirmaAlmacenado.getValorBinario(), contenidoFirmaAlmacenado.getMime());
	
								if (response != null && response.getResponse() != null && response.getResponse().getCodigo()
										.equals(Constantes.csvStorage.YAEXISTE)) {
									logger.warn("No se ha guardado la firma porque ya existe: " + response.getResponse().getCodigo() + " - " + response.getResponse().getDescripcion());
									// Llamamos a modificar
									response = insideCsvStorageMtomService.modificarFirmaCsvStorage(
											dir3, firmaName, contenidoFirmaAlmacenado.getValorBinario(), contenidoFirmaAlmacenado.getMime());
								} 
								
								if (response != null && response.getResponse() != null && !response.getResponse().getCodigo()
										.equals(Constantes.csvStorage.GUARDADOCORRECTO)) {
									logger.error("No se ha guardado la firma porque se ha producido un error: " 
										+ response.getResponse().getCodigo() + " - " + response.getResponse().getDescripcion());
									// Lanzamos excepción
	...
	public ObjetoDocumentoInside almacenaObjetoDocumentoInside(ObjetoDocumentoInside documento)
		...
		try {
			...
			logger.debug("IdentificadorRepositorio: "
					+ documento.getIdentificadorRepositorio());
			...
			if (documento.getIdentificadorRepositorio() != null) {
				...
				String[] identifiadorSplit = documento
						.getIdentificadorRepositorio().split("/");
				...
				response = insideCsvStorageMtomService.modificarDocumentoCsvStorage(dir3, documento.getIdentificador(), 
						contentMarshall, mimeType);
				
			} else {
				
				// El IdentificadorRepositorio se compone del dir3/identificador
				// del documento
				documento.setIdentificadorRepositorio(dirusuario + "/"
			...
			logger.error("Error almacenando el documento: " + (documento != null ? documento.getIdentificador()
					: "") + " Usuario: " + dirusuario);
	...
	public ObjetoDocumentoInside recuperaDocumentoInside(ObjetoDocumentoInside documento)
		...
		if (documento.getIdentificadorRepositorio() != null) {
			...
			try {
				DocumentoMtomResponse documentoRespone = insideCsvStorageMtomService.obtenerDocumentoCsvStorage(id, dir3);
	 ...
	 */
	private void convertDocumentResponseToObjetoDocumentoInside(
			byte[] contenido, ObjetoDocumentoInside documento) throws InsideServiceAdapterException {
		try {
			...
			if (contenido !=null) {
				...
				try {
					...
					ObjetoDocumentoInside documentoAux = InsideConverterDocumento.documentoEniAndMetadatosToDocumentoInside(tipoDocumento, null);
					...
					// Para recuperar documentos de versiones anteriores almacenados como ObjetoDocumentoInside
	...
	private String getUserDir() {
		...
		UsuarioInside usuario = (UsuarioInside) insideServiceJta.findObjeto(UsuarioInside.class, criterias);
		...
		if (usuario != null) {
			...
			List<UnidadUsuario> usuariosUnidad = insideServiceJta.getAllObjetosCriteria(UnidadUsuario.class, criterias);
			...
			InsideWsAplicacion aplicacion = (InsideWsAplicacion) insideServiceJta.findObjeto(InsideWsAplicacion.class, criterias);
			
			criterias = new ArrayList<Criterion>();
			criterias.add(Restrictions.eq("idAplicacion", aplicacion.getId()));
			criterias.add(Restrictions.eq("activo", true));
			List<UnidadWsAplicacion> aplicacionesUnidad = insideServiceJta.getAllObjetosCriteria(UnidadWsAplicacion.class, criterias);
	...
	public ObjetoDocumentoInside eliminaDocumentoInside(ObjetoDocumentoInside documento)
		...
		if (documento.getIdentificadorRepositorio() != null) {
			...
			try {
				EliminarDocumentoResponse eliminarDocumentoResponse = insideCsvStorageMtomService.eliminarDocumento(id, dir3);
				...
				if (eliminarDocumentoResponse.getResponse() == null 
					...
					logger.warn("El documento " + id + " no se ha eliminado del repositorio: "
							+ eliminarDocumentoResponse.getResponse()
									.getCodigo()
							+ " - "
							+ eliminarDocumentoResponse.getResponse()
									.getDescripcion());
