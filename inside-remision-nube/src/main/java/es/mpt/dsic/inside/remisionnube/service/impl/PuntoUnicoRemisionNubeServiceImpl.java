...
public class PuntoUnicoRemisionNubeServiceImpl implements PuntoUnicoRemisionNubeService {
	...
	public RespuestaConsultaEnvioJusticia consultaEstadoRemisionAJusticia(String codigoEnvioATEA) throws InsideWSException {
		...
		if (dato != null) {
			try {
				consulta = puntoUnicoJusticiaService.consultaRemisionExpedienteMJU(dato.getAuditoriaEsb_aplicacion(),
						dato.getAuditoriaEsb_modulo(), dato.getAuditoriaEsb_servicio(), dato.getAuditoriaEsb_marcaTiempo(),
						codigoEnvioATEA, dato.getCodigoUnidadOrganoRemitente());
	...
	private RespuestaConsultaEnvioJusticia toRespuestaConsultaEnvioJusticia(ConsultarRemisionAJusticiaResponse consulta) {
		...
		respuesta.setDescripcionEstado(consulta.getResultadoConsultaRemisionToJusticia().getDatosEnvio().getDescripcionEstado());
		respuesta.setComentario(consulta.getResultadoConsultaRemisionToJusticia().getDatosEnvio().getComentario());
		respuesta.setResguardo(consulta.getResultadoConsultaRemisionToJusticia().getDatosEnvio().getResguardo());
		return respuesta;

	}
	
	public ParametrosEnvioJusticia getParametrosMJU(DatosRemisionJusticiaType datosRemisionJusticiaType, String dir3Juzgado)
		...
		return getParametrosEnvioJusticia(nigCodigoPoblacion, nigTipoOrganoJudicial, nigNumeroOrden, nigAnyoAsunto,
				nigNumeroAsunto, dir3Juzgado, rjClaseProcedimiento, dir3Remitente, descripcion, rjNumeroProcedimiento,
				rjAnyoProcedimiento);

	}
	
	public ParametrosEnvioJusticia getParametrosEnvioJusticia(String nigCodigoPoblacion, String nigTipoOrganoJudicial,
			String nigNumeroOrden, String nigAnyoAsunto, String nigNumeroAsunto, String rjDir3Juzgado,
			String rjClaseProcedimiento, String dir3Remitente, String descripcion, String rjNumeroProcedimiento,
			String rjAnyoProcedimiento) throws InsideWSException {

		OrganoType organoType = obtenerOrganoParaParametrosMJU(nigCodigoPoblacion, nigTipoOrganoJudicial, nigNumeroOrden,
				rjDir3Juzgado);

		OrdenType orden = new OrdenType();
		orden.setCodigo(nigNumeroOrden);

		RespuestaObtenerClasesProcedimientoType datosClasesProcedimiento = puntoUnicoJusticiaService
				.listadoClasesProcedimientosByOrdenAndOrganos(organoType, orden);

		if (datosClasesProcedimiento != null && datosClasesProcedimiento.getClasesProcedimiento() != null
				&& CollectionUtils.isNotEmpty(datosClasesProcedimiento.getClasesProcedimiento().getClaseProcedimiento())
				&& !validaClaseProcedimiento(rjClaseProcedimiento,
						datosClasesProcedimiento.getClasesProcedimiento().getClaseProcedimiento())) {
			logger.error("La clase de procedimiento especificado no se encuentra para el organo " + organoType.getNumeroOrgano()
					+ " y orden especificado " + nigNumeroOrden);
	...
	private boolean validaClaseProcedimiento(String claseProcedimiento, List<ClaseProcedimientoType> listaClaseProcedimiento) {
	...
	private OrganoType obtenerOrganoParaParametrosMJU(String nigCodigoPoblacion, String nigTipoOrganoJudicial,
			String nigNumeroOrden, String rjDir3Juzgado) throws InsideWSException {
		...
		if (CollectionUtils.isEmpty(unidades)) {
			logger.error("No se encuentra el código de UNIDAD ORGANICA remitente para el dir3 destinatario " + rjDir3Juzgado);
		...
		RespuestaObtenerOrdenType datosOrden = puntoUnicoJusticiaService.listadoOrdenesByOrgano(organoType);
		...
		if (datosOrden != null && datosOrden.getOrdenes() != null
			...
			logger.error("El número de orden especificado en el Nig no se encuentra para el órgano especificado "
