...
public class DocumentController {
	...
	private static final String DOCUMENT_CONTROLLER_GENERAR_DOCUMENTO_POST = "DocumentController.generarDocumento";
	...
	@ResponseBody
	public Map<String, Object> generarDocumentoPost(@ModelAttribute TipoDocumentoConversionInside documentoEni,
			HttpServletRequest request, Locale locale, @RequestParam("metadatosAdicionalesDoc") String metadatosAdicionales,
			HttpSession session) {
		...
		try {
			...
			TipoDocumentoInsideConMAdicionales docConvertido = generateDocument(documentoEni, request, locale,
					metadatosAdicionales, session, retorno, aditionalData);
	...
	public TipoDocumentoInsideConMAdicionales generateDocument(TipoDocumentoConversionInside documentoEni,
			HttpServletRequest request, Locale locale, String metadatosAdicionales, HttpSession session,
			Map<String, Object> retorno, TipoMetadatosAdicionales aditionalData) throws InsideServiceInternalException {
		TipoDocumentoInsideConMAdicionales tipoDoc = null;
		try {
			
			validateData(documentoEni, locale, request.getParameter(DOCUMENT_ID));
			String documentId = temporalDataBusinessService.getPath(session.getId(), request.getParameter(DOCUMENT_ID));
			...
			tipoDoc = insideUtilService.convertirDocumentoAEniConMAdicionales(documentoEni, aditionalData, contenidoParam , false, null, docInside);
			
		} catch (InsideServiceObjectValidationException e1) {
			logger.warn(DOCUMENT_CONTROLLER_GENERAR_DOCUMENTO_POST, e1);
			throw new InsideServiceInternalException(MessageUtils.contruirMensaje(locale, e1, messageSource).getMessage());
	...
	public void setXmlTemporal(TipoDocumentoConversionInside documentoEni, HttpSession session,
		...
		try {
			if (CollectionUtils.isNotEmpty(aditionalData.getMetadatoAdicional())) {
				String pathFicheroDocENIConAdicionales = temporalDataBusinessService.getPath(session.getId(),
						documentoEni.getMetadatosEni().getIdentificador() + WebConstants.FORMAT_XML);

				marshaller.marshallDataDocumentAndWriteFile(docConvertido, pathFicheroDocENIConAdicionales, NAMESPACE_WEBSERVICE,
						DOCUMENTO, "insidews");
			} else {
	...
	public ModelAndView descargarDocumentoEni(@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response,
	...
	public ModelAndView descargarContenidoDocumento(@RequestParam(IDENTIFICADOR) String identificador,
		...
		try {
			...
			String identificadorCompleto = StringUtils.isNotBlank(extension) ? identificador + extension : identificador;
	...
	public boolean isFormatTif(String extension, String mime) {
		return StringUtils.isEmpty(extension) && StringUtils.containsIgnoreCase("." + mime, WebConstants.FORMAT_TIF);
	}
	...
	public ModelAndView descargarVisualizar(@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response,
			Locale locale, HttpSession session) {
		try {
			...
			TipoDocumentoInsideConMAdicionales tDoc = marshaller.unmarshallDataDocumentAditional(docEniBytes);
			visualizar.setMetadatosAdicionales(tDoc.getMetadatosAdicionales());

			TipoResultadoVisualizacionDocumentoInside resultado = insideUtilService.visualizarDocumentoEni(visualizar);
	...
	@ResponseBody
	public Map<String, Object> validarDocumentoPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
	...
	public TipoOpcionesValidacionDocumentoInside getOpcionesValidacion(List<String> opcionesValidacion) {
	...
	public ModelAndView visualizarDocumentoPost(Locale locale, @RequestParam(NAVEGADOR) String navegador, HttpServletRequest request, HttpSession session) {
		MessageObject mensaje = null;
		ModelAndView retorno = new ModelAndView("documento/visualizarDocumento");
		try {
	...
	public ModelAndView visualizaDescargarContenidoDocumento(@RequestParam(IDENTIFICADOR) String identificador,
			HttpServletResponse response, Locale locale, HttpSession session) {
		try {
	...
	public Map<String, Object> updateIndexExp(Locale locale, HttpServletRequest request) throws InSideServiceException {
		...
		try {
			MessageObject mensaje;
			String usuario = ((ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION)).getNif();
			if (StringUtils.isEmpty(request.getParameter(INDEX_EXP_B64))) {
				...
				String expedienteEniString = mapper.readValue(request.getParameter("expedienteEni"), String.class);
	...
	private ObjetoExpedienteInside validarExpedienteUnidad(HttpServletRequest request, Map<String, Object> retorno, Locale locale)
			throws InsideServiceInternalException {
		...
		try {
			...
			if (StringUtils.isNotBlank(idJsonExpediente)) {
				...
				validar = insideService.permisoObjetoUnidad(expediente,
						((ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION)).getNif(), true);
			}
		} catch (InsideStoreObjectNotFoundException e) {
			retorno.put(MENSAJE_USU, getErrorMessage(WebConstants.MSG_IMPORTAR_DOC_EXP_NO_ENCONTRADO, null, locale));
			throw new InsideServiceInternalException("Error al recuperar el expediente: " + identificadorExpediente, e);
		} catch (IOException e) {
			retorno.put(MENSAJE_USU, getErrorMessage(WebConstants.MSG_IMPORTAR_EXP_EXP_NO_VALIDO, null, locale));
			throw new InsideServiceInternalException("Error al recuperar el JSON: " + idJsonExpediente, e);
		} catch (InSideServiceException e) {
		...
		if (!validar) {
			retorno.put(MENSAJE_USU, getErrorMessage(WebConstants.MSG_IMPORTAR_DOC_USUARIO_NO_ASOCIADO_EXP, null, locale));
	...
	@ResponseBody
	public Map<String, Object> saveDocumentPost(@ModelAttribute TipoDocumentoConversionInside documentoEni,
			HttpServletRequest request, Locale locale, @RequestParam("metadatosAdicionalesDoc") String metadatosAdicionales,
			HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		TipoMetadatosAdicionales aditionalData = new TipoMetadatosAdicionales();
		try {
			...
			ObjetoDocumentoInside docInside = insideUtilService
					.validateDocumentImport(insideUtilService.generateDocXml(docConvertido));
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
	...
	public ObjetoDocumentoInside getDataSigned(TipoDocumentoConversionInside documentoEni, HttpSession session, String signId) {
		ObjetoDocumentoInside docInside = null;
		if (StringUtils.isEmpty(signId)) {
			docInside = obtenerDatosFirma(session.getId(), documentoEni.getMetadatosEni().getIdentificador());
		}
		return docInside;
	}
	...
	public String extraerFirmaResultado(HttpServletRequest request, HttpSession session) {
		String signId = null;
		if (FIRMA.equals(request.getParameter(FIRMA)) && StringUtils.isNotEmpty(request.getParameter(FIRMA_RESULTADO))) {
			signId = temporalDataBusinessService.getPath(session.getId(), request.getParameter(FIRMA_RESULTADO));
		}
		return signId;
	}
	...
	public void setAdicionales(String metadatosAdicionales, TipoMetadatosAdicionales aditionalData)
		...
		try {
			...
			ObjectMapper mapper = new ObjectMapper();
			listAditionalData = mapper.readValue(metadatosAdicionales, new TypeReference<List<MetadatoAdicional>>() {
			});
			aditionalData.getMetadatoAdicional().addAll(listAditionalData);
		} catch (Exception e3) {
			logger.error(messageSource.getMessage(WebConstants.MSG_GENERAR_DOC_ERROR_METADATOS, null, null), e3);
			throw new InsideServiceInternalException(
	...
	@ResponseBody
	public Map<String, Object> updateDocumentPost(@ModelAttribute TipoDocumentoConversionInside documentoEni,
			HttpServletRequest request, Locale locale, @RequestParam("metadatosAdicionalesDoc") String metadatosAdicionales,
			HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		TipoMetadatosAdicionales aditionalData = new TipoMetadatosAdicionales();
	...
	@ResponseBody
	public Map<String, Object> importNewDocumentPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		ObjectMapper mapper = new ObjectMapper();
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
			...
			if (firmaServidor) {
				insideService.altaDocumento(objDoc, expediente.getIdentificador(), null, usuario.getNif(), true);
				...
				docs.add(objDoc);
				expediente = insideService.vincularDocumentosEnExpedienteWithoutSave(expediente, docs, null, usuario.getNif(),
						true);
				addExpReturn(locale, retorno, expediente);
			}
	...
	@ResponseBody
	public Map<String, Object> importDocumentPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		ObjectMapper mapper = new ObjectMapper();
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
			...
			if (firmaServidor) {
				insideService.modificaDocumento(objDoc, expediente.getIdentificador(), null, usuario.getNif(), true);
				addExpReturn(locale, retorno, expediente);
			} else {
	...
	@ResponseBody
	public Map<String, Object> importNewDocumentXmlPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		ObjectMapper mapper = new ObjectMapper();
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
	...
	public ObjetoDocumentoInside getDocumentRequest(Locale locale, HttpServletRequest request, HttpSession session,
			Map<String, Object> retorno) throws InSideServiceException {
		ObjetoDocumentoInside doc = null;
		try {
			ObjectMapper mapper = new ObjectMapper();
			String identificadorDoc = mapper.readValue(request.getParameter(DOCUMENT_ID), String.class);
			doc = insideUtilService
					.validateDocumentImport(temporalDataBusinessService.getFile(session.getId(), identificadorDoc));
		} catch (IOException e2) {
	...
	public void addExpReturn(Locale locale, Map<String, Object> retorno, ObjetoExpedienteInside expediente)
		...
		try {
			...
			retorno.put(MENSAJE_USU, getErrorMessage(WebConstants.MSG_IMPORTAR_DOC_NO_VALIDO, null, locale));
			retorno.put(ERROR_GUARDAR, true);
			throw new InsideServiceInternalException("Error al convertir el expediente a Eni", e);
		}
	}

	private void checkExistDocumentInIndex(String identifierDoc, ObjetoExpedienteInsideIndice indiceContenido,
			Map<String, Object> retorno, Locale locale) throws InsideServiceInternalException {
		existDocumentIndex = false;
		if (comprobarSubcarpeta(identifierDoc, indiceContenido.getIndiceContenido().getElementosIndizados())) {
			retorno.put(MENSAJE_USU, getErrorMessage(WebConstants.MSG_IMPORTAR_DOC_EXISTE_EN_INDICE, null, locale));
			throw new InsideServiceInternalException("Error en checkExistDocumentInIndex. Documento repetido en el indice");
		}
	}

	private boolean comprobarSubcarpeta(String identifierDoc, List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> list) {
		for (ObjetoExpedienteInsideIndiceContenidoElementoIndizado elemento : list) {
			if (elemento instanceof ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) {
				existDocumentIndex = comprobarSubcarpeta(identifierDoc,
						((ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) elemento).getElementosIndizados());
	...
	public MessageObject getSuccessMessage(String tipo, Object[] args, Locale locale) {
		return new MessageObject(WebConstants.MESSAGE_LEVEL_SUCCESS, messageSource.getMessage(tipo, args, locale));
	}

	public MessageObject getErrorMessage(String tipo, Object[] args, Locale locale) {
		return new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, messageSource.getMessage(tipo, args, locale));
	}
	...
	public ModelAndView borrarDocumentoPost(HttpSession session, @RequestParam(IDENTIFICADOR) String identificador, Locale locale)
			throws InSideServiceException {
		...
		try {
			...
			logger.warn("DocumentController.borrarDocumento --> Error al borrar el documento: " + identificador, e);
			...
			logger.error("DocumentController.borrarDocumento --> Error al borrar el documento: " + identificador, e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
			retorno.addObject(DOCUMENTOS, getDocumentosAlmacenados(session));
		}
	...
	public ModelAndView documentosAlmacenados(HttpSession session,
		...
		try {
			...
			logger.error("InsideDocumentController.documentosAlmacenados --> Error al obtener los documentos del usuario", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
		}
		return retorno;
	...
	public ModelAndView editarDocumento(Locale locale, @RequestParam(IDENTIFICADOR) String identificador,@RequestParam(NAVEGADOR) String navegador, HttpSession session,
		...
		try {
			...
			TipoDocumentoInsideConMAdicionales docConvertido = insideUtilService
					.convertirDocumentoAEniConMAdicionales(documentoEni, aditionalData, null, false, null, docInside);
			...
			Item itemDocumento = insideServiceVisualizacion.documentoInsideToItemVisualizacion(
					InsideConverterDocumento.documentoEniAndMetadatosToDocumentoInside(docConvertido.getDocumento(),
							docConvertido.getMetadatosAdicionales()),
					null);
			List<VisualizacionItem> visItems = visualizacionUtils.obtenerVisualizacionItems(itemDocumento);
			...
			retorno.addObject(MENSAJE_USU, mensaje);
			/* CARM ### v2.0.7.1			
			retorno.setViewName("redirect:/documentosAlmacenados?textoMensajeUsuario=" + mensaje.getMessage()
					+ "&nivelMensajeUsuario=" + mensaje.getLevel()); */
			retorno.addObject("textoMensajeUsuario", mensaje.getMessage());
			retorno.setViewName("redirect:/documentosAlmacenados?nivelMensajeUsuario=" + mensaje.getLevel());			
			// CARM 2.0.7.1 ###
		} catch (Exception e) {
	...
	public void setMetadatosDocument(HttpSession session, ModelAndView retorno, TipoDocumentoConversionInside documentoEni,
	...
	private void setMetadatosAdicionalesDocument(List<ObjetoInsideMetadatoAdicional> metadatosAdicionales, ModelAndView retorno) {
		List<ObjetoInsideMetadatoAdicional> listMetadatosAdicional = new ArrayList<ObjetoInsideMetadatoAdicional>();
		for (ObjetoInsideMetadatoAdicional metadatoAdicional : metadatosAdicionales) {
			if (MetadatosEEMGDE.METADATO_NOMBRE_NOMBRE_NATURAL.getValue().equals(metadatoAdicional.getNombre())) {
	...
	public void setDocumentContentTemporal(HttpSession session, ModelAndView retorno, TipoDocumentoConversionInside documentoEni)
			throws InsideServiceInternalException {
		try {
			...
			String name = "content_" + documentoEni.getMetadatosEni().getIdentificador() + "." + contenido.getNombreFormato();
			temporalDataBusinessService.escribir(contenido.getValorBinario(), session.getId(), name, true);
			retorno.addObject(DOCUMENT_ID, name);
		} catch (Exception e) {
			logger.error("Error al editar el documento: " + documentoEni.getMetadatosEni().getIdentificador()
					+ " en setDocumentContentTemporal", e);
			throw new InsideServiceInternalException("Error al editar documento en setDocumentContentTemporal", e);
		}
	}
	...
	public boolean contentCSVSignature(ObjetoDocumentoInside docInside) {
		return CollectionUtils.isNotEmpty(docInside.getFirmas()) && docInside.getFirmas().size() == 1
				&& FirmaInsideTipoFirmaEnum.TF_01.name().equals(docInside.getFirmas().get(0).getTipoFirma().name());
	}

	public void setDocumentContent(TipoDocumentoConversionInside documentoEni, ObjetoDocumentoInside docInside) {
		if (docInside.getContenido().getValorBinario() != null) {
			documentoEni.setContenido(docInside.getContenido().getValorBinario());
		}
	}

	public void setSignatureType(ModelAndView retorno, ObjetoDocumentoInside docInside, boolean firmaSoloCSV) {
	...
	public void setDocumentEniInTemporal(HttpSession session, TipoDocumentoConversionInside documentoEni,
			TipoDocumentoInsideConMAdicionales docInside) throws InsideServiceInternalException {
		try {
			temporalDataBusinessService.escribir(insideUtilService.generateDocXml(docInside), session.getId(),
			...
			throw new InsideServiceInternalException("Error al editar documento en setDocumentEniInTemporal", e);
		}
	}
		...
		if (CollectionUtils.isNotEmpty(documentoEni.getMetadatosEni().getOrgano())) {
			...
			for (String organo : loadOrganos) {
				...
				if (CollectionUtils.isNotEmpty(data)) {
					documentoEni.getMetadatosEni().getOrgano().add(data.get(0).getKey() + " - " + data.get(0).getValue());
				}
			}
		}
	}

	public void getDisplayDocument(Locale locale, ModelAndView retorno, ObjetoDocumentoInside docInside) {
		MessageObject mensaje;
		// Tamanio de fichero mayor de 2MB
		if (isFileTooLarge(docInside)) {
			String tamanio = FileUtils.byteCountToDisplaySize(docInside.getContenido().getValorBinario().length);
			String[] argumentos = { tamanio };
			logger.info("El tamaño del documento es: " + tamanio + " y supera los 2 MB. No es posible previsualizar");
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_INFO,
			...
			try {
				retorno.addObject(DATA_DOC_BASE64,
						Base64.encodeBase64String(insideServiceVisualizacion.visualizarContenidoOriginal(docInside)));
			} catch (InsideServiceVisualizacionException e) {
	...
	public boolean isFileTooLarge(ObjetoDocumentoInside docInside) {
		return docInside.getContenido().getValorBinario() != null
				&& (BigInteger.valueOf(docInside.getContenido().getValorBinario().length)).compareTo(DOS_MB) >= 0;
	}
	...
	private List<ObjetoInsideDocumentoUnidad> getDocumentosAlmacenados(HttpSession session) throws InSideServiceException {
		ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
	...
	private void setData(TipoDocumentoConversionInside documentoEni, HttpServletRequest request, String documento,
			HttpSession session, String signPath) throws ParseException, DatatypeConfigurationException,
					IOException, InsideConverterUtilsException, InSideServiceException {
		...
		documentoEni.getMetadatosEni()
				.setFechaCaptura(InsideWSUtils.stringToXmlCalendar(fechaCaptura, WebConstants.FORMATO_FECHA_DEFECTO));
		...
		if (StringUtils.isEmpty(documentoEni.getCsv().getRegulacionCSV())|| StringUtils.isEmpty(documentoEni.getCsv().getValorCSV())) {
		...
		documentoEni.getMetadatosEni().getOrgano().clear();
		documentoEni.getMetadatosEni().getOrgano().addAll(ComboUtils.getOrganosKeys(listOrganos, request));
	}

	public void setSignatureData(TipoDocumentoConversionInside documentoEni, HttpServletRequest request, String documento,
			HttpSession session, String signPath) throws InSideServiceException {
		try {
			if (FIRMA.equals(request.getParameter(FIRMA))) {
				...
				documentoEni.setFirmadoConCertificado(true);
				documentoEni
						.setContenido(getSignatureServer(documentoEni.getMetadatosEni().getIdentificador(), documento, session));
			} else {
			...
			throw new InsideServiceInternalException("Error al cargar la firma del documento: " + e.getMessage());
		}
	}
	...
	private byte[] getSignatureServer(String idDocumento, String documento, HttpSession session) {
		...
		try {
			...
			ObjetoAuditoriaFirmaServidor auditoria = new ObjetoAuditoriaFirmaServidor(Constantes.ELEMENTO_DOCUMENTO, idDocumento,
					usuario.getNif());
	...
	private WSCredentialInside getSignatureServerByUser(ObjetoInsideUsuario objetoInsideUsuario) {
		...
		try {
			ObjetoInsideUnidadAplicacionEeutil aplicationEeutil = insideService.getApplicationEeutilByUser(objetoInsideUsuario);
	...
	private void validateData(TipoDocumentoConversionInside tipoDocumentoConversion, Locale locale, String documento)
	...
	private ObjetoDocumentoInside obtenerDatosFirma(String folder, String fileName) {
		...
		try {
			byte[] fichTemp = temporalDataBusinessService.getFile(folder, fileName + WebConstants.FORMAT_XML);

			if (isPades(fichTemp) && fichTemp.length > 0) {
				TipoDocumento docTemp = jAXBMarshaller.unmarshallDataDocument(fichTemp);
				docInside = InsideConverterDocumento.documentoEniAndMetadatosToDocumentoInside(docTemp, null);
