...
public class LoginController {
	...
	public ModelAndView loginPost(@RequestParam(value = "error", required = false) String error, HttpServletRequest request,
			Locale locale, HttpSession session) {
		ModelAndView retorno = new ModelAndView("login");
		if (error != null) {
			/* CARM ### v2.0.7.1 
		...
		*/
		...
		try {
			...
			model.addAttribute("errorMsg",
					context.getMessage(bundle.getString(WebConstants.KEY_ERROR_LOGIN), null, request.getLocale()));
			*/
			model.addAttribute("errorMsg",
