...
public class AutocompleteController {
  ...
  /**
   * Recupera las unidades DIR3 para el autocompletado del envio a justicia. son las que tienen
   * valor en el campo codigoExterno distinto de null y nivelAdministracion = 6
   * 
   * @param codigo String con el código a completar o parte de él
   * @return List<SelectItemVO> lista a mostrar en el autocomplete
   */
  ...
  @ResponseBody
  public List<ComboItem> autocompleteCodigoDIREnvioJusticia(
      @RequestParam(value = "term") final String codigo) {
    ...
    List<ComboItem> listaReturn = new ArrayList<ComboItem>();
    for (UnidadOrganica unidad : unidades) {
      listaReturn.add(new ComboItem(unidad.getCodigoUnidadOrganica(),
          unidad.getNombreUnidadOrganica(), unidad.getCodigoExterno()));
    }

    logger.info("[FIN] Salimos de autocompleteCodigoDIREnvioJusticia. Total a mostrar: "
        + listaReturn.size());
  ...
  /**
   * Recupera las unidades DIR3 para el autocompletado del envio a justicia. Los dir3 vigentes son
   * estada V y T son las que tienen valor en el campo codigoExterno distinto de null y
  ...
  public List<ComboItem> autocompleteCodigoDIREnvioJusticiaVigentes(
    ...
    logger.info("[FIN] Salimos de autocompleteCodigoDIREnvioJusticiaVigentes. Total a mostrar: "
        + listaReturn.size());
   ...
   * 
   * @param codigo String con el código a completar o parte de él
   * @param allExp incluimos expedientes cerrados y abierto
   * @param uniAct solo expedientes asociados a nuestra unidad activa
   * @return List<SelectItemVO> lista a mostrar en el autocomplete
   */
  ...
  public List<ComboItem> autocompleteExpedientes(final HttpServletRequest request,
      @RequestParam(value = "allExp") final boolean allExp,
      @RequestParam(value = "uniAct") final boolean uniAct,
      @RequestParam(value = "term") final String codigo) {
    List<ComboItem> listaReturn = new ArrayList<ComboItem>();
  ...
  public int addElementsComboExpedient(final boolean allExp, final String codigo,
      List<ComboItem> listaReturn, List<ObjetoInsideExpedienteUnidad> expedientes) {
    int i = 0;
    for (ObjetoInsideExpedienteUnidad expedienteUnidad : expedientes) {
      if (isCandidateForCombo(allExp, codigo, i, expedienteUnidad)) {
        i++;
        listaReturn.add(new ComboItem(expedienteUnidad.getIdentificador(),
            expedienteUnidad.getIdentificador(), null));
  ...
  public boolean isCandidateForCombo(final boolean allExp, final String codigo, int i,
    ...
    return like(expedienteUnidad.getIdentificador().toLowerCase(), codigo.toLowerCase())
        && i < MAX_RESULTS
        && (allExp ? true
  ...
  private boolean like(String toBeCompare, String by) {
    if (by != null && toBeCompare != null) {
      return InsideUtils.remplazaCaracteres(toBeCompare)
          .contains(InsideUtils.remplazaCaracteres(by));
    } else {
      return false;
    }
