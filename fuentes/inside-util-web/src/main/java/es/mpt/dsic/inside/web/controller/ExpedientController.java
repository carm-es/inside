...
public class ExpedientController {
	...
	public ModelAndView generarExpediente(Locale locale, HttpSession session, HttpServletRequest request) {
		...
		tipoExpedienteMA.getExpediente().getMetadatosExp().setIdentificador(generateDefaultId(request, false));

		modelAndView.addObject("expedienteMetadatos", tipoExpedienteMA);

		modelAndView.addObject(INDICE, convertIndex(tipoExpedienteMA.getExpediente().getIndice().getIndiceContenido()
		...
		ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
	...
	public Map<String, Object> comprobarIdentificadorExpedientePost(
		...
		try {
			...
			if (StringUtils.isBlank(identificadorExpediente)) {
				mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, "Identificador vacio o blanco no válido");
	...
	@ResponseBody
	public Map<String, Object> generarExpedientePost(@ModelAttribute TipoExpedienteInsideConMAdicionales tipoExpedienteMA,
			Locale locale, HttpServletRequest request, @RequestParam("contenidoExp") String contenidoExp) {
		...
		try {
			...
			logger.warn("ExpedientController.generarExpedientePost --> Error al generar expediente: " + e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, mensaje);
		} catch (Exception e) {
			if (!retorno.containsKey(MENSAJE_USU)) {
				logger.error("ExpedientController.generarExpedientePost --> Error al generar expediente: " + e);
	...
	public ObjetoExpedienteInside generateExpedient(TipoExpedienteInsideConMAdicionales tipoExpedienteMA, Locale locale,
			HttpServletRequest request, String contenidoExp, Map<String, Object> retorno) throws InsideServiceInternalException {
		...
		try {
			...
			if (tipoExpedienteMA.getExpediente().getIndice().getIndiceContenido()
					...
					|| CollectionUtils.isEmpty(tipoExpedienteMA.getExpediente().getIndice().getIndiceContenido()
							.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada())) {
				generateValidateMessage(locale, retorno, new InsideServiceInternalException(
			...
			if ("server".equalsIgnoreCase(request.getParameter("firmaExp"))) {
				...
				ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession()
						.getAttribute(WebConstants.USUARIO_SESSION);
				...
				firmaIndice = insideSigner.firmarFicheroWhitPropertie(firmaIndice, null, "XAdES Enveloped", "IMPLICIT",
						idNodeToSign, credential);
				ObjetoAuditoriaFirmaServidor objetoAuditoriaFirmaServidor = new ObjetoAuditoriaFirmaServidor(
						Constantes.ELEMENTO_EXPEDIENTE, tipoExpedienteMA.getExpediente().getMetadatosExp().getIdentificador(),
						usuario.getNif());
				insideService.saveAuditoriaFirmaServidor(objetoAuditoriaFirmaServidor);
			}
			tipoExpedienteMA.getExpediente().getIndice().setFirmas(signatureUtils.setSignaturesExp(firmaIndice,
					tipoExpedienteMA.getExpediente().getMetadatosExp().getIdentificador()));

			ObjetoExpedienteInside expedienteInside = InsideConverterExpediente
					.expedienteEniToInside(tipoExpedienteMA.getExpediente());
			...
			if (tipoExpedienteMA.getMetadatosAdicionales() != null) {
				...
				this.eliminaMetadatosAdicionalesVacios(tipoExpedienteMA.getMetadatosAdicionales().getMetadatoAdicional());
			...
			expedienteInside = insideService.obtenerVisualizacionIndiceSiActivo(expedienteInside, false, false, null, false, null,
					true);
			tipoExpedienteMA.getExpediente().setVisualizacionIndice(
					InsideConverterDocumento.Contenido.contenidoInsideToEni(expedienteInside.getVisualizacionIndice(), null));

			String data = incluirAdicionales(tipoExpedienteMA, retorno);

			retorno.put("visualizar",
					Base64.encodeBase64String(tipoExpedienteMA.getExpediente().getVisualizacionIndice().getValorBinario()));
			...
			logger.warn("ExpedientController.generarExpedientePost --> Error en las validaciones del expediente: " + e1);
			...
			logger.warn("ExpedientController.generarExpedientePost --> Error al generar expediente: " + e3);
			throw new InsideServiceInternalException(e3.getMessage());
		}
	}

	private void cleanSpecialCaractersInFolders(List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> list) {
		for (ObjetoExpedienteInsideIndiceContenidoElementoIndizado elem : list) {
			if (elem instanceof ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) {
				((ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) elem)
						.setIdentificadorCarpeta(InsideUtils.remplazaCaracteres(
								((ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) elem).getIdentificadorCarpeta()));
				...
				cleanSpecialCaractersInFolders(
						((ObjetoExpedienteInsideIndiceContenidoElementoContenedorElementos) elem).getElementosIndizados());
			}
		}
	}
	...
	private void generateValidateMessage(Locale locale, Map<String, Object> retorno, InSideServiceException e) {
		MessageObject mensaje;
		String mensajeException = e.getMessage() != null ? e.getMessage() : "";
		if (esIndiceIncompleto(locale, mensajeException)) {
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
					messageSource.getMessage(WebConstants.MSG_VALIDACION_EXP_INDICE_INCOMPLETO_MENSAJE, null, locale));
		} else if (esIndiceConCarpetaIncompleta(locale, mensajeException)) {
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
					messageSource.getMessage(WebConstants.MSG_VALIDACION_EXP_CARPETA_INCOMPLETA_MENSAJE, null, locale));
		} else {
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
	...
	private boolean esIndiceIncompleto(Locale locale, String mensajeException) {
		return mensajeException
				.contains(messageSource.getMessage(WebConstants.MSG_VALIDACION_EXP_INDICE_INCOMPLETO, null, locale));
	}

	private boolean esIndiceConCarpetaIncompleta(Locale locale, String mensajeException) {
		return mensajeException
				.contains(messageSource.getMessage(WebConstants.MSG_VALIDACION_EXP_CARPETA_INCOMPLETA, null, locale));
	}

	private List<ObjetoInsideMetadatoAdicional> metadatosAdicionalesToInside(TipoMetadatosAdicionales metadatosAdicionales) {
	...
	public String incluirAdicionales(TipoExpedienteInsideConMAdicionales tipoExpedienteMA, Map<String, Object> retorno)
			throws InSideServiceException, JAXBException {
	...
	public ModelAndView descargarExpedienteEni(@RequestParam("contenido") String contenido,
			@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response, Locale locale,
		...
		try {
			...
			response.addHeader("Content-Disposition", "attachment; filename=\"" + identificador + ".xml\"");
	...
	public ModelAndView descargarExpedienteEniValidarFirma(@RequestParam("contenidoValidarFirma") String contenidoValidarFirma,
			@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response, Locale locale,
			@RequestParam("viewName") String viewName) {
		try {
	...
	private byte[] transformarExpedienteDescargaCompletoParaValidarFirma(TipoExpediente tExpAdi, String contenido)
			throws InsideServiceInternalException {
		...
		try {
			...
			expedienteDespuesTransformacion = XMLUtils.deFirmaBase64_A_DSSignature(data.getBytes("UTF-8"));
		} catch (Exception e) {
	...
	public ModelAndView descargarExpedienteEniCompleto(HttpSession session, @RequestParam("contenido") String contenido,
			@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response, Locale locale,
			@RequestParam("viewName") String viewName) {
		try {
			TipoExpediente tipoExpediente = getExpedient(contenido.getBytes(), identificador);
			Map<String, String> indice = insideService.obtenerDocsExpTipo(
					tipoExpediente.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
					"");
			...
			byte[] expedienteENIListoParaValidar = transformarExpedienteDescargaCompletoParaValidarFirma(tipoExpediente,
					contenido);
			...
			response.addHeader("Content-Disposition", "attachment; filename=\"" + identificador + ".zip\"");
	...
	public ModelAndView descargarExpedienteEniCompletoFisico(HttpSession session, @RequestParam("contenido") String contenido,
			@RequestParam(IDENTIFICADOR) String identificador, HttpServletResponse response, Locale locale,
			@RequestParam("viewName") String viewName) {
		try {
			TipoExpediente tipoExpediente = getExpedient(contenido.getBytes(), identificador);
	...
	public static byte[] generateZipFicherosFisicos(Map<String, byte[]> ficheros, JAXBMarshaller marshaller,
			InSideService insideService) throws Exception {
		...
		while (it.hasNext()) {
			...
			ObjetoDocumentoInsideContenido contenido = insideService
					.getDocumentoContenido(docEni.getMetadatos().getIdentificador(), null);
	...
	/**
	 * Devuelve en un Map los ficheros que componen un índice. Se tienen en
	 * cuenta las carpetas según parámetro conCarpetas. Se tiene en cuenta si
	 * contienen metadatos. Los ficheros del índice se buscan en primer lugar en
	 * la carpeta temporal (session). Si no se encuentran se obtienen de la base
	 * de datos.
	 */
	private Map<String, byte[]> getMapFicherosIndice(Map<String, String> indice, boolean conCarpetas, HttpSession session)
			throws InsideServiceInternalException {
	...
	@ResponseBody
	public Map<String, Object> validarExpedientePost(Locale locale, HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			expedienteValidacion.setOpcionesValidacionExpediente(getOpcionesValidacion(opcionesValidacion));
			...
			logger.warn("ExpedientController.validarExpedientePost --> Error interno generar expediente", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, mensaje);
		} catch (Exception e) {
	...
	private void validaLongitudIdentificador(TipoExpedienteValidacionInside expedienteValidacion, byte[] data)
			throws InsideConverterException, InSideServiceException, InsideWSException {
	...
	public TipoOpcionesValidacionExpedienteInside getOpcionesValidacion(List<String> opcionesValidacion) {
	...
	private TipoResultadoValidacionExpedienteInside validarExpediente(TipoExpedienteValidacionInside expedienteValidacion,
			byte[] data) throws InSideServiceException {
		expedienteValidacion.setContenido(data);
	...
	public ModelAndView visualizarExpedientePost(@RequestParam("visualizar") String visualizar,
		...
		try {
			...
			if ((identificador == null) || (identificador.length() <= 0)) {
				identificador = messageSource.getMessage(WebConstants.MSG_KEY_DOC_VISUALIZAR_NOMBRE_DOCENI, null, locale);
			}
			OutputStream pr = response.getOutputStream();
			response.setContentType("application/pdf");
			response.addHeader("Content-Disposition", "attachment; filename=\"" + identificador + ".pdf\"");
	...
	private void init(ModelAndView modelAndView, Locale locale) {
		...
		try {
			...
			logger.error("ExpedientController.init --> Error al convertir la fecha apertura expediente", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
		...
		modelAndView.addObject("estadosExpediente", ComboUtils.getEstadosExpediente(messageSource, locale));
	}
	...
	private void setData(TipoExpediente tipoExpediente, HttpServletRequest request)
		...
		tipoExpediente.getMetadatosExp()
				.setFechaAperturaExpediente(InsideWSUtils.stringToXmlCalendar(fechaApertura, WebConstants.FORMATO_FECHA_DEFECTO));
		...
		tipoExpediente.getMetadatosExp().getOrgano().addAll(ComboUtils.getOrganosKeys(organosData, request));

		List<String> interesados = new ArrayList<String>(tipoExpediente.getMetadatosExp().getInteresado());
		tipoExpediente.getMetadatosExp().getInteresado().clear();
		tipoExpediente.getMetadatosExp().getInteresado().addAll(ComboUtils.getInteresadosKeys(interesados, request));
	...
	public Map<String, Object> crearCarpetaIndice(HttpServletRequest request, final ModelMap model) {
		...
		try {
			...
			TipoExpediente expedienteEniResultante = getExpedient(request.getParameter(EXPEDIENT_ENI).getBytes(), idExpediente);
	...
	@ResponseBody
	public Map<String, Object> crearEstructuraCarpetaIndice(HttpServletRequest request, Locale locale) {
		logger.debug("Iniciado ExpedientController.crearCarpetaIndice");
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject mensaje = null;
		try {
			...
			if (usuario != null) {
				...
				TipoExpediente expedienteEniResultante = getExpedient(request.getParameter(EXPEDIENT_ENI).getBytes(),
						idExpediente);
				ObjetoEstructuraCarpetaInside estructuraCarpeta = insideService.getEstructuraCarpeta(usuario);
				TipoIndiceContenido tipoIndice = InsideConverterIndice.estructuraCarpetasInsideToEni(estructuraCarpeta);
				String path = request.getParameter("path");

				boolean correctMove = checkMoveElementNoRepeatInLevel(expedienteEniResultante, estructuraCarpeta, path);

				if (correctMove) {
					retorno = incluirEstructuraCarpetaEnIndiceExpediente(retorno, expedienteEniResultante, tipoIndice, path);
				} else {
					mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
	...
	public Map<String, Object> incluirEstructuraCarpetaEnIndiceExpediente(Map<String, Object> retorno,
			...
			throws InsideServiceInternalException {
		try {
			TipoExpediente expedienteConEstructuraCarpeta = IndexActions.createFolderComplete(expedienteEniResultante, tipoIndice,
					path);
			...
			throw new InsideServiceInternalException("Error en incluirEstructuraCarpetaEnIndiceExpediente", e);
		}
	}
	...
	public boolean checkMoveElementNoRepeatInLevel(TipoExpediente expedienteEniResultante,
		...
		for (ObjetoExpedienteInsideIndiceContenidoElementoIndizado elemIndizado : estructuraCarpeta.getElementosIndizados()) {
			correctMove = IndexActions.checkMoveElementNoRepeatInLevel(expedienteEniResultante,
					((ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada) elemIndizado).getIdentificadorCarpeta(), path);
			if (!correctMove)
				break;
		}
		return correctMove;
	...
	@ResponseBody
	public Map<String, Object> crearDocumentoIndice(HttpServletRequest request, Locale locale, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject mensaje = null;
		try {
			...
			if (id != null) {
				...
				temporalDataBusinessService.escribir(data, session.getId(), id + WebConstants.FORMAT_XML, true);
			}
			...
			if (resultValidateIdDocument) {
				...
				XMLGregorianCalendar fechaIncorpExpXML = InsideWSUtils.stringToXmlCalendar(fechaIncorpExp,
						WebConstants.FORMATO_FECHA_DEFECTO2);

				expedienteEniResultante = IndexActions.createDocument(expedienteEniResultante, path, id, valorHuella,
						funcionResumen, fechaIncorpExpXML, ordenDocExp);

				retorno.put(ID, id);
				retorno.put(NOMBRE_NATURAL_EXP, getNombreNatural(docInside.getMetadatos().getMetadatosAdicionales()));
				retorno = setDataIndex(expedienteEniResultante, retorno);
	...
	@ResponseBody
	public Map<String, Object> createSubExpedient(HttpSession session, HttpServletRequest request, Locale locale) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		...
		try {
			...
			if (checkIdentifierExpedient(session, locale, retorno, identifier))
				retorno = includeSubExpedient(session, locale, retorno, expedienteEniResultante, identifier);
		} catch (Exception e) {
	...
	private boolean checkIdentifierExpedient(HttpSession session, Locale locale, Map<String, Object> retorno, String identifier)
		...
		try {
			...
			if (insideService.permisoObjetoUnidad(exp, usuario.getNif(), true)) {
				...
				msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
						messageSource.getMessage(WebConstants.MSG_IMPORTAR_DOC_USUARIO_NO_ASOCIADO_EXP, null, locale));
				retorno.put(MENSAJE_USU, msg);
			}
	...
	@ResponseBody
	public Map<String, Object> createSubExpedientToken(HttpSession session, HttpServletRequest request, Locale locale) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject msg = null;
		try {
			...
			if (StringUtils.isNotBlank(identifierExpedient) && StringUtils.isNotBlank(csv) && StringUtils.isNotBlank(uuid)) {

				String nifConsultaPuestaDisposicion = getUsuarioConsultaPuestaDisposicion(session);
				Map<String, Object> result = insideUtilService.checkToken(nifConsultaPuestaDisposicion, identifierExpedient, csv,
						uuid);
				...
				if (token != null && !tokenCaducado && !tokenInvalidoNif) {
					retorno = includeSubExpedient(session, locale, retorno, expedienteEniResultante, identifierExpedient);
				} else {
					String errorToken = (String) result.get("errorToken");
					msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, messageSource.getMessage(errorToken, null, locale));
					retorno.put(MENSAJE_USU, msg);
				}
			} else {
				msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
						messageSource.getMessage(WebConstants.MSG_EXP_PUESTADISPOSICION_NO_VALIDO, null, locale));
				retorno.put(MENSAJE_USU, msg);
			}
		} catch (Exception e) {
			logger.error("ExpedientController.createSubExpedientToken --> Error al crear subExpediente", e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, msg);
		}
		return retorno;
	...
	public Map<String, Object> includeSubExpedient(HttpSession session, Locale locale, Map<String, Object> retorno,
			TipoExpediente expedienteEniResultante, String identifierExpedient) {
		...
		try {
			...
			List<String> listRepeatIds = insideService.checkRepeatDocsExpTipo(
					expedienteEniResultante.getIndice().getIndiceContenido()
							.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
					tExp.getIndice().getIndiceContenido().getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada());
			...
			logger.error("ExpedientController.crearVistaCerradaExpediente --> Error al guardar expediente", e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, msg);
		}
		return resultado;
	...
	@ResponseBody
	public Map<String, Object> createDocumentInside(HttpSession session, HttpServletRequest request, Locale locale) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject msg = null;
		...
		try {
			...
			if (checkIdentifierDocument(session, locale, retorno, idDocumento)) {
				...
				expedienteEniResultante = IndexActions.createDocument(expedienteEniResultante, path, idDocumento, valorHuella,
						funcionResumen, fechaIncorpExpXML, ordenDocExp);
			...
			logger.warn("ExpedientController.createDocumentInside --> Error al asociar Documento Inside: " + idDocumento, e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, msg);
		} catch (Exception e) {
			logger.error("ExpedientController.createDocumentInside --> Error al asociar Documento Inside: " + idDocumento, e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, msg);
		}
		return retorno;
	...
	public String getValorHuella(ObjetoDocumentoInside docInside) {
		...
		{//si devuelve la referencia. Hay que ir a la referencia a buscar el contenido para hacer el hash, no hacer el hash de la cadena de la referencia
			...
			ContenidoFirmaCertificadoContenidoBinarioInside firmaBase64 = (ContenidoFirmaCertificadoContenidoBinarioInside)docInside.getFirmas().get(0).getContenidoFirma();
	...
	private String getNombreNatural(List<ObjetoInsideMetadatoAdicional> list) {
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (ObjetoInsideMetadatoAdicional adicional : list) {
				if (MetadatosEEMGDE.METADATO_NOMBRE_NOMBRE_NATURAL.getValue().equals(adicional.getNombre())) {
	...
	private boolean checkIdentifierDocument(HttpSession session, Locale locale, Map<String, Object> retorno, String id)
			throws InSideServiceException {
		MessageObject msg = null;
		...
		try {
			...
			if (insideService.permisoObjetoUnidad(doc, usuario.getNif(), true)) {
				...
				msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
						messageSource.getMessage(WebConstants.MSG_IMPORTAR_DOC_USUARIO_NO_ASOCIADO_DOC, null, locale));
				retorno.put(MENSAJE_USU, msg);
			}
		} catch (InsideStoreObjectNotFoundException e) {
	...
	public void validateElementsCrearDocumentoIndice(HttpServletRequest request, Locale locale)
		...
		if ((StringUtils.isBlank(request.getParameter("fileName")) && StringUtils.isBlank(request.getParameter(ID)))
	...
	private boolean validateIdDocument(HttpSession session, Locale locale, String id, Map<String, Object> retorno) {
		MessageObject mensaje;
	...
	private Map<String, Object> setDataIndex(TipoExpediente expedienteEniResultante, Map<String, Object> map)
			throws JsonParseException, IOException, JAXBException, ParserConfigurationException, SAXException,
			InsideConverterException, InSideServiceException, ServiceException, DatatypeConfigurationException {
		...
		if (expedienteEniResultante.getMetadatosExp() != null)
			expedienteEniResultante.getIndice().getIndiceContenido()
					.setId("EXP_INDICE_CONTENIDO" + expedienteEniResultante.getMetadatosExp().getIdentificador());
	...
	@ResponseBody
	public Map<String, Object> moverElementoIndice(HttpServletRequest request, final ModelMap model, HttpSession session,
			Locale locale) {
		...
		try {
			...
			boolean correctMove = pathOrigen.equals(pathDestino) ? true
					: IndexActions.checkMoveElementNoRepeatInLevel(expedienteEniResultante, identificador, pathDestino);

			if (correctMove) {
				expedienteEniResultante = IndexActions.moveElement(expedienteEniResultante, pathOrigen, identificador,
						pathDestino, false, orden, session);
			} else {
				mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
						messageSource.getMessage(WebConstants.MSG_GENERAR_EXP_CARPETAS_MISMO_ID, null, locale));
				retorno.put(MENSAJE_USU, mensaje);
	...
	@ResponseBody
	public Map<String, Object> borrarElementoIndice(HttpServletRequest request, final ModelMap model, HttpSession session) {
		...
		try {
			...
			expedienteEniResultante = getExpedient(request.getParameter(EXPEDIENT_ENI).getBytes(), idExpediente);
			expedienteEniResultante = IndexActions.moveElement(expedienteEniResultante, pathOrigen, identificador, null, true,
					null, session);

			retorno = setDataIndex(expedienteEniResultante, retorno);
		} catch (Exception e) {
			logger.error("ExpedientController.borrarElementoIndice --> Exp: " + expedienteEniResultante != null
					? expedienteEniResultante.getMetadatosExp().getIdentificador()
					: "" + ". identificador: " + identificador + ". pathOrigen: ", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			model.addAttribute(MENSAJE_USU, mensaje);
		}
	...
	@ResponseBody
	public Map<String, Object> renombrarElementoIndice(HttpServletRequest request, final ModelMap model, Locale locale) {
		...
		try {
			...
			boolean correctRename = IndexActions.checkNewNameNoRepeatInLevel(expedienteEniResultante,
					request.getParameter("path"), request.getParameter("actualName"), request.getParameter("newName"));

			if (correctRename) {
				expedienteEniResultante = IndexActions.renameElement(expedienteEniResultante, request.getParameter("path"),
	...
	private TipoExpediente getExpedient(byte[] data, String idExpedienteInput) throws InsideServiceInternalException {
	...
	private void validateData(TipoExpediente tipoExpediente, Locale locale) throws InsideValidationDataException {
		...
		if (!StringUtils.isEmpty(error)) {
			...
			args[0] = error;
			throw new InsideValidationDataException(
					messageSource.getMessage(WebConstants.KEY_ERROR_DATOS_NO_VALIDOS_PARAMETERS, args, locale));
		}
	}
		...
		if (data != null) {
			for (Object element : data) {
				if (element instanceof TipoDocumentoIndizado) {
					...
					List<Object> dataConvert = convertIndex(
							((TipoCarpetaIndizada) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
							initializeDate);
					((TipoCarpetaIndizada) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().clear();
					((TipoCarpetaIndizada) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada()
							.addAll(dataConvert);
					...
					List<Object> dataConvert = convertIndex(
							((TipoIndiceContenido) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(),
							initializeDate);
					((TipoIndiceContenido) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada().clear();
					((TipoIndiceContenido) element).getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada()
							.addAll(dataConvert);
					retorno.add(element);
				}
			...
			if (element.getIdentificadorCarpeta().contains("_EXP_")) {
				...
				if (expInside != null) {
					...
					natural = natural != null ? natural
							: element.getIdentificadorCarpeta().substring(13, element.getIdentificadorCarpeta().length());
	...
	private void includeNombreNaturalDocumento(TipoDocumentoIndizado element) {
		...
		if (element != null && element.getIdentificadorDocumento() != null) {
			...
			if (docInside != null) {
				...
				natural = natural != null ? natural
						: element.getIdentificadorDocumento().substring(13, element.getIdentificadorDocumento().length());
				element.setId(natural);
			}
		}
	}
	...
	public String validarDocumentoEni(String session, String documento, String format) throws IOException {
		String retorno = null;
	...
	public ModelAndView expedientesAlmacenados(HttpSession session,
		...
		ModelAndView retorno = new ModelAndView(VIEW_EXPEDIENTS_STORED);
		MessageObject mensaje;
		if (StringUtils.isNotEmpty(textoMensajeUsuario) && StringUtils.isNotEmpty(nivelMensajeUsuario)) {
			retorno.addObject(MENSAJE_USU, new MessageObject(Integer.parseInt(nivelMensajeUsuario), textoMensajeUsuario));
		}
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
			...
			logger.error("ExpedientController.expedientesAlmacenados --> Error al obtener expedientes del usuario", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
			retorno.addObject(EXPEDIENTES, new ArrayList<ObjetoInsideExpedienteUnidad>());
			retorno.addObject(EXPEDIENTES_ALMACENADOS, false);
	...
	public ModelAndView cargarVistasExpediente(HttpSession session, @RequestParam(IDENTIFICADOR) String identificador,
		...
		try {
			...
			logger.error("ExpedientController.cargarVistasExpediente --> Error al obtener las vistas del expediente", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
		}
		return retorno;
	...
	public ModelAndView volverExpedienteAsociado(Locale locale, HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			if (lastElement != null) {
				ObjetoExpedienteInside exp = getExpedienteInside((String) lastElement.getKey(), (String) lastElement.getValue());
			...
			logger.error("ExpedientController.volverExpedienteAsociado --> Error al volver al expediente asociado", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
		}
		return retorno;
	...
	public ModelAndView volverVistasAsociadas(HttpSession session) {
		...
		try {
			...
			logger.error("ExpedientController.volverExpedienteAsociado --> Error al obtener expedientes del usuario", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, mensaje);
		}
		return retorno;
	...
	public void getViewsExpedient(ModelAndView retorno, String identificador) throws InSideServiceException {
	...
	private Object checkBreadCrumbExpedients(HttpSession session) {
		LinkedHashMap<String, String> listExpedients = (LinkedHashMap<String, String>) session.getAttribute(RASTRO_EXPEDIENTE);
	...
	@SuppressWarnings("unchecked")
	public Entry<String, String> recoverBreadCrumbExpedient(HttpSession session, boolean deleteBreadCrumbExpedient) {
		LinkedHashMap<String, String> listExpedients = (LinkedHashMap<String, String>) session.getAttribute(RASTRO_EXPEDIENTE);
	...
	public Map<String, Object> visualizarContenido(Locale locale, HttpSession session,
		...
		try {
			...
			try {
				...
				byte[] data = temporalDataBusinessService.getFile(session.getId(), idDocumento + WebConstants.FORMAT_XML);
			...
			try {
				retorno.put("contenidoVisualizar", serviceVisualizar.visualizarContenidoOriginal(documento));
			...
			retorno.put("desEstado",
					messageSource.getMessage(
							"estadoElaboracion." + tDoc.getMetadatos().getEstadoElaboracion().getValorEstadoElaboracion().value(),
							null, locale));
			...
			logger.error("ExpedientController.informacionVisualizarContenido --> Error al obtener información de visualizar", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, mensaje);
		} catch (Exception e) {
			logger.error("ExpedientController.informacionVisualizarContenido --> Error al obtener información de visualizar", e);
	...
	@SuppressWarnings("unchecked")
	private TipoDocumento convertirDocAEni(byte[] data) throws ParserConfigurationException, SAXException, IOException,
			JAXBException, TransformerFactoryConfigurationError, TransformerException {
		...
		try {
			...
			retorno = ((JAXBElement<TipoDocumento>) unmarshaller.unmarshal(new ByteArrayInputStream(data))).getValue();
			...
			if (nodoAdicionales != null) {
				...
				JAXBContext jc = JAXBContext.newInstance(TipoDocumento.class.getPackage().getName());
				Unmarshaller unmarshaller = jc.createUnmarshaller();
				retorno = ((JAXBElement<TipoDocumento>) unmarshaller.unmarshal(new ByteArrayInputStream(nodoEniString)))
						.getValue();
	...
	@ResponseBody
	public Map<String, Object> importExpedientPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject mensaje = null;
		...
		try {
			...
			List<ObjetoDocumentoInside> listDocuments = getListNewDocumentsRequest(request, session, retorno, LIST_IDS_DOCUMENTO);
	...
	@ResponseBody
	public Map<String, Object> importExpedienteFormatoSIPPost(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject mensaje = null;
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
			ObjetoExpedienteInside objetoExpedienteInside = null;
			List<ObjetoDocumentoInside> listaObjetoDocumentoInside = new ArrayList<ObjetoDocumentoInside>();
			List<String> listaIdsDocumentoInside = new ArrayList<String>();
			Map<String, byte[]> ficheros = this.insideUtilService.unzip(session.getId(), request.getParameter(ID_EXPEDIENT));
			for (Map.Entry<String, byte[]> entry : ficheros.entrySet()) {
				if (entry.getKey().contains("EXP")) {
					...
					if (!insideUtilService.esLongitudIdentificadorValido(objetoExpedienteInside.getIdentificador())) {
						throw new InsideWSException(InsideWsErrors.IDENTIFICADOR_DOCUMENTO_LONGITUD, null, "");
					}
				} else {
					ObjetoDocumentoInside objetoDocumentoInside = insideUtilService.validateDocumentImport(entry.getValue());
			...
			if (resultado != null) {
				...
				aditonal[0] = org.springframework.util.StringUtils.collectionToCommaDelimitedString(resultado);
				mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
						messageSource.getMessage(WebConstants.MSG_IMPORTAR_EXP_CORRESP_NO_VALIDA, aditonal, locale));
				retorno.put(MENSAJE_USU, mensaje);
			} else {
				retorno = createExpedientAndDocument(usuario, objetoExpedienteInside, listaObjetoDocumentoInside);
			}

			if (!retorno.containsKey(MENSAJE_USU))
				getUserMessage(locale, retorno, WebConstants.MSG_IMPORTAR_EXP_OK);
		} catch (InsideWSException e) {
			logger.error("ExpedientController.importarExpedienteFormatoSIP --> Error al importar expediente", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
					messageSource.getMessage(WebConstants.MSG_IMPORTAR_EXP_ID_NO_VALIDO, null, locale));
			retorno.put(MENSAJE_USU, mensaje);
		} catch (InsideServiceInternalException e) {
			logger.warn("ExpedientController.importarExpedienteFormatoSIP --> Error al importar expediente", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, mensaje);
		} catch (Exception e) {
			if (!retorno.containsKey(MENSAJE_USU)) {
	...
	@ResponseBody
	public Map<String, Object> saveExpedientPost(@ModelAttribute TipoExpedienteInsideConMAdicionales tipoExpedienteMA,
			Locale locale, HttpServletRequest request, HttpSession session, @RequestParam("contenidoExp") String contenidoExp) {
		Map<String, Object> retorno = new HashMap<String, Object>();

		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) request.getSession().getAttribute(WebConstants.USUARIO_SESSION);
			ObjetoExpedienteInside objExp = generateExpedient(tipoExpedienteMA, locale, request, contenidoExp, retorno);
			...
			if (checKCorrecpondenciaExpDocument(locale, session, retorno, objExp, DOCS, null)) {
				List<ObjetoDocumentoInside> newDocuments = getListNewDocumentsRequest(request, session, retorno, DOCS);
			...
			logger.warn("ExpedientController.saveExpedient --> Error validaciones al guardar expediente", e);
	...
	@ResponseBody
	public Map<String, Object> updateExpedientPost(@ModelAttribute TipoExpedienteInsideConMAdicionales tipoExpedienteMA,
			Locale locale, HttpServletRequest request, HttpSession session, @RequestParam("contenidoExp") String contenidoExp) {
		Map<String, Object> retorno = new HashMap<String, Object>();
	...
	private ObjetoExpedienteInside getExpedientRequest(Locale locale, HttpServletRequest request, HttpSession session,
			Map<String, Object> retorno, String element) throws InsideServiceInternalException {
		ObjetoExpedienteInside exp = null;
	...
	@SuppressWarnings("unchecked")
	public List<ObjetoDocumentoInside> getListNewDocumentsRequest(HttpServletRequest request, HttpSession session,
			Map<String, Object> retorno, String element) throws InSideServiceException {
	...
	@SuppressWarnings("unchecked")
	private boolean checKCorrecpondenciaExpDocument(Locale locale, HttpSession session, Map<String, Object> retorno,
			ObjetoExpedienteInside objExp, String element, List<String> listIdDocImports) throws IOException {
		MessageObject mensaje;
		...
		Collection<String> resultado = insideService.correspondencia(objExp, cleanExtension(listIdDocs));
	...
	public void getUserMessage(Locale locale, Map<String, Object> retorno, String msg) {
		...
		if (retorno.get(DOCS_NO_CREATE) != null) {
			...
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_SUCCESS, messageSource.getMessage(msg, null, locale));
			retorno.put(MENSAJE_USU, mensaje);
		}
	}

	private Map<String, Object> createExpedientAndDocument(ObjetoInsideUsuario usuario, ObjetoExpedienteInside objExp,
			List<ObjetoDocumentoInside> documentos) throws TransformerFactoryConfigurationError, InSideServiceException {
		Map<String, Object> retorno = new HashMap<String, Object>();

		MessageObject mensaje;
		try {
			Map<String, Object> result = insideUtilService.crearNuevoExpediente(objExp, documentos, usuario.getNif(), true);
	...
	private Map<String, Object> updateExpedientAndDocument(ObjetoInsideUsuario usuario, ObjetoExpedienteInside objExp,
			List<ObjetoDocumentoInside> documentos) throws TransformerFactoryConfigurationError, InSideServiceException {
		Map<String, Object> retorno = new HashMap<String, Object>();

		MessageObject mensaje;
		try {
			Map<String, Object> result = insideUtilService.actualizarExpediente(objExp, documentos, usuario.getNif(), true);
			retorno.put(DOCS_NO_CREATE, result.get(DOCS_NO_CREATE));
		} catch (Exception e) {
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			throw new InsideServiceInternalException(mensaje.getMessage(), e);
	...
	public Map<String, Object> getVersionesExpediente(HttpServletRequest request) {
		...
		try {
			List<ObjetoInsideVersion> versiones = insideService.getListaVersionesExpediente(identificadorExp);
	...
	@ResponseBody
	public Map<String, Object> expedienteGenerarTokenDescarga(Locale locale, HttpServletRequest request, HttpSession session) {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject msg = null;
		try {
			ObjetoInsideUsuario usuario = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
			...
			try {
				...
				ObjetoExpedienteToken objetoExpedienteToken = new ObjetoExpedienteToken(usuario, identificadorExp, csv, uuid,
						fechaCaducidad, dir3DesdeModal, asuntoDesdeModal, nifsDesdeModal, mailEnvioToken);
				...
				if (mailEnvioToken != null && !"".equals(mailEnvioToken.trim())) {
					try {
						...
						logger.error("expedienteGenerarTokenDescarga: Error en el envio de correo de Credenciales de Acceso" + e);
					}
				} else {
				...
				logger.error("Se he producido un error al generar el Credencial de Acceso de descarga:" + e.getMessage());
				msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
	...
	public ModelAndView verExpediente(Locale locale, HttpSession session, @RequestParam(IDENTIFICADOR) String identificador,
	...
	public ModelAndView editarExpediente(Locale locale, HttpSession session, HttpServletRequest request,
		...
		try {
			...
			logger.error("Se he producido un error al obtener el expediente almacenado: " + identificador + ". " + e);
	...
	public ModelAndView visualizaDescargarContenidoDocumento(@RequestParam(RESGUARDODOCBASE63) String resguardoDocBase64,
			@RequestParam("codigoEnvioResguardo") String codigoEnvioResguardo, HttpServletResponse response, Locale locale,
			HttpSession session) {
		try {
	...
	public Map<String, Object> crearVistaAbiertaExpediente(HttpServletRequest request) {
		...
		try {
			...
			String identifierView = mapper.readValue(request.getParameter("identifierView"), String.class);
			boolean firmaServidor = mapper.readValue(request.getParameter(SIGNATURE_SERVER), Boolean.class);

			ObjetoExpedienteInside exp = insideService.createOpenViewExpedient(identifier, identifierView, firmaServidor,
					usuario.getNif(), true);
			...
			logger.error("ExpedientController.crearVistaAbiertaExpediente --> Error al recuperar expediente", e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.put(MENSAJE_USU, msg);
		}
		return retorno;
	...
	public Map<String, Object> crearVistaCerradaExpediente(HttpServletRequest request) {
		...
		try {
			...
			ObjetoExpedienteInside exp = insideService.createClosedViewExpedient(identifier, identifierView, firmaServidor,
					usuario.getNif(), true);
	...
	@ResponseBody
	public Map<String, Object> updateIndexExp(Locale locale, HttpServletRequest request) throws InSideServiceException {
		Map<String, Object> retorno = new HashMap<String, Object>();
		MessageObject mensaje;
	...
	public void saveOrUpdateIndexExp(HttpServletRequest request, String identificadorExpediente, ObjetoInsideUsuario usuario)
			throws InsideServiceInternalException {
		ObjectMapper mapper = new ObjectMapper();
		...
		try {
			...
			if (isNewView != null && isNewView.booleanValue()) {
				String expedienteEniString = mapper.readValue(request.getParameter(EXPEDIENTE_ENI), String.class);
				TipoExpediente expedienteEni = marshaller.unmarshallDataExpedient(expedienteEniString.getBytes());
				expediente = InsideConverterExpediente.expedienteEniToInside(expedienteEni);

				expediente.getIndice().setFirmas(InsideConverterFirmas
						.firmasEniToInside(signatureUtils.setSignaturesExp(firmaIndice, expediente.getIdentificador())));
	...
	private ModelAndView loadData(ModelAndView model, ObjetoExpedienteInside exp, Locale locale, HttpSession session)
			throws InSideServiceException {
		...
		try {
			...
			retorno.addObject("listaRespuestaEnvioJusticiaLista", exp.getObjectInsideRespuestaEnvioJusticiaLista());
			...
			Map<String, String> docsExp = insideService.obtenerDocsExpTipo(tipoExpedienteMA.getExpediente().getIndice()
			...
			if (tipoExpedienteMA.getExpediente().getVisualizacionIndice() == null) {
				ObjetoExpedienteInside expedienteInside = insideService.obtenerVisualizacionIndiceSiActivo(exp, false, false,
						null, false, null, true);
				tipoExpedienteMA.getExpediente().setVisualizacionIndice(
						InsideConverterDocumento.Contenido.contenidoInsideToEni(expedienteInside.getVisualizacionIndice(), null));
			}

			retorno.addObject("visualizar",
					Base64.encodeBase64String(tipoExpedienteMA.getExpediente().getVisualizacionIndice().getValorBinario()));
			...
			expAuxiliar.getIndice().setIndiceContenido(tipoExpedienteMA.getExpediente().getIndice().getIndiceContenido());
			...
			retorno.addObject(INDICE, convertIndex(tipoExpedienteMA.getExpediente().getIndice().getIndiceContenido()
			...
			logger.error("ExpedientController.crearVistaAbiertaExpediente --> Error al guardar expediente", e);
			msg = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR, e.getMessage());
			retorno.addObject(MENSAJE_USU, msg);
	...
	private void cleanMetadatosEspeciales(ModelAndView retorno, TipoExpedienteInsideConMAdicionales tipoExpedienteMA) {
		...
		if (tipoExpedienteMA.getMetadatosAdicionales() != null
			...
			for (MetadatoAdicional adicional : tipoExpedienteMA.getMetadatosAdicionales().getMetadatoAdicional()) {
				...
				} else if (MetadatosEEMGDE.METADATO_FECHAS_FECHA_FIN.getValue().equals(adicional.getNombre())) {
			...
			tipoExpedienteMA.getMetadatosAdicionales().getMetadatoAdicional().addAll(finalListAdicionales);
		}
	}

	private boolean checkAssociatedViewsExpedient(String identificadorExpediente) throws InSideServiceException {
		return insideService.checkAssociatedViewsExpedient(identificadorExpediente);
	}

	public ObjetoExpedienteInside getExpedienteInside(String identificador, String version) throws InSideServiceException {
	...
	private GregorianCalendar getFechaApertura(TipoExpedienteInsideConMAdicionales tipoExpedienteMA) {
		...
		if (tipoExpedienteMA != null && tipoExpedienteMA.getExpediente() != null
				&& tipoExpedienteMA.getExpediente().getMetadatosExp() != null
				&& tipoExpedienteMA.getExpediente().getMetadatosExp().getFechaAperturaExpediente() != null) {
			fecha = tipoExpedienteMA.getExpediente().getMetadatosExp().getFechaAperturaExpediente().toGregorianCalendar();
		}
		return fecha;
	}
	...
	public ModelAndView puestaDisposicionPost(Locale locale, HttpSession session,
			@RequestParam("tokenIdentificador") String identificador, @RequestParam("tokenCsv") String csv,
			@RequestParam("tokenUuid") String uuid, @RequestParam("navegador") String navegador) {
		...
		try {
			...
			if (StringUtils.isNotBlank(identificador) && StringUtils.isNotBlank(csv) && StringUtils.isNotBlank(uuid)) {
				String nifConsultaPuestaDisposicion = getUsuarioConsultaPuestaDisposicion(session);
				
				Map<String, Object> result = insideUtilService.checkToken(nifConsultaPuestaDisposicion, identificador, csv, uuid);
	...
	public void auditAccessWithToken(HttpSession session, ObjetoExpedienteToken token) {
		...
		try {
			ObjetoInsideUsuario user = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
			inSideAuditoriaService.auditoriaRemisionNube(user, token, session.getId());
		} catch (Exception e) {
			logger.error("Error en la puestaDisposicion: Fallo al auditar el acceso en auditoriaRemisionNube", e);
		}
	}

	@RequestMapping(value = "/borrarExpediente", method = RequestMethod.POST)
	public ModelAndView borrarExpedientePost(HttpSession session, @RequestParam(IDENTIFICADOR) String identificador,
			Locale locale) {
		ModelAndView retorno = null;
		MessageObject mensaje = null;
		try {
			...
			retorno = expedientesAlmacenados(session, null, null);
			logger.error(
					"ExpedienteController.borrarExpedientePost --> Error al borrar los expediente del usuario -" + identificador,
					e);
			Object[] args = new String[1];
			args[0] = e.getMessage();
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
	...
	private void initDocument(ModelAndView retorno, Locale locale, HttpServletRequest request) {
		...
		try {
			...
			logger.error("DocumentController.generarDocumento --> Error al convertir la fecha captura documento", e);
			mensaje = new MessageObject(WebConstants.MESSAGE_LEVEL_ERROR,
					messageSource.getMessage(WebConstants.MSG_KEY_EXPED_ERROR_FECHA_INICIO, null, locale));
			retorno.addObject(MENSAJE_USU, mensaje);
		}
		...
		retorno.addObject("estadosElaboracion", ComboUtils.getEstadosElaboracion(messageSource, locale));
		...
		TipoDocumentoConversionInside.MetadatosEni metadatos = new TipoDocumentoConversionInside.MetadatosEni();
	...
	private String getUsuarioConsultaPuestaDisposicion(HttpSession session) {
		...
		ObjetoInsideUsuario usuarioLogado = (ObjetoInsideUsuario) session.getAttribute(WebConstants.USUARIO_SESSION);
	...
	private void eliminaMetadatosAdicionalesVacios(List<MetadatoAdicional> listaMetadatosAdicionales) {
		for (Iterator<MetadatoAdicional> iterator = listaMetadatosAdicionales.iterator(); iterator.hasNext();) {
