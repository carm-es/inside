...
public class UploaderController {
  ...
  @ResponseBody
  public String uploadTempData(@RequestBody MultipartFile file, @RequestParam String name,
      @RequestParam(required = false, defaultValue = "-1") int chunks,
      @RequestParam(required = false, defaultValue = "-1") int chunk, HttpSession session)
  ...
  @ResponseBody
  public Map<String, String> uploadTempDataString(HttpServletRequest request, HttpSession session,
    ...
    try {
      ...
      String fileUploaded = temporalDataBusinessService.escribir(data.getBytes(), folder,
          "tempBase64_" + chunk + ".txt", true);

      if (chunk == chunks - 1) {
        StringBuilder strBuilder = new StringBuilder();
        for (int position = 0; position < chunks; position++) {
          strBuilder.append(temporalDataBusinessService.getFileContent(folder,
              "tempBase64_" + position + ".txt"));
        }
  ...
  @ResponseBody
  public String getDataTemp(@RequestParam String name, HttpSession session)
      throws ServiceException {
    String retorno = null;
    try {
      retorno =
