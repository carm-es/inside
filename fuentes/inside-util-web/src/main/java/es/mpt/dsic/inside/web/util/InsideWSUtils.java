...
public class InsideWSUtils {
	...
	public static XMLGregorianCalendar stringToXmlCalendar(String fecha) throws ParseException, DatatypeConfigurationException {
	...
	public static XMLGregorianCalendar stringToXmlCalendar(String fecha, String format) throws ParseException,
	...
	public static void setExpedienteSinFirmaAsValid(TipoResultadoValidacionExpedienteInside expedientiEniFile) {
		ArrayList<TipoResultadoValidacionDetalleExpedienteInside> results = (ArrayList<TipoResultadoValidacionDetalleExpedienteInside>) expedientiEniFile
		...
		for (int i = 0; ((i < results.size()) && (!encontrado)); i++) {
			...
			if (tipoResult.getTipoValidacion().value().equals(TipoOpcionValidacionExpediente.TOVE_01.value())
					&& !tipoResult.isResultadoValidacion()
					&& (tipoResult.getDetalleValidacion().contains(WebConstants.MSG_ERROR_VALIDACION_EXPE_SIN_FIRMA) || tipoResult
							.getDetalleValidacion().contains(WebConstants.MSG_ERROR_VALIDACION_EXPE_SIN_FIRMA_ADITIONAL))) {
	...
	public static boolean todayBeforDate(Date today, Date dateCaducidad){
		if(dateCaducidad == null)
			return false;
		else return today.before(dateCaducidad);
	}
	...
	public static boolean containsString1ToString2( String string1, String string2 ) {
		  if(string1==null || string1.trim().equals("") || string2==null || string2.trim().equals(""))
			  return false;
		  else return string1.toLowerCase().contains( string2.toLowerCase() );
		}
