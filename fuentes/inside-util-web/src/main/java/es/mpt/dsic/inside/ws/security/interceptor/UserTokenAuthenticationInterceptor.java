		...
		try {
			...
			for (WSHandlerResult wshr : results) {
				for (WSSecurityEngineResult wsser : wshr.getResults()) {
					...
					ObjetoInsideAplicacion app = insideUsersService.getAplicacion(principal.getName());
					if (app == null) {
						securityContext.setAuthentication(null);
						throw new SoapFault("Credenciales Erroneas. compruebe identificador y password", message.getVersion()
								.getSender());
					} else {
						Authentication authentication = new UsernamePasswordAuthenticationToken(app.getAplicacion(),
								app.getPasswordHash());
			...
			securityContext.setAuthentication(null);
			throw new SoapFault("Credenciales Erroneas. compruebe identificador y password", e, message.getVersion().getSender());
