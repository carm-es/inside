...
public class MiniappletLoginFilter extends AbstractAuthenticationProcessingFilter {
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!StringUtils.isBlank(signature)) {
				...
				ResultadoValidarCertificado validarCertificado = infoFirmaService.getInfoCertificate(certificado);
	...
	private UserAuthentication validarUsuarioDatosTablas(HttpServletRequest request, List<GrantedAuthority> roles, ResultadoValidarCertificado validarCertificado) throws InsideUsersServiceException 
	{
...
//			} 
	...
	private UserAuthentication validarActualizandoCONDatosPortales(HttpServletRequest request, List<GrantedAuthority> roles, ResultadoValidarCertificado validarCertificado) throws InSideServiceException {
		
		List<UnidadOrganicaRolPortales> portalesDir3 = insideService.getlistaUnidadesRolesPortales(validarCertificado.getIdUsuario());
		if (portalesDir3!=null) 
		{
			// Actualiza los datos de las tablas con los datos recogidos de Portales
			actualizarUsuarioConDatosUnidadesRolesPortales(portalesDir3, validarCertificado.getIdUsuario());
			return validarUsuarioDatosTablas(request, roles, validarCertificado);
		}
		else
		{
