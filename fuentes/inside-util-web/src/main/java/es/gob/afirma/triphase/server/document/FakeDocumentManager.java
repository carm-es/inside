...
public final class FakeDocumentManager implements DocumentManager {
  ...
  public byte[] getDocument(final String id, final X509Certificate[] certChain,
      final Properties config, HttpSession session) throws IOException {
  ...
  public String storeDocument(final String id, final X509Certificate[] certChain, final byte[] data,
    ...
    try {
      ...
      LOGGER.severe(
          "Error al almacenar los datos en el fichero '" + tempFile.getAbsolutePath() + "': " + e); //$NON-NLS-1$ //$NON-NLS-2$
      if (fos != null) {
        try {
          fos.close();
