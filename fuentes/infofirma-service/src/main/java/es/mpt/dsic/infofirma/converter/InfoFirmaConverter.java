...
public class InfoFirmaConverter {
  ...
  private static InfoFirmante firmaInfoToInfoFirmante(FirmaInfo firmaInfo) {
    ...
    infoFirmante.setDescripcion(
        firmaInfo.getNombre() + " " + firmaInfo.getApellido1() + " " + firmaInfo.getApellido2());
   ...
   * @return objeto del modelo de infoFirmaService que represetna una lista de información de
   *         firmantes.
   */
   ...
   */
  public static InfoFirmaElectronica informacionFirmaToInfoFirmaElectronica(
    ...
    if (informacionFirma.getDocumentoFirmado() != null) {
      ...
      infoFirmaElectronica
          .setContenidoFirmado(informacionFirma.getDocumentoFirmado().getContenido());
    ...
    if (informacionFirma.getFirmantes() != null
        && informacionFirma.getFirmantes().getInformacionFirmas() != null
        && informacionFirma.getFirmantes().getInformacionFirmas().getInformacionFirmas() != null) {

      infoFirmaElectronica.setFirmantes(listFirmaInfoToListInfoFirmante(
          informacionFirma.getFirmantes().getInformacionFirmas().getInformacionFirmas()));
  ...
  public static OpcionesObtenerInformacionFirma OpcionesInfoFirmaToOpcionesObtenerInformacionFirma(
      OpcionesInfoFirma opcionesInfoFirma) {
    OpcionesObtenerInformacionFirma opcionesObtenerInformacionFirma =
        new OpcionesObtenerInformacionFirma();
    opcionesObtenerInformacionFirma
        .setObtenerDatosFirmados(opcionesInfoFirma.isObtenerDatosFirmados());
