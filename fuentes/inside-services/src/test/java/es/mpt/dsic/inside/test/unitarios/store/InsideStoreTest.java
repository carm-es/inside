...
public class InsideStoreTest extends InSideServiceTestBase {
  ...
  public void storeExpediente()
    ...
    logger.info("Almacenado correctamente expediente con identificador "
        + expediente.getIdentificador() + ", expediente: " + expediente);
  }

  @Test
  public void updateExpediente() throws InsideServiceStoreException,
      InsideStoreObjectAlreadyExistsException, InsideStoreObjectNotFoundException {
    ObjetoExpedienteInside expediente = UtilsTest.createObjetoExpedienteInside(true, true, true);
    // expediente = store.storeObject(expediente);
    ObjetoExpedienteInside expediente_recuperado =
        getObjeto(ObjetoExpedienteInside.class, expediente.getIdentificador());
    ...
    ObjetoExpedienteInside expediente_actualizado =
        getObjeto(ObjetoExpedienteInside.class, expediente.getIdentificador());
  ...
  @Test
  public void testStoreExpedienteDuplicado() throws InsideServiceStoreException,
      InsideStoreObjectAlreadyExistsException, InsideStoreObjectNotFoundException {
    ObjetoExpedienteInside expediente = UtilsTest.createObjetoExpedienteInside(true, true, true);
    // store.storeObject(expediente);
    // try {
    // store.storeObject(expediente);
    // throw new InsideServiceStoreException("El store no ha fallado al insertar un objeto con un
    // identificador que ya existía");
    // } catch (InsideStoreObjectAlreadyExistsException e) {
    // logger.debug("Comprobado correctamente que no se inserta un expediente con un identificador
    // (" + expediente.getIdentificador() + ") que ya existe");
    // }
  }

  @Test
  public void storeDocumento() throws InsideServiceStoreException,
      InsideStoreObjectAlreadyExistsException, InsideStoreObjectNotFoundException {
    ...
    logger.info(
        "Almacenado correctamente documento con identificador " + documento.getIdentificador());
  }

  @Test
  public void updateDocumento() throws InsideServiceStoreException,
      InsideStoreObjectAlreadyExistsException, InsideStoreObjectNotFoundException {
    ObjetoDocumentoInside documento = UtilsTest.createObjetoDocumentoInside(true, true, true, true);
    // documento = store.storeObject(documento);
    logger.info(
        "Almacenado correctamente documento con identificador " + documento.getIdentificador());
    ObjetoDocumentoInside documento_recuperado =
        getObjeto(ObjetoDocumentoInside.class, documento.getIdentificador());
    UtilsTest.assertEqual(documento, documento_recuperado, false, false);
    documento_recuperado.getMetadatos()
        .setTipoDocumental(ObjetoDocumentoInsideMetadatosTipoDocumental.TD_13);
  ...
  private <T extends ObjetoInside<?>> T getObjeto(Class<T> tipo, final String identificador)
    ...
    String info = "Recuperado correctamente " + objeto_recuperado.getClass() + " con identificador "
        + objeto_recuperado.getIdentificador();
    if (objeto_recuperado instanceof ObjetoInsideVersionable) {
      ...
      info += " con version " + version.getVersion() + " actualizado en "
          + (version.getFechaVersion() != null ? version.getFechaVersion().getTime() : "vacio");
    }
    logger.info(info);
