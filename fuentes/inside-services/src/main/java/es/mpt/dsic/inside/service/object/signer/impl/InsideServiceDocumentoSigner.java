...
public class InsideServiceDocumentoSigner
	...
	@Override
	/**
	 * Firma un objeto de tipo ObjetoDocumentoInside. Aquí asumimos que ya
	 * tenemos el mime del contenido. Se firmarán los bytes del valorBinario del
	 * documento. Los bytes y el mime de la firma se colocarán en el atributo
	 * valorBinario del objeto FirmaInside.
	 * 
	 * @param documento
	 *            El documento a ser firmado
	 * @return El mismo documento con una firma añadida
	 * 
	 */
	public ObjetoDocumentoInside sign(ObjetoDocumentoInside documento, String usuario) throws InSideServiceException {

		if (documento.getContenido() == null) {
			throw new InsideServiceObjectSignerException("El contenido del documento no puede estar vacío");
		}
		...
		if (documento.getFirmas() == null || documento.getFirmas().isEmpty()
			...
			FirmaInsideTipoFirmaEnum eTipoFirma = FirmaInside.getTipoFirmaByMimeAFirmar(documento.getContenido().getMime());

			try {
				bytesFirma = signer.firmarFichero(bytesAFirmar, null, sTipoFirma, null, null);
				ObjetoAuditoriaFirmaServidor objetoAuditoriaFirmaServidor = new ObjetoAuditoriaFirmaServidor(Constantes.ELEMENTO_DOCUMENTO, documento.getIdentificador(), usuario);
				insideService.saveAuditoriaFirmaServidor(objetoAuditoriaFirmaServidor);
			} catch (InSideServiceSignerException ise) {
				String mensajeError = String.format("No se ha podido firmar el documento con identificador %s. %s",
			...
			if (eTipoFirma == FirmaInsideTipoFirmaEnum.TF_02 || eTipoFirma == FirmaInsideTipoFirmaEnum.TF_03) {
				...
				contenidoFirma.setReferenciaFirma("#" + documento.getContenido().getIdentificadorEnDocumento());

				nuevaFirma = new FirmaInside(eTipoFirma, contenidoFirma, "FIRMA_" + documento.getFirmas().size(),
						documento.getFirmas().size());

				// CAdES Detached y Attached
			} else if (eTipoFirma == FirmaInsideTipoFirmaEnum.TF_04 || eTipoFirma == FirmaInsideTipoFirmaEnum.TF_05) {
			...
			if (contenidoFirma instanceof ContenidoFirmaCertificadoContenidoBinarioInside) {
				ContenidoFirmaCertificadoContenidoBinarioInside contenidoBinario = (ContenidoFirmaCertificadoContenidoBinarioInside) contenidoFirma;
				...
				try {
					...
					String mensajeError = String.format(
							...
							documento.getMetadatos().getIdentificadorObjetoInside(), firmaInside.getIdentificadorEnDocumento(),
							ise.getMessage());
					throw new InsideServiceObjectSignerException(mensajeError, ise);
				}
				...
				ContenidoFirmaCertificadoReferenciaInside contenidoReferencia = new ContenidoFirmaCertificadoReferenciaInside();
				...
				throw new InsideServiceObjectSignerException("Todavía no soportamos ContenidoFirmaCertificadoDsSignatureInside");
				...
				String ref = ((ContenidoFirmaCertificadoReferenciaInside) contenidoFirma).getReferenciaFirma();
				// Quitamos el #
				...
				if (documento.getContenido().getIdentificadorEnDocumento().equalsIgnoreCase(id)) {
					...
					ContenidoFirmaCertificadoReferenciaInside contenidoReferencia = new ContenidoFirmaCertificadoReferenciaInside();
					contenidoReferencia.setReferenciaFirma("#" + documento.getContenido().getIdentificadorEnDocumento());
					// Añadimos un nuevo nodo de firma:
					// Mismo tipo que la firma que hemos refirmado.
					// Identificador y orden dependiendo del orden de la última
					// firma.
					...
					ContenidoFirmaCertificadoContenidoBinarioInside contenidoBinario = (ContenidoFirmaCertificadoContenidoBinarioInside) firmaInside
							.getContenidoFirma();
	...
	/**
	 * Obtiene la firma que tenga un determinado identificador. Null si no lo
	 * encuentra
	 * 
	 * @param firmas
	 *            Lista de firmas.
	 * @param id
	 *            Identificador de la firma que queremos buscar.
	...
	/**
	 * Devuelve la primera firma cuyo contenido sea instancia de
	 * ContenidoFirmaCertificadoInside
	 * 
	 * @param firmas
	 *            Lista de firmas.
	 * @return
	...
	private static FirmaInside getFirstFirmaInsideCertificado(List<FirmaInside> firmas) {
		...
		while (i < firmas.size() && firma == null) {
			if (firmas.get(i).getContenidoFirma() instanceof ContenidoFirmaCertificadoAlmacenableInside
					|| firmas.get(i).getContenidoFirma() instanceof ContenidoFirmaCertificadoReferenciaInside) {
				firma = firmas.get(i);
			}
			i++;
		}
	...
	/**
	 * Devuelve el tipo de firma con que habrá que invocar al servicio de firma
	 * dependiendo del tipo de firma Inside
	 * 
	 * @return
	 */
