...
public class InsideServiceStoreHibernateConverterExpediente
  ...
  @Override
  public ExpedienteInside toEntity(ObjetoExpedienteInside expediente, Session session)
    ...
    entity.setIdentificadorRepositorio(expediente.getIdentificadorRepositorio());
    if (expediente.getVersion() != null) {
      if (expediente.getVersion().getFechaVersion() != null) {
        entity.setFechaVersion(expediente.getVersion().getFechaVersion().getTime());
      }
    ...
    if (expediente.getMetadatos() != null) {
      ...
      entity.setClasificacion(expediente.getMetadatos().getClasificacion());
      if (expediente.getMetadatos().getFechaAperturaExpediente() != null) {
        entity.setFechaAperturaExpediente(
            expediente.getMetadatos().getFechaAperturaExpediente().getTime());
      }
    ...
    Set<ExpedienteInsideInteresado> expedienteInsideInteresados =
        new HashSet<ExpedienteInsideInteresado>();
    ...
    if (expediente.getIndice() != null) {
      if (expediente.getIndice().getFirmas() != null
        ...
        Set<ExpedienteInsideIndiceFirmas> expedienteInsideIndiceFirmas =
            new HashSet<ExpedienteInsideIndiceFirmas>();
        for (FirmaInside firma : expediente.getIndice().getFirmas()) {
          ...
          try {
            indiceFirma.setFirmaInside(InsideServiceStoreHibernateConverterFirma.toEntity(firma));
      ...
      ExpedienteInsideIndice entityIndice =
          InsideServiceStoreHibernateConverterExpedienteIndiceContenido
              .toEntity(expediente.getIndice().getIndiceContenido(), session);
    ...
    if (expediente.getMetadatos().getMetadatosAdicionales().size() > 0) {
      Set<ExpedienteInsideMetadatosAdicionales> expedienteInsideMetadatosAdicionaleses =
          new HashSet<ExpedienteInsideMetadatosAdicionales>();
      for (ObjetoInsideMetadatoAdicional metadatoAdicional : expediente.getMetadatos()
          .getMetadatosAdicionales()) {
        ExpedienteInsideMetadatosAdicionales metadatoAaditionaEntity =
            new ExpedienteInsideMetadatosAdicionales();
        ...
        expedienteInsideMetadatosAdicionaleses.add(metadatoAaditionaEntity);
      }
  ...
  @Override
  public ObjetoExpedienteInside toInside(ExpedienteInside entity, Session session)
      throws InsideServiceStoreException {
    ObjetoExpedienteInside expediente = new ObjetoExpedienteInside();

    expediente.setIdentificador(entity.getIdentificador());
    expediente.setIdentificadorRepositorio(entity.getIdentificadorRepositorio());
    ...
    if (entity.getFechaAperturaExpediente() != null) {
      ...
      fechaAperturaExpediente.setTime(entity.getFechaAperturaExpediente());
    }

    expediente.getMetadatos().setFechaAperturaExpediente(fechaAperturaExpediente);
    ...
    if (entity.getEstado() != null) {
      expediente.getMetadatos().setEstado(
          ObjetoExpedienteInsideMetadatosEnumeracionEstados.fromValue(entity.getEstado()));
    }
    expediente.getMetadatos().setIdentificadorObjetoInside(entity.getIdentificador());
    expediente.getMetadatos().setClasificacion(entity.getClasificacion());

    List<String> organosInside = new ArrayList<String>();
    for (ExpedienteInsideOrgano organo : entity.getExpedienteInsideOrganos()) {
    ...
    for (ExpedienteInsideInteresado interesado : entity.getExpedienteInsideInteresados()) {
    ...
    for (ExpedienteInsideIndiceFirmas entityFirma : entity.getExpedienteInsideIndiceFirmases()) {
      firmasInside
          .add(InsideServiceStoreHibernateConverterFirma.toInside(entityFirma.getFirmaInside()));
    }
    expediente.getIndice().setFirmas(firmasInside);

    if (entity.getExpedienteInsideIndice() != null) {
      expediente.getIndice()
          .setIndiceContenido(InsideServiceStoreHibernateConverterExpedienteIndiceContenido
  ...
  public List<ObjetoExpedienteInside> getExpedientesAsociados(String identificador, Session session)
    ...
    List<ExpedienteInsideIndice> objetos = (List<ExpedienteInsideIndice>) crit.list();
    ...
    for (ExpedienteInsideIndice entity : objetos) {
      ...
      if (entity.getExpedienteInsideIndice() != null) {
        ...
        retorno = getExpediente(entity.getExpedienteInsideIndice().getId(), session, retorno);

      } else {
        // si el documento esta vinculado a una carpeta dentro del
        // expediente
        retorno = getExpedienteCarpeta(entity.getExpedienteInsideIndiceCarpetaIndizada(), session,
            retorno);
      }
    }
  ...
  public Map<Integer, ObjetoExpedienteInside> getExpedienteCarpeta(
      ...
      Map<Integer, ObjetoExpedienteInside> expedientes) throws InsideServiceStoreException {
    Map<Integer, ObjetoExpedienteInside> retorno = expedientes;
    if (carpeta.getExpedienteInsideIndice() != null) {
      retorno = getExpediente(carpeta.getExpedienteInsideIndice().getId(), session, expedientes);
    } else {
      // buscamos hacia arriba en la estructura de carpetas del expediente
      Criteria critCarpeta = session.createCriteria(ExpedienteInsideIndiceCarpetaIndizada.class);
      critCarpeta
          .add(Restrictions.eq("id", carpeta.getExpedienteInsideIndiceCarpetaIndizada().getId()));
      ExpedienteInsideIndiceCarpetaIndizada carpetaPadre =
          (ExpedienteInsideIndiceCarpetaIndizada) critCarpeta.uniqueResult();
      retorno = getExpedienteCarpeta(carpetaPadre, session, expedientes);
    }
    return retorno;
  }

  public Map<Integer, ObjetoExpedienteInside> getExpediente(Integer idIndice, Session session,
      Map<Integer, ObjetoExpedienteInside> expedientes) throws InsideServiceStoreException {
    Map<Integer, ObjetoExpedienteInside> retorno = expedientes;

    // buscamos el expediente asociado al indice
    Criteria critExpIndice = session.createCriteria(ExpedienteInside.class);
    critExpIndice.add(Restrictions.eq("expedienteInsideIndice.id", idIndice));
    ExpedienteInside expedienteIndice = (ExpedienteInside) critExpIndice.uniqueResult();

    // buscamos la ultima version del expediente
    Criteria critExpLast = session.createCriteria(ExpedienteInside.class);
    critExpLast.add(Restrictions.eq("identificador", expedienteIndice.getIdentificador()));
    critExpLast.addOrder(Order.desc("version"));
    ExpedienteInside expendienteLastVersion =
        (ExpedienteInside) critExpLast.list().iterator().next();
