...
public class InsideMetadatoValidatorService {
	...
	public void validateClasificacion(Object clasificacion, Map<String, Object> dataDepends) throws InSideServiceValidationException {
		logger.debug("Inicio validateClasificacion:" + clasificacion.toString());
		Session session = sessionFactory.openSession();
		try {
			Integer anio = ((GregorianCalendar) dataDepends.get("fechaAperturaExpediente")).get(Calendar.YEAR);
			...
			if (!exists) {
				...
				if (dataSplit[dataSplit.length - 1].length() > 30) {
					throw new InSideServiceValidationException("El dato '" + clasificacion.toString() +"' el identificador supera 30 caracteres'");
				}
				...
				if (value == null) {
					throw new InSideServiceValidationException("Clasificación: El dato '" + data +"' no encuentra correspondencia en las unidades organicas del SIA ni cumple con la expresión regular '" + validatorString.getExpresionRegular() + "'");
				}
			}
		} catch (InSideServiceValidationException e) {
			throw new InSideServiceValidationException("El dato '" + clasificacion.toString() +"' no encuentra correspondencia en SIA ni cumple con la expresión regular '" + validatorString.getExpresionRegular() + "'");
		} finally {
			session.close();
		}
