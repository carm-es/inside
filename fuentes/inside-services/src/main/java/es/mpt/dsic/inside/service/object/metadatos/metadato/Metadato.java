...
public abstract class Metadato<T> {
  ...
  public void setValidator(MetatadatoValidator<T> validator)
    ...
    if (implementedType != validatorType) {
      throw new InsideServiceMetadatoValueExtractionException(
          "El tipo de datos que implementa el validador '" + validatorType
              + "' no es complatible con el tipo de datos asignado a este metadato '"
              + implementedType + "'");
    }
    this.validator = validator;
    logger.debug("Establecido validador " + validator.getClass() + " de tipo " + validatorType
        + " para metadato " + this.getNombre());
  }
  ...
  public T clean(Object value) throws InSideServiceMetadatoValidationException,
  ...
  protected void checkIfValueIsAceptableForMe(Object value)
      throws InSideServiceMetadatoValidationException,
      InsideServiceMetadatoValueExtractionException {
    Class<?> metadatoConcreteType = getConcreteType(this);
    if (!ClassUtils.isAssignableValue(metadatoConcreteType, value)) {
      throw new InSideServiceMetadatoValidationException(this,
          "Valor no válido para el parámetro " + this.getNombre(),
          new Error("El tipo del valor de la propiedad " + this.getNombre() + " ("
              + value.getClass() + " ) no es compatible con la definición del metadato ("
              + metadatoConcreteType + ")"));
    }
  }

  /**
   * Devuelve el Tipo T con el que se ha instanciado un objeto parametrizado
   * 
   * @return
   * @throws InsideServiceMetadatoValueExtractionException
   */
  public static Class<?> getConcreteType(Object objeto)
      throws InsideServiceMetadatoValueExtractionException {
    ...
    if (type instanceof ParameterizedType) {
      ...
      // objeto.getClass().getGenericSuperclass();
      type = ((ParameterizedType) type).getActualTypeArguments()[0];
    } else {
      throw new InsideServiceMetadatoValueExtractionException(
          "Error obteniendo el tipo concreto del Objeto : type no es un 'ParameterizedType' es un "
              + type.getClass());
    }
    ...
    if (type.getClass() != java.lang.Class.class) {
      throw new InsideServiceMetadatoValueExtractionException(
          "Error obteniendo el tipo concreto del Objeto : type no es un 'Class' es un "
              + type.getClass());
    }
    return (Class<?>) type;
  }
  ...
  public void setValidatorDataBase(InsideMetadatoValidatorDataBase validatorDataBase) {
