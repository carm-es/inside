...
public class InsideSemanticValidationServiceImpl implements InsideSemanticValidationService {
  ...
  @Override
  public void validarUnidadOrganica(ObjetoDocumentoInside documentoInside)
      throws InsideSemanticValidationException {
    String validate = validateOrganos(documentoInside.getMetadatos().getOrgano());
    if (!validate.equals("")) {
      throw new InsideSemanticValidationException(
          "Los datos: " + documentoInside.getMetadatos().getOrgano()
              + " no encuentran correspondencias en las unidades organicas del DIR3");
    }
  }

  @Override
  public void validarUnidadOrganica(ObjetoExpedienteInside expedienteInside)
      throws InsideSemanticValidationException {
    String validate = validateOrganos(expedienteInside.getMetadatos().getOrgano());
    if (!validate.equals("")) {
      throw new InsideSemanticValidationException(
          "Los datos: " + expedienteInside.getMetadatos().getOrgano()
              + " no encuentran correspondencias en las unidades organicas del DIR3");
    }
  }

  @Override
  public void validarClasificacion(ObjetoExpedienteInside expedienteInside)
      throws InsideSemanticValidationException {
    boolean validate = validateClasificacion(expedienteInside.getMetadatos().getClasificacion(),
        expedienteInside.getMetadatos().getFechaAperturaExpediente());
    if (!validate) {
      throw new InsideSemanticValidationException(
          "Los datos: " + expedienteInside.getMetadatos().getClasificacion()
              + " no encuentra correspondencia en SIA ni cumple con la expresión regular '"
              + clasificacionPattern + "'");
    }
  }
