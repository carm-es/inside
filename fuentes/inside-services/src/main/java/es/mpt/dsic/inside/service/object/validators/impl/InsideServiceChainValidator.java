...
/**
 * Un validador de objetos de Inside que se compone
 * de una serie de validaciones en serie
 * @author miguel.ortega
 *
 */
public class InsideServiceChainValidator<T extends ObjetoInside<K>, K extends ObjetoInsideMetadatos> implements InsideServiceObjectValidator<T,K>{
	...
	public  T validate(T objeto)throws InSideServiceException, InSideServiceValidationException
	{
		if(this.validadores == null){
			...
			for (InsideServiceObjectValidator<T,K> validador : this.validadores) {
				logger.debug("Validando objeto " + objeto.getClass() + " mediante validador " + validador.getClass());
