...
public class InsideServiceFoliadoImpl implements InsideServiceFoliado {
  ...
  public void configureFoliado() {
    ...
    if (!ACTIVO.contentEquals(foliadoActivo)) {
      ...
      logger.debug(String.format("Utilizando para foliado idaplicacion/password : %s/%s",
   ...
   * 
   * @Override public FoliadoInside foliar (String identificador, ObjetoExpedienteInsideMetadatos
   * metadatos, FoliadoNodo nodo) throws InsideServiceFoliadoException {
   * 
   * if (!activo) { throw new InsideServiceFoliadoException
   * ("El WS de FOLIADO para la obtención del foliado del expediente no está activo"); }
   * 
   * logger.debug ("Foliando expediente con identificador: " + identificador); Item item =
   * obtenerItem (nodo); Item item = null; try { item = obtenerItem (); } catch (Exception e) {
   * e.printStackTrace(); } Atributos atributos = obtenerAtributos (identificador, metadatos);
   * OpcionesFoliado opciones = obtenerOpcionesFoliado ();
   * 
   * SalidaFoliado salidaFoliado = null;
   * 
   * try { salidaFoliado = port.foliar(applicationLogin, atributos, item, opciones); } catch
   * (Exception e) { throw new InsideServiceFoliadoException
   * ("Se ha producido un error en la llamada al servicio de foliado " , e); }
   * 
  ...
  private Atributos obtenerAtributos(String identificador,
  ...
  private ListaPropiedades obtenerPropiedades(String identificador,
      ObjetoExpedienteInsideMetadatos metadatos) {
   ...
   * 
   * if (is instanceof ByteArrayInputStream) { size = is.available(); buf = new byte[size]; len =
   * is.read(buf, 0, size); } else { ByteArrayOutputStream bos = new ByteArrayOutputStream(); buf =
   * new byte[size]; while ((len = is.read(buf, 0, size)) != -1) bos.write(buf, 0, len); buf =
   * bos.toByteArray(); } return buf; }
   */
