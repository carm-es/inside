...
public class InsideServiceStoreHibernateConverterAplicacion {
	...
	protected static final Log logger = LogFactory.getLog(InsideServiceStoreHibernateConverterAplicacion.class);
	...
	public static InsideWsAplicacion toEntity(ObjetoInsideAplicacion objeto) {
		...
		if (CollectionUtils.isNotEmpty(objeto.getRoles())) {
			retorno.setAltaExpediente(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_ALTA_EXPEDIENTE));
			retorno.setAdministrarPermisos(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_ADMINISTRAR_PERMISOS));
			retorno.setAltaDocumento(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_ALTA_DOCUMENTO));
			retorno.setLeerDocumento(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_LEER_DOCUMENTO));
			retorno.setLeerExpediente(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_LEER_EXPEDIENTE));
			retorno.setModificarDocumento(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_MODIFICA_DOCUMENTO));
			retorno.setModificarExpediente(objeto.getRoles().contains(ObjetoInsideAplicacion.ROLE_MODIFICA_EXPEDIENTE));
		} else {
