...
public class InsideServiceStoreHibernateConverterObjetoInside <T extends ObjetoInside<?>, K extends Object> implements InsideServiceStoreHibernateConverter <T, K> {
	...
	public K toEntity(T objetoInside, Session session) throws InsideServiceStoreException{
		...
		if(objetoInside instanceof ObjetoExpedienteInside){
			return (K) (new InsideServiceStoreHibernateConverterExpediente()).toEntity((ObjetoExpedienteInside) objetoInside,session);
		}else if(objetoInside instanceof ObjetoDocumentoInside){
			return (K) (new InsideServiceStoreHibernateConverterDocumento()).toEntity((ObjetoDocumentoInside) objetoInside,session);
		}
	...
	public T toInside(K entity, Session session) throws InsideServiceStoreException{
		...
		if(entity instanceof ExpedienteInside){
			return (T) (new InsideServiceStoreHibernateConverterExpediente()).toInside((ExpedienteInside) entity,session);
		}else if(entity instanceof DocumentoInside){
			return (T) (new InsideServiceStoreHibernateConverterDocumento()).toInside((DocumentoInside) entity,session);
		}
