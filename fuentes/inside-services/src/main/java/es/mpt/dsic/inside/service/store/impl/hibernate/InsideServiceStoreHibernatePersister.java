...
public class InsideServiceStoreHibernatePersister<I extends ObjetoInside<?>, E extends Object> {
	...
	public <T extends ObjetoInside<?>> void persistExpedienteOrDocumento(T object, Object objectUnidad)
			throws InsideServiceStoreException {
		Session session = sessionFactory.openSession();

		try {
			if (object instanceof ObjetoExpedienteInside) {
				saveExpediente((ExpedienteInside) converter.toEntity((I) object, session), session, objectUnidad);
			} else if (object instanceof ObjetoDocumentoInside) {
				DocumentoInside docUpdate = recuperaDocumentoEntity(((ObjetoDocumentoInside) object).getIdentificador());
				saveDocumento((DocumentoInside) converter.toEntity((I) object, session), docUpdate, session, objectUnidad);
			} else {
				throw new InsideServiceStoreException("No se como almacenar objetos de tipo " + object.getClass());
			}
	...
	private void saveDocumento(DocumentoInside documentoEntity, DocumentoInside docUpdate, Session session, Object objectUnidad)
			throws InsideServiceStoreException {
	...
	public void saveObjetoUnidad(Object objectUnidad, Session session) throws InsideServiceStoreException {
		Transaction tx = session.beginTransaction();
		try {
			session.saveOrUpdate(objectUnidad);
			tx.commit();
	...
	private void updateInsideMetadatosAdicionales(DocumentoInside documentoEntity, DocumentoInside docUpdate, Session session) {
		for (DocumentoInsideMetadatosAdicionales insideMetadatosUpdate : docUpdate.getDocumentoInsideMetadatosAdicionaleses()) {
			boolean delete = true;
			for (DocumentoInsideMetadatosAdicionales insideMetadatos : documentoEntity
				...
				if (insideMetadatosUpdate.getNombre().equals(insideMetadatos.getNombre())
						...
						&& insideMetadatosUpdate.getDocumentoInside().getId().intValue() == insideMetadatos.getDocumentoInside()
								.getId().intValue()) {
	...
	private void updateInsideFirmas(DocumentoInside documentoEntity, DocumentoInside docUpdate, Session session) {
		for (DocumentoInsideFirmas insideFirmasUpdate : docUpdate.getDocumentoInsideFirmases()) {
			for (DocumentoInsideFirmas insideFirmas : documentoEntity.getDocumentoInsideFirmases()) {
				if (insideFirmasUpdate.getFirmaInside().getTipoFirma().equals(insideFirmas.getFirmaInside().getTipoFirma())
						&& insideFirmasUpdate.getDocumentoInside().getId().intValue() == insideFirmas.getDocumentoInside().getId()
								.intValue()
						&& insideFirmasUpdate.getFirmaInside().getOrden().intValue() == insideFirmas.getFirmaInside().getOrden()
								.intValue()) {
	...
	private void updateInsideOrgano(DocumentoInside documentoEntity, DocumentoInside docUpdate, Session session) {
		for (DocumentoInsideOrgano insideOrganoUpdate : docUpdate.getDocumentoInsideOrganos()) {
			...
			for (DocumentoInsideOrgano insideOrgano : documentoEntity.getDocumentoInsideOrganos()) {
				if (insideOrganoUpdate.getOrgano().equals(insideOrgano.getOrgano()) && insideOrganoUpdate.getDocumentoInside()
						.getId().intValue() == insideOrgano.getDocumentoInside().getId().intValue()) {
	...
	public void saveInsideMetadatosAdicionales(DocumentoInside documentoEntity, Session session) {
		for (DocumentoInsideMetadatosAdicionales metadato : documentoEntity.getDocumentoInsideMetadatosAdicionaleses()) {
	...
	private void saveExpediente(ExpedienteInside expedienteEntity, Session session, Object objectUnidad)
			throws InsideServiceStoreException {
		...
		try {
			...
			for (ExpedienteInsideIndiceFirmas docFirma : expedienteEntity.getExpedienteInsideIndiceFirmases()) {
				session.saveOrUpdate(docFirma.getFirmaInside());
				session.saveOrUpdate(docFirma);
			}
			for (ExpedienteInsideInteresado interesado : expedienteEntity.getExpedienteInsideInteresados()) {
				session.saveOrUpdate(interesado);
			}
			for (ExpedienteInsideOrgano organo : expedienteEntity.getExpedienteInsideOrganos()) {
				session.saveOrUpdate(organo);
			}
			for (ExpedienteInsideMetadatosAdicionales metadato : expedienteEntity.getExpedienteInsideMetadatosAdicionaleses()) {
				session.saveOrUpdate(metadato);
			}
	...
	private void saveIndiceExpediente(ExpedienteInsideIndice indiceExpedienteEntity, Session session)
		...
		try {
			...
			for (ExpedienteInsideIndice expedienteIndizado : indiceExpedienteEntity.getExpedienteInsideIndices()) {
			...
			throw new InsideServiceStoreException("Excepción almacenando Indice de Expediente en BBDD", e);
		}
	}

	private void saveCarpetaExpediente(ExpedienteInsideIndiceCarpetaIndizada carpetaEntity, Session session)
			throws InsideServiceStoreException {
		try {
			...
			for (ExpedienteInsideIndiceCarpetaIndizada carpeta : carpetaEntity.getExpedienteInsideIndiceCarpetaIndizadas()) {
				saveCarpetaExpediente(carpeta, session);
			}
	...
	public void setConverterAuditoriaToken(InsideServiceStoreHibernateConverterAuditoriaToken converterAuditoriaToken) {
	...
	public void deleteDocument(String identificador, Session session) throws InsideServiceStoreException {
	...
	public void deleteDocInsideMetadatosAdicionales(DocumentoInside documentoEntity, Session session) {
		for (DocumentoInsideMetadatosAdicionales metadato : documentoEntity.getDocumentoInsideMetadatosAdicionaleses()) {
	...
	public void deleteExpedient(String identificador, Session session) throws InsideServiceStoreException {
		...
		try {
			...
			NumeroProcedimiento numeroProcedimiento = expUni.getIdProcedimiento() != null
					? getNumeroProcesoEntity(expUni.getIdProcedimiento(), session) : null;
	...
	private void deleteExpInsideRespuestaEnvioJustica(ExpedienteInside expedienteEntity, UnidadOrganica unidadOrganica,
			NumeroProcedimiento numeroProcedimiento, Session session) {
		for (ExpedienteInsideRespuestaEnvioJusticia envio : expedienteEntity.getExpedienteInsideRespuestaEnvioJusticia()) {
	...
	public void deleteExpInsideMetadatosAdicionales(ExpedienteInside expedienteEntity, Session session) {
		for (ExpedienteInsideMetadatosAdicionales metadato : expedienteEntity.getExpedienteInsideMetadatosAdicionaleses()) {
			session.delete(metadato);
		}
	}
	...
	public void deleteExpInsideFirmas(ExpedienteInside expedienteEntity, Session session) {
		for (ExpedienteInsideIndiceFirmas expFirma : expedienteEntity.getExpedienteInsideIndiceFirmases()) {
	...
	private void deleteElementInsideIndice(ExpedienteInsideIndice indice, Session session) {
		for (ExpedienteInsideIndiceDocumentoIndizado docIndizado : indice.getExpedienteInsideIndiceDocumentoIndizados()) {
		...
		for (ExpedienteInsideIndiceCarpetaIndizada carpeta : indice.getExpedienteInsideIndiceCarpetaIndizadas()) {
			deleteCarpetaExpediente(carpeta, session);
		}
		session.delete(indice);
	}

	private void deleteCarpetaExpediente(ExpedienteInsideIndiceCarpetaIndizada carpeta, Session session) {
		for (ExpedienteInsideIndiceDocumentoIndizado docIndizado : carpeta.getExpedienteInsideIndiceDocumentoIndizados()) {
			session.delete(docIndizado);
		}
		for (ExpedienteInsideIndice expedienteIndizado : carpeta.getExpedienteInsideIndices()) {
			deleteElementInsideIndice(expedienteIndizado, session);
		}
		for (ExpedienteInsideIndiceCarpetaIndizada carpetaIn : carpeta.getExpedienteInsideIndiceCarpetaIndizadas()) {
			deleteCarpetaExpediente(carpetaIn, session);
		}
		session.delete(carpeta);
	}

	public void deleteObjetoUnidad(Object objeto, Session session) throws InsideServiceStoreException {
	...
	public void updateObjetoUnidad(Object objeto, Session session) throws InsideServiceStoreException {
	...
	public void saveToken(ObjetoExpedienteToken expedienteToken, Session session) throws InsideServiceStoreException {
	...
	public void saveComunicacionTokenExpediente(ComunicacionTokenExpediente comunicacionTokenExpediente, Session session)
			throws InsideServiceStoreException {
	...
	public void saveAuditoriaAccesoDocumento(AuditoriaAccesoDocumento auditoriaAccesoDocumento, Session session)
			throws InsideServiceStoreException {
	...
	public void saveSolicitudAccesoExpediente(SolicitudAccesoExpediente solicitudAccesoExpediente, Session session)
			throws InsideServiceStoreException {
	...
	public ObjetoInsideUsuario saveUser(ObjetoInsideUsuario usuario, UnidadOrganica unidad, Integer idProcedimiento,
	...
	private void actualizarUnidadUsuario(UsuarioInside data, Integer idUnidad, Integer idProcedimiento, Integer idRol) {
	...
	public void nuevaUnidadUsuario(UsuarioInside data, Integer idUnidad, Integer idProcedimiento, Integer idRol) {
	...
	public ObjetoInsideUsuario saveUser(UsuarioInside usuario, Session session) throws InsideServiceStoreException {
		Transaction tx = session.beginTransaction();
		try {
	...
	public ObjetoInsideAplicacion saveApp(ObjetoInsideAplicacion app, UnidadOrganica unidad, Integer idProcedimiento,
			Session session) throws InsideServiceStoreException {
	...
	private byte[] getDigest(ObjetoInsideAplicacion app) throws NoSuchAlgorithmException, UnsupportedEncodingException {
	...
	public ObjetoInsideAplicacion actualizarCredencialesEeetuilApp(String app, ObjetoCredencialEeutil credential, Session session)
			throws InsideServiceStoreException {
		...
		try {
			...
			if (data != null) {
				...
				CredencialesEeutil credentialEntity = (CredencialesEeutil) criteriaCredential.uniqueResult();
	...
	private void actualizarUnidadAplicacion(InsideWsAplicacion data, Integer idUnidad, Integer idProcedimiento) {
	...
	public void nuevaUnidadAplicacion(InsideWsAplicacion data, Integer idUnidad, Integer idProcedimiento) {
	...
	public ObjetoInsideAplicacion saveApp(InsideWsAplicacion app, Session session) throws InsideServiceStoreException {
		Transaction tx = session.beginTransaction();
		ObjetoInsideAplicacion retorno = null;
		try {
	...
	public void persistRespuestaRemisionJusticiaExpediente(ObjectInsideRespuestaEnvioJusticia objectInsideRespuestaEnvioJusticia,
			String identificadorExpediente, String idVersion) throws InsideServiceStoreException {

		ExpedienteInside expedienteEntity = recuperaExpedienteEntity(identificadorExpediente, idVersion);
		...
		respuestaEnvioJusticia
				.setCodigoUnidadOrganoRemitente(objectInsideRespuestaEnvioJusticia.getCodigoUnidadOrganoRemitente());
	...
	public void persistRespuestaRemisionJusticiaExpedienteNoInside(ObjectInsideRespuestaEnvioJusticia objectInsideRespuestaEnvioJusticia,
			String identificadorExpediente, String idVersion) throws InsideServiceStoreException {

		// convierto el objeto respuestajusticia a entity
		ExpedienteNoInsideRespuestaEnvioJusticia respuestaEnvioJusticia = new ExpedienteNoInsideRespuestaEnvioJusticia();
		...
		try {
			...
			tx.rollback();
			throw new InsideServiceStoreException("Excepción almacenando respuesta justicia expediente No Inside en BBDD", e);
		} finally {
			session.close();
		}
		...
		try {
			...
			if (objetos.isEmpty()) {
				throw new InsideServiceStoreException(
						"Excepcion recuperando entity de documento con identificador: " + identificadorDocumento);
			}
			doc = objetos.get(0);
	...
	public List<DocumentoInside> recuperaListDocumentoEntity(String identificadorDocumento, Session session) {
	...
	private ExpedienteInside recuperaExpedienteEntity(String identificadorExpediente, String idVersion)
			throws InsideServiceStoreException {
		Session session = sessionFactory.openSession();

		ExpedienteInside exp = null;
		try {
			List<ExpedienteInside> objetos = recuperaListExpedienteEntity(identificadorExpediente, idVersion, session);

			if (objetos.isEmpty()) {
				throw new InsideServiceStoreException("Excepcion recuperando entity de expediente con identificador: "
	...
	public List<ExpedienteInside> recuperaListExpedienteEntity(String identificadorExpediente, String idVersion,
			Session session) {
	...
	public ObjetoInsideUnidadAplicacionEeutil saveUnidadAplicacionEeutil(UnidadAplicacionEeutil unidadAplicacionEeutil,
			Session session) throws InsideServiceStoreException {
		...
		return InsideServiceStoreHibernateConverterUnidadAplicacionEeutil.toInside(unidadAplicacionEeutil, null);
	}
	...
	public EstructuraCarpetaInside saveEstructuraCarpeta(EstructuraCarpetaInside estructuraCarpeta, Session session)
			throws InsideServiceStoreException {
		Transaction tx = session.beginTransaction();
	...
	public void deleteEstructuraCarpeta(EstructuraCarpetaInside estructuraCarpeta, Session session)
		...
		Transaction tx = session.beginTransaction();
		try {
			deleteCarpetaExpediente(estructuraCarpeta.getExpedienteInsideIndiceCarpetaIndizada(), session);
	...
	public SolicitudAccesoExpAppUrl saveSolicitudAccesoExpAppUrl(SolicitudAccesoExpAppUrl dato, Session session)
			throws InsideServiceStoreException {
	...
	public GeneradorIdInside saveDefaultId(GeneradorIdInside idInside, Session session) throws InsideServiceStoreException {
		Transaction tx = session.beginTransaction();
		try {
