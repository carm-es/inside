 ...
 *
 */
public class InsideServiceValidatorExpedienteIndice<T extends ObjetoInside<K>, K extends ObjetoInsideMetadatos>
  ...
  public T validate(T objeto)
    ...
    if (objeto instanceof ObjetoExpedienteInside) {
      ...
      if (expediente.getIndice().getIndiceContenido().getFechaIndiceElectronico() == null) {
        expediente.getIndice().getIndiceContenido()
            .setFechaIndiceElectronico(GregorianCalendar.getInstance());
      }
      ...
      Collections.sort(elementosIndice,
      ...
      if (elementosIndice.size() > 0) {
        if (elementosIndice.get(0).getOrden() != 1) {
          throw new InsideServiceInternalException(
              "El orden del primer elemento del indice tiene un orden "
                  + elementosIndice.get(0).getOrden() + " distinto a 1");
        }
        ObjetoExpedienteInsideIndiceContenidoElementoIndizado ultimoElemento =
            elementosIndice.get(elementosIndice.size() - 1);
        if (ultimoElemento.getOrden() != elementosIndice.size()) {
          throw new InsideServiceInternalException("El último  elemento del indice tiene un orden "
              + ultimoElemento.getOrden() + " distinto a " + elementosIndice.size());
        }
      }
      ...
      throw new InsideServiceInternalException(
          "El objeto no es un expediente, es un " + objeto.getClass());
    }
  }
