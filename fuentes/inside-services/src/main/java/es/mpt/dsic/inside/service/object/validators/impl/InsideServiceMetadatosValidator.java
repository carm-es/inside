 ...
 *
 * Para la validación de los metadatos se busca en los metadatos y en los metadatos adicionales de
 * los objetos inside
 * 
 * @author miguel.ortega
 *
 */
public class InsideServiceMetadatosValidator<T extends ObjetoInside<K>, K extends ObjetoInsideMetadatos>
  ...
  public T validate(T objeto)
      throws InSideServiceException, InsideServiceObjectValidationException {
    logger.debug("Validando objeto " + objeto.getClass());

    List<InSideServiceMetadatoValidationException> validacionErrors =
        new ArrayList<InSideServiceMetadatoValidationException>();

    for (Metadato<?> metadato : this.metadatosDefinitions.values()) {
      logger
          .debug("Validando metadato " + metadato.getNombre() + " del objeto " + objeto.getClass());
      ...
      logger.debug("Validado correctamente el metadato " + metadato.getNombre() + " del objeto "
          + objeto.getClass());
  ...
  private void cleanMetadato(Metadato<?> metadato, ObjetoInside<?> objeto)
  ...
  private Object getPropertyValue(MetadatoPropertyDescriptor desc)
    ...
    if (readMethod == null) {
      throw new InsideServiceMetadatoValueExtractionException(
          "No se puede acceder en lectura al metadato " + desc.getMetadato().getNombre()
              + " en el objeto " + desc.getObjeto().getClass());
    }
    ...
    try {
      ...
      throw new InsideServiceMetadatoValueExtractionException(
          "Error leyendo metadato " + desc.getMetadato().getNombre() + " en el objeto "
              + desc.getObjeto().getClass() + " : " + e.getMessage(),
          e);
    }
  ...
  private void setPropertyValue(MetadatoPropertyDescriptor desc, Object value)
      throws InsideServiceMetadatoValueExtractionException {
    Method writeMethod = desc.getPropertyDescriptor().getWriteMethod();
    if (writeMethod == null) {
      throw new InsideServiceMetadatoValueExtractionException(
          "No se puede acceder en escritura al metadato " + desc.getMetadato().getNombre()
              + " en el objeto " + desc.getObjeto().getClass());
    }
    ...
    try {
      ...
      throw new InsideServiceMetadatoValueExtractionException(
          "Error estableciendo valor de metadato " + desc.getMetadato().getNombre()
              + " en el objeto " + desc.getObjeto().getClass() + " : " + e.getMessage(),
          e);
    }
  }

  private MetadatoPropertyDescriptor findMetadatoPropertyDescriptor(Metadato<?> metadato,
      ObjetoInside<?> objeto) throws InsideServiceMetadatoNotFoundInObjectException {
    ...
    if (pd != null) {
      logger.debug("Encontrado el metadato " + metadato.getNombre() + " en los metadatos de tipo '"
          + metadatos.getClass() + "' dentro del objeto " + objeto.getClass());
  ...
  private static PropertyDescriptor findMetadatoPropertyDescriptorInMetadatos(Metadato<?> metadato,
      Object metadatos) {
     ...
     * if(pd == null){ fieldName = InsideUtils.camelize(fieldName);
     * logger.debug("No existe el metadato " + metadato.getNombre() +
     * " en las propiedades del objeto " + metadatos.getClass() +
     * ", probaré con la versión camelizada: " + fieldName); pd =
     * BeanUtils.getPropertyDescriptor(metadatos.getClass(),fieldName); }
     * 
     * // Si tampoco se encuentra, se mirará si coincide ignorando mayúsculas y minúsculas if (pd ==
     * null) { PropertyDescriptor[] pds = BeanUtils.getPropertyDescriptors(metadatos.getClass());
     * boolean found = false; for (PropertyDescriptor propd : pds && !found) {
     * 
     * } }
     */

    if (pd != null) {
      logger.debug("Encontrado el metadato " + metadato.getNombre() + " el el campo " + fieldName
          + " dentro del objeto " + metadatos.getClass());
  ...
  /**
   * POJO para tener la referencia al objeto metadatos o metadatosAdicionales y el descriptor de la
   * propiedad sobre la que estamos trabajando al manipular un metadato en un objeto Inside
   * 
   * @author miguel.ortega
   *
   */
  ...
  private void validateDataBase(Metadato<?> metadato, Object dato)
    ...
    try {
      Method miMetodo = metadato.getValidatorDataBase().getClass()
          .getMethod(metadato.getValidatorDataBase().getMetodo(), Object.class);
      ...
      throw new InSideServiceMetadatoValidationException(metadato,
  ...
  private void validateService(Metadato<?> metadato, Object dato, Map<String, Object> datosAux)
      throws InSideServiceMetadatoValidationException {
    try {
      Method miMetodo = metadato.getValidatorService().getClass()
          .getMethod(metadato.getValidatorService().getMetodo(), Object.class, Map.class);
  ...
  private Map<String, Object> getMetadataDepends(Metadato<?> metadato, ObjetoInside<?> objeto)
      throws InsideServiceMetadatoValueExtractionException, InSideServiceValidationException {
    Map<String, Object> retorno = new HashMap<String, Object>();
    if (metadato.getDependsValidation() != null && !metadato.getDependsValidation().equals("")) {
