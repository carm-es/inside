...
public class CmisSessionManagerImpl implements CmisSessionManager {
	...
	/**
	 * Tiempo máximo (milisegundos) por defecto para intentar conectarse al
	 * servidor
	 */
	private static final int DEFAULT_CONNECT_TIMEOUT = 1 * 60 * 1000;

	/**
	 * Tiempo máximo (milisegundos) por defecto para que caduque la sesión del
		...
		if (!isSessionAlive()) {
			...
			try {
				...
				if (BindingType.ATOMPUB.value().equals(bindingType)) {
					...
					throw new RepositoryCmisException("El bindingType " + bindingType + " no está soportado ");
				}
	...
	private String getRepositoryId(final SessionFactory sessionFactory, final Map<String, String> parameters)
	...
	public Folder getRootFolder() throws RepositoryCmisException {
		...
		if (root == null) {
			...
			/* CARM ### v2.0.7.1
			El conector CMIS busca la carpeta server.cmis.pathRootFolder en la raíz del repositorio documental. Cuando no la encuentra, si se ha proporcionado el 
			property complementario server.cmis.complementary.root.defaultUserHomeSpacesFolder, la búsqueda se amplía a todas las carpetas que cuelguen de la raíz 
			y contengan el nombre de la raíz complementaria
			*/
			if (this.root==null && this.defaultUserHomeSpacesFolder!=null && !this.defaultUserHomeSpacesFolder.isEmpty()) {
				for (final CmisObject children : session.getRootFolder().getChildren()) {
