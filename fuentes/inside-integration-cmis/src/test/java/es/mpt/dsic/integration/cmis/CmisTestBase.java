...
public class CmisTestBase extends AbstractJUnit4SpringContextTests {
  ...
  public void init() throws RepositoryCmisException, URISyntaxException {
    ...
    if (rutaDocsPruebasDir == null) {
      File rutaDocsDir = new File(new File(new File(
          CmisTestBase.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath())
              .getParent()).getParent()
          + testDocsRelativePath);


      Assert.assertTrue("No existe la ruta de los ficheros de prueba " + rutaDocsDir,
          rutaDocsDir.exists());
      Assert.assertTrue("La ruta de los ficheros de prueba no es un directorio" + rutaDocsDir,
          rutaDocsDir.isDirectory());
      logger.info("Ruta de los documentos de prueba " + rutaDocsDir);

      File[] files = rutaDocsDir.listFiles();
      Assert.assertTrue(
          "La ruta de los ficheros de prueba no contiene ningún fichero" + rutaDocsDir,
          files.length > 0);
