...
});$mf.autocomplete = function(){
    var extraParam = function(params,formulario){
        var aux_param = {},
            key;
        for(key in params){
            if(params[key].substring(0, 9) == 'callback_'){
                if( typeof(params[key]) == 'function'){
                    aux_param[params[key]] = params[key]();
                }
            }
                ...
                aux_param[formulario+'['+params[key]+']'] = $( '#'+formulario+'_'+params[key] ).val();
            }
        }
        return aux_param;
    };
        ...
        this.opciones = {
            ...
            ajax: {
                ...
                data:  function (term, page) {
                    var values = []
                    $("."+data.autocomplete).children('input:not(:first)').each(function(key,elements){values.push($(elements).val())})
                    var aux = extraParam(data.params.split( /,\s*/ ),data.form);
            ...
            initSelection: function(element, callback) {
                    callback(window['values_'+data.autocomplete]());
            },
            multiple: true
        };
        if(!data.multiple){
            $.extend(this.opciones, {'maximumSelectionSize': 1});
        }
        if(data.template.length > 0){
            $.extend(this.opciones, {'formatResult': data.template, 'formatSelection': data.template});
        }
        $this.select2(this.opciones)
            .on('change', function(e){
                if(e.added != undefined){
                    if(!data.multiple){
                        $(this).siblings('input').each(function(k,v){
                            if($(v).val() == e.removed.id){
                                $(v).remove();
                            }
                        });
                        this.value = e.val;
                        ...
                        if($(this).val() != ''){
                            var $this = $(this);
                            $($this.val().split( /,\s*/)).each(function(i,a){
                                if(i == 0){
                                    $this.val(a);
                                }
                                else{
...
$mf.TopBar = function(){
    ...
    this.$el = $('#topBar');
    $userMenu.find('.show_all').each(mostrar_todos);
    var $selectProfile = $('#userMenu>.first>a>span');
    $selectProfile.each(function(i,el){
        var profileText = $.trim($(el).text());
        $(el).text(profileText.length > 25 ? profileText.substr(0,10)+'... ...'+profileText.substr(profileText.length-10,10) : profileText);
    });
}

$mf.Formulario = function(){
    var init = function(ind, el){
            init_del($(el));
            init_add($(el));
            $(el).find('ul').attr('data-index', $(el).find('ul').children().length);
        },
        init_del = function($el){
            $el.find('a.collection-delete').on('click', function(event){
                event.preventDefault();
                sup(event);
            });
        },
        init_add = function($el){
            $el.find('a.collection-add').on('click', function(event){
                event.preventDefault();
                add(event);
            });
        },
        ...
        add = function(event) {
            ...
            var prototype = $ul.data('prototype');
            var newForm = prototype.replace(/__name__/g, $ul.attr('data-index'));
            init_del($(newForm).appendTo($ul));
            $ul.attr('data-index', parseInt($ul.attr('data-index')) + 1);
        };
    $('.morfos-collection').each(init);
};


(function (global,$) {
    $mf.fixed_heads = function($contenedor, my_class, container_max_height){
        ...
        if(container_max_height == undefined)
            container_max_height = data && data.height != undefined ? data.height : $contenedor.find('table').outerHeight();
        ...
        var table = this,
            ...
            resize_height = function(container_max_height){
                ...
                if(table.left_head != undefined)$(table.left_head.$wrapper).css('height', container_max_height - aux_height);
            ...
            left_head = function(col_left){
                var sacar_colum  = function(num_col){
                        ...
                        if(num_col == undefined){
                            ...
                            $thead_aux.append($('<tr><th>'+(hasth != -1 ? $($ths[hasth]).html() : $($ths[0]).html())+'</th></tr>'));
                ...
                this.render = function(){
                    ...
                    this.$wrapper.css('height', container_max_height - (table.top_head != undefined ? table.top_head.$el.parent().outerHeight() : 0));
                }
        ...
        this.render = function(top,left,col_left){
            ...
            $(window).resize(function(e){
                resize_height(container_max_height-(window_height-$('body').height()));
            });
        }
...
$mf.DataGrid = function(){
    var heads = {},
        ...
        init_filter = function(key,value){
            ...
            $filter.find('.mf-datagrid--filter-content').height($table.parent('.mf-datagrid--table-container').outerHeight());
        ...
        pintar = function(data,$el){
            ...
            if($data.find('.morfos-autocomplete'))$mf.my_autocomplete = new $mf.autocomplete();
            init();
            $mf.timer.off();
        }
        init = function(){
            ...
            new $mf.fixed_heads($('table').parent());
        }
    init();
...
$mf.Dialog = function($dialogo, classes, $dialog_parent){
    ...
    this.appendDialog = function(datos,es_draggable,es_modal){
        ...
        var open_dialog =  function(texto){
            ...
            if(datos.title != undefined)self.$title.html(datos.title);
            if(datos.buttons != undefined)self.$buttons.html(datos.buttons).addClass('has-buttons').show();
            if(datos.is_min || self.$data.css('display') != 'none'){
                self.$min_button.show();
                self.$max_button.hide();
            }
            if(datos.clazz != undefined)self.$el.addClass(datos.clazz);
            self.$el.show();
        ...
        if(datos.url != undefined){
            $.post(datos.url, datos.data_url).always(function(text){
                open_dialog.call(self,text);
                if(datos.init_dialog != undefined){
                    datos.init_dialog();
                }
            });
        }
        else{
            open_dialog.call(self,datos.data != undefined ? datos.data : self.$data);
        }
    }
    function draggable(){
        this.$title_bar.on('mousedown', function(e){
            ...
            $('body').on('mousemove', function(e){
                if(e.clientX-init_x+pos_ini_x > 0 && e.clientX-init_x+pos_ini_x < $('body').width() - width){
                    $dialogo.css('left', e.clientX-init_x+pos_ini_x);
                }
                if(e.clientY-init_y+pos_ini_y > $('.mf-header--container').height() && e.clientY-init_y+pos_ini_y < $('body').height() - height){
                    $dialogo.css('top', e.clientY-init_y+pos_ini_y);
                }
    ...
    function modal(){
        $('.mf-veil').show().click(self.close_dialog);
        
    }
}

$mf.Marquesina = function(event){
    var $marquesinas = $('.mf-scroll'),
...
//        prev_px = min_max[0]> prev_px ? min_max[0] : min_max[1] < prev_px ? min_max[1] : prev_px;
    ...
    $marquesinas.each(init);
}

$mf.my_dialog = new $mf.Dialog($("div.mf-dialog").empty());
$mf.my_autocomplete = new $mf.autocomplete();
new $mf.Formulario();
new $mf.TopBar();
new $mf.DataGrid();
new $mf.Marquesina();
$mf.my_status = new $mf.Status();
if($('.mf-veil').length == 0)$('<div class="mf-veil"></div>').appendTo($('body')).hide();
$(document).ready(function(){
    $('.mf-dialog-confirm-nav').each(function(i,a){
        $(a).click(function(e){
            e.preventDefault();
    ...
    $('.mf-click-submit').click(function(){
        var $form = $(this).closest('form')
        $('<div class="mf-veil"><p><span></span>Cargando datos...</p></div>').appendTo($form);
        $form.submit()
    });
});
(function (global,$) {
...
//var help_items = React.createClass({
//    displayName: 'help_item',
//    render: function(){
//        var page = this.props.page,
//            create_help = function(helpObject,index){
//            var $helpObject = $(helpObject),
//                helpData = $helpObject.data(),
//                position = {};
//            $helpObject.addClass('mfhelp-item');
//            if(helpData.mfhelpPos) {
//                var top = function(){
//                        position = $mf.position($helpObject,$helpObject,'top','center',false,true,true);
//                    },
//                    right = function(){
//                        position = $mf.position($helpObject,$helpObject,'top','right',true,false,true);
//                    },
//                    bottom = function(){
//                        position = $mf.position($helpObject,$helpObject,'bottom','left',false,true,true);
//                    },
//                    left = function(){
//                        position = $mf.position($helpObject,$helpObject,'center','left',true,false,true);
//                    }
//                eval(helpData.mfhelpPos)();
//            }
//            else{
...
//                    key:"help-item-"+page+'-'+index,
//                    className:"mf-help-tip "+(helpData.mfhelpPos != undefined ? helpData.mfhelpPos : 'left'),
//                    style: _.extend(position,{'position':'absolute'}),
//                    id:"mf-help-item-"+index
//                },
//                React.DOM.h6({className:"mf-help-title"}, helpData.mfhelpTitle),
//                React.DOM.p({className:"mf-help-text"}, helpData.mfhelpText)
//            );
//        }
//        return React.DOM.div({id:"mf-help-overlay", key: 'help-overlay'+page}, _.map(this.props.helps,create_help));
//    }
//});
//var help_frame = React.createClass({
//    displayName: 'help_frame',
//    render: function(){
//        var page = this.props.page,
//            create_help = function(helpObject,index){
//            var $helpObject = $(helpObject),
//                offset = $helpObject.offset();
//            return React.DOM.div({
//                className:"mf-help-frame",
//                id:"mf-help--item-frame-" + index ,
//                key:"help-item-frame-"+page+'-'+index,
//                style: {
//                    'width': $helpObject.width() + 20,
//                    'height': $helpObject.height() + 20,
//                    'left': offset.left,
...
//            });
//        }
//        return React.DOM.div({id:"mf-help-frames", key: 'help-frames'+page}, _.map(this.props.helps,create_help));
//    }
//});
//
//var helps_navBar = React.createClass({
//    handleClose: function(){
//        $.cookie('show-help',0);
//        React.unmountComponentAtNode(document.getElementById('coverContainer'));
//    },
//    handlePrev: function(e){
//        e.preventDefault();
//        if(this.props.page != 0){
//            this.props.cargar_page(-1);
//        }
//    },
//    handleNext: function(e){
//        e.preventDefault();
//        if($('.mfhelp-page-'+(this.props.page+1)).length != 0){
//            this.props.cargar_page(1);
//        }
//    },
//    render: function() {
//        return React.DOM.div({id:"mf-help-navbar"},
//                React.DOM.a({
//                    id:"mf-help-prevbtn",
//                    className:"simbutton",
//                    onClick: this.handlePrev,
//                    style:{display:this.props.show_first}
//                },'Previa'),
//                React.DOM.a({
//                    id:"mf-help-nextbtn",
//                    className:"simbutton",
//                    onClick: this.handleNext,
//                    style:{display:this.props.show_last}
//                },'Siguiente'),
//                React.DOM.a({
//                    id:"mf-help-closebtn",
//                    className:"simbutton",
...
//                },'¡Muy bien, entendido!'),
//                React.DOM.span({},'o'),
//                React.DOM.a({},['Necesito saber más, ',React.DOM.u({},'quiero ir a la ayuda')])
//        );
//    }
//});
//
//var helps = React.createClass({
//    data_body: $('body').data(),
//    page: 0,
...
//            helps: (function(){
//                if($('[class*=mfhelp-page]').length != 0){
//                    return $('.mfhelp-page-0');
//                }
//                else{
//                    return $('[data-mfhelp-title]');
//                }
//            })(),
//            show_first:'none',
//            show_last:''
...
//        $('.mfhelp-item').removeClass('mfhelp-item');
//        var has_next = function(page){
//                return $('.mfhelp-page-'+(page+1)).length != 0 ? '' : 'none'
//            },
//            has_prev = function(page){
//                return page != 0 ? '' : 'none'
//            }
//
//        this.setState({
//            helps:$('.mfhelp-page-'+this.page),
//            show_first:has_prev(this.page),
//            show_last:has_next(this.page)
//        });
//    },
//    render: function() {
//        return React.DOM.div({id:"coverContainer"},
//            React.DOM.div({id:"mf-help-container"}),
//            help_items({helps: this.state.helps, page: this.page}),
//            help_frame({helps: this.state.helps, page: this.page}),
//            helps_navBar({
//                show_first: this.state.show_first,
//                show_last: this.state.show_last,
//                page: this.page,
//                cargar_page: this.cargar_page
//            })
//        );
//    }
//});
...
//$(document).ready( function () {
//    if($('body').hasClass('show_help') && ($.cookie('show-help') != undefined ? eval($.cookie('show-help')) : true)){
//       setTimeout(function(){React.renderComponent(helps(null), $('<div id="coverContainer"></div>').appendTo($('body'))[ 0 ])},100);
//    }
//});