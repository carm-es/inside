...
@WebService(endpointInterface = "es.mpt.dsic.inside.ws.service.AdminWebService", targetNamespace = "https://ssweb.seap.minhap.es/Inside/XSD/v1.0/WebService", portName = "InsideWSPort", serviceName = "AdminWS")
public class AdminWebServiceImpl implements AdminWebService {
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public RespuestaEnvioJusticia respuestaEnvioJusticia(String codigoEnvio) throws InsideWSException {
		try {
			ObjectInsideRespuestaEnvioJusticia aux = new ObjectInsideRespuestaEnvioJusticia();
			aux.setCodigoEnvio(codigoEnvio);

			ObjectInsideRespuestaEnvioJusticia dato = inSideService.getRespuestaEvioJusticaByCodigoEnvio(aux);
	...
	private RespuestaEnvioJusticia convertDataToWSRespuestaEnvioJusticia(ObjectInsideRespuestaEnvioJusticia respuestaObject) {
		...
		respuestaEnvioJusticia.setAuditoriaEsbMarcaTiempo(respuestaObject.getAuditoriaEsb_marcaTiempo());
		...
		respuestaEnvioJusticia.setCodigoUnidadOrganoRemitente(respuestaObject.getCodigoUnidadOrganoRemitente());
	...
	private void extraerUnidadProcedimiento(ObjetoInsideAplicacion dato) throws InSideServiceException {
		List<ObjetoInsideUnidad> unidades = inSideService.getUnidadesAplicacion(dato.getAplicacion(), false);
	...
	public List<InfAdicional> infAdicionalAltaApp() throws InsideWSException {
		...
		retorno.add(generateData(ObjetoInsideAplicacion.ROLE_MODIFICA_EXPEDIENTE, Boolean.TRUE.toString()));
		...
		retorno.add(generateData(ObjetoInsideAplicacion.ROLE_MODIFICA_DOCUMENTO, Boolean.TRUE.toString()));
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public Unidad altaUnidadAplicacion(String identificadorAplicacion, String identificadorUnidad, String numeroProcedimiento)
			throws InsideWSException {
		try {
			ObjetoInsideUnidad retorno = inSideService.asociarUnidadAplicacion(identificadorUnidad, numeroProcedimiento,
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public void eliminarUnidadAplicacion(String identificadorAplicacion, String identificadorUnidad, String numeroProcedimiento)
			throws InsideWSException {
		try {
			if (StringUtils.isBlank(identificadorAplicacion.trim()) || StringUtils.isBlank(identificadorUnidad.trim())) {
				throw new InsideWSException(InsideWsErrors.CAMPOS_OBLIGATORIOS);
			}
			inSideService.eliminarUnidadAplicacion(identificadorUnidad, numeroProcedimiento, identificadorAplicacion);
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public Unidad altaUnidadUsuario(String identificadorUsuario, String identificadorUnidad, String numeroProcedimiento)
			throws InsideWSException {
		try {
			ObjetoInsideUnidad retorno = inSideService.asociarUnidadUsuario(identificadorUnidad, numeroProcedimiento,
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public void eliminarUnidadUsuario(String identificadorUsuario, String identificadorUnidad, String numeroProcedimiento)
			throws InsideWSException {
		try {
			if (StringUtils.isBlank(identificadorUsuario.trim()) || StringUtils.isBlank(identificadorUnidad.trim())) {
				throw new InsideWSException(InsideWsErrors.CAMPOS_OBLIGATORIOS);
			}
			inSideService.eliminarUnidadUsuario(identificadorUnidad, numeroProcedimiento, identificadorUsuario);
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public List<Unidad> getUnidadesAplicacion(String identificadorAplicacion) throws InsideWSException {
		try {
			List<Unidad> retorno = new ArrayList<Unidad>();
			List<ObjetoInsideUnidad> insideUnidad = inSideService.getUnidadesAplicacion(identificadorAplicacion, true);
	...
	public List<Unidad> getUnidadesUsuario(String identificadorUsuario) throws InsideWSException {
		try {
			List<Unidad> retorno = new ArrayList<Unidad>();
			List<ObjetoInsideUnidad> insideUnidad = inSideService.getUnidadesUsuario(identificadorUsuario, true);
	...
	public Aplicacion actualizarCredencialesEeetuilApp(Aplicacion data) throws InsideWSException {
		try {
			...
			credential.setAplicacion(data.getCredencialEeutil().getAplicacion());
			credential.setPassword(data.getCredencialEeutil().getPassword());
			ObjetoInsideAplicacion objetoInsideAplicacion = inSideService.actualizarCredencialesEeetuilApp(app, credential);
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public UnidadAplicacionEeutil crearActualizarUnidadAplicacionEeutil(UnidadAplicacionEeutil data) throws InsideWSException {
		try {

			if (StringUtils.isBlank(data.getCodigoUnidad().trim()) || StringUtils.isBlank(data.getAplicacion().trim())
					|| StringUtils.isBlank(data.getPassword().trim())) {
				throw new InsideWSException(InsideWsErrors.CAMPOS_OBLIGATORIOS);
			}

			ObjetoInsideUnidadAplicacionEeutil objetoInsideUnidadAplicacionEeutil = inSideService
					.crearActualizarUnidadAplicacionEeutil(data.getCodigoUnidad(), data.getAplicacion(), data.getPassword());
	...
	private UnidadAplicacionEeutil convertDataToWSUnidadAplicacionEeutil(
		...
		unidadAplicacionEeutil.setCodigoUnidad(objetoInsideUnidadAplicacionEeutil.getCodigoUnidadOrganica());
		unidadAplicacionEeutil.setPassword(objetoInsideUnidadAplicacionEeutil.getPassword());
		return unidadAplicacionEeutil;
	}

	@Override
	public List<UnidadAplicacionEeutil> getUnidadAplicacionEeutil(String codigoUnidadOrganica) throws InsideWSException {
		List<UnidadAplicacionEeutil> listUnidadAplicacionEeutil = new ArrayList<UnidadAplicacionEeutil>();

		try {
			if (StringUtils.isNotBlank(codigoUnidadOrganica.trim())) {
				ObjetoInsideUnidadAplicacionEeutil objetoInsideUnidadAplicacionEeutil = inSideService
						.getUnidadAplicacionEeutil(codigoUnidadOrganica);
				listUnidadAplicacionEeutil.add(convertDataToWSUnidadAplicacionEeutil(objetoInsideUnidadAplicacionEeutil));
			} else {
				List<ObjetoInsideUnidadAplicacionEeutil> listObjetoInsideUnidadAplicacionEeutil = inSideService
						.getListUnidadAplicacionEeutil();
	...
	@Override
	public Aplicacion crearActualizarSerialNumberCertificado(String idAplicacion, String serialNumberCertificado)
			throws InsideWSException {
		try {
			if (StringUtils.isBlank(idAplicacion.trim()) || StringUtils.isBlank(serialNumberCertificado.trim())) {
				throw new InsideWSException(InsideWsErrors.CAMPOS_OBLIGATORIOS);
			}
			return convertDataToWS(inSideService.crearActualizarSerialNumberCertificado(idAplicacion, serialNumberCertificado));
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	private List<NumeroProcedimiento> convertDataToWSListProcedimiento(List<String> listaNumeroProcedimiento) {
	...
	@Override
	public NumeroProcedimiento altaNumeroProcedimiento(String numeroProcedimiento) throws InsideWSException {
		try {
			return convertDataToWSProcedimiento(inSideService.altaNumeroProcedimiento(numeroProcedimiento));
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	@Override
	public List<EstructuraCarpeta> listaEstructuraCarpeta(String identificadorEstructura) throws InsideWSException {
		try {
			return convertDataToWSListEstructuraCarpeta(inSideService.listaEstructuraCarpeta(identificadorEstructura));
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	private EstructuraCarpeta convertDataToWSEstructuraCarpeta(ObjetoEstructuraCarpetaInside estructuraCarpetaInside) {
		EstructuraCarpeta estructuraCarpeta = new EstructuraCarpeta();
		estructuraCarpeta.setIdentificadorEstructura(estructuraCarpetaInside.getIdentificadorEstructura());
		...
		List<TipoCarpetaIndizada> carpetasIndizados = convertEstructuraCarpeta(estructuraCarpetaInside.getElementosIndizados());
	...
	private TipoCarpetaIndizada convertDataToWSCarpeta(ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada carpetaIndizada) {
		...
		tipoCarpetaIndizada.getCarpetaIndizada().addAll(convertEstructuraCarpeta(carpetaIndizada.getElementosIndizados()));
		return tipoCarpetaIndizada;
	}

	@Override
	public EstructuraCarpeta altaEstructuraCarpeta(EstructuraCarpeta estructuraCarpeta) throws InsideWSException {
		try {
			List<ObjetoEstructuraCarpetaInside> estructura = inSideService
					.listaEstructuraCarpeta(estructuraCarpeta.getIdentificadorEstructura());
			if (CollectionUtils.isNotEmpty(estructura)) {
				throw new InsideWSException(InsideWsErrors.OBJECT_ALREADY_EXISTS);
			}
			return convertDataToWSEstructuraCarpeta(
					inSideService.altaEstructuraCarpeta(convertWSToDataListEstructuraCarpeta(estructuraCarpeta)));
		} catch (InSideServiceException e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}

	private ObjetoEstructuraCarpetaInside convertWSToDataListEstructuraCarpeta(EstructuraCarpeta estructuraCarpeta) {
		ObjetoEstructuraCarpetaInside objetoEstructuraCarpetaInside = new ObjetoEstructuraCarpetaInside();
		objetoEstructuraCarpetaInside.setIdentificadorEstructura(estructuraCarpeta.getIdentificadorEstructura());
		objetoEstructuraCarpetaInside.setCodigoUnidadOrganica(estructuraCarpeta.getCodigoUnidadOrganica());
		objetoEstructuraCarpetaInside.setNumeroProcedimiento(estructuraCarpeta.getNumeroProcedimiento());
		List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> carpetasIndizados = convertEstructuraCarpetaWS(
				estructuraCarpeta.getCarpetaIndizada());
	...
	private List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> convertEstructuraCarpetaWS(
		...
		List<ObjetoExpedienteInsideIndiceContenidoElementoIndizado> carpetasIndizados = new ArrayList<ObjetoExpedienteInsideIndiceContenidoElementoIndizado>();

		int orden = 0;
		for (TipoCarpetaIndizada carpetaIndizada : carpetaIndizadaWS) {
			ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada carpetaInside = convertWSToDataCarpeta(carpetaIndizada, orden);
			carpetasIndizados.add(carpetaInside);
			orden++;
		}
		return carpetasIndizados;
	...
	private ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada convertWSToDataCarpeta(TipoCarpetaIndizada carpetaIndizada,
			int orden) {
		ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada objCarpetaIndizada = new ObjetoExpedienteInsideIndiceContenidoCarpetaIndizada();
		...
		if (CollectionUtils.isNotEmpty(carpetaIndizada.getCarpetaIndizada()))
			objCarpetaIndizada.setElementosIndizados(convertEstructuraCarpetaWS(carpetaIndizada.getCarpetaIndizada()));
	...
	public void deleteEstructuraCarpeta(String identificadorEstructura) throws InsideWSException {
		try {
			...
			List<ObjetoEstructuraCarpetaInside> estructura = inSideService.listaEstructuraCarpeta(identificadorEstructura);
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public SolicitudAccesoExpAppUrl altaSolicitudAccesoExpAppUrl(SolicitudAccesoExpAppUrl data) throws InsideWSException {
		try {
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public SolicitudAccesoExpAppUrl actualizarSolicitudAccesoExpAppUrl(SolicitudAccesoExpAppUrl data) throws InsideWSException {
		try {
			if (!validaSolicitudAccesoExpAppUrl(data)) {
				throw new InsideWSException(InsideWsErrors.CAMPOS_OBLIGATORIOS);
			}
			ObjetoSolicitudAccesoExpAppUrl objeto = inSideService.getSolicitudAccesoExpAppUrlPorDir3(data.getDir3Padre());
			if (objeto == null) {
				throw new InsideWSException(InsideWsErrors.OBJECT_NOT_FOUND);
			}
			int id = objeto.getId();
	 ...
	 * "${schema-validation-enabled}" /> se hace aquí.
	 */
	private boolean validaSolicitudAccesoExpAppUrl(SolicitudAccesoExpAppUrl data) {

		if (StringUtils.isBlank(data.getUrlDestinoPeticion().trim()) || StringUtils.isBlank(data.getDir3Padre().trim())) {
			return false;
		}
		return true;
	}

	private ObjetoSolicitudAccesoExpAppUrl toObjetoSolicitudAccesoExpAppUrl(SolicitudAccesoExpAppUrl data) {
	...
	private ObjetoFilterPageRequest toObjetoFilterPageRequest(FilterPageRequest data) {
		...
		ObjetoFilterPageRequest respuesta = new ObjetoFilterPageRequest(data.getFiltro(), data.getPagina(), data.getLimite(), null);
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public UsuarioResultadoBusqueda listaUsuariosPaginado(FilterPageRequest filterPageRequest) throws InsideWSException {
		...
		try {
			...
			resultado = InsideConverterBusquedaResultado.
					respuestaBusquedaUsuarioInside(resultadoBusqueda);
	...
	@Override
	@Secured("ROLE_ADMINISTRAR_PERMISOS")
	public UsuarioAdminMensajes actualizarUsuarioAdminMensajes(UsuarioAdminMensajes usuarioAdminMensajes) throws InsideWSException {
		ObjetoInsideUsuario user = insideMessageService.actualizarUsuarioAdminMensajes(convertWsUserAdminMensajeToDataUser(usuarioAdminMensajes));
	...
	private UsuarioAdminMensajes convertUserDataToWSUserAdminMensajes(ObjetoInsideUsuario altaUsuario) {
