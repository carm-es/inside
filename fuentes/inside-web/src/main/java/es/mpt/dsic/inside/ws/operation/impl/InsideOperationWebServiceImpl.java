...
public class InsideOperationWebServiceImpl implements InsideOperationWebService {
	...
	protected static final String VERSION_NTI_EXP = "http://administracionelectronica.gob.es/ENI/XSD/v1.0/expediente-e";
	protected static final String VERSION_NTI_DOC = "http://administracionelectronica.gob.es/ENI/XSD/v1.0/documento-e";
	...
	public ExpedienteInsideInfo altaExpedienteEni(TipoExpediente expedienteEni, TipoMetadatosAdicionales metadatosAdicionales,
		...
		try {
			...
			if (expediente == null) {
				...
				ResultadoValidacion resultado = insideUtilService.comprobarDocumentosIndiceExpediente(expediente,
						listaDocumentos);
				// CARM ### v2.0.7.1
				...
				if (wsEnhancedConf.isActivoAltaExpedientEniDocumentoIndizadoAceptaVacio() &&
						...
						expediente.getIndice().getIndiceContenido().getElementosIndizados().get(0) instanceof ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado &&
						((ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado) expediente.getIndice().getIndiceContenido().getElementosIndizados().get(0)).getIdentificadorDocumento().isEmpty() &&
						((ObjetoExpedienteInsideIndiceContenidoDocumentoIndizado) expediente.getIndice().getIndiceContenido().getElementosIndizados().get(0)).getValorHuella().isEmpty()				
					) {
				...
				if (!resultado.isValido())
					...
					// CARM 2.0.7.1 ###
					throw new InsideWSException(InsideWsErrors.ERRORES_EN_EL_INDICE_EXPEDIENTE, null, resultado.getMensaje());
	...
	public ObjetoExpedienteInside validarExpedienteRecibido(TipoExpediente expedienteEni,
			TipoMetadatosAdicionales metadatosAdicionales, String aplicacion) throws InSideServiceException {
		try {
			...
			expediente = InsideConverterExpediente.expedienteEniToInside(expedienteEni, metadatosAdicionales);
			...
			//expediente = insideUtilService.validateExpedientImport(insideUtilService.generateExpXml(expAdic));
	...
	public ExpedienteInsideInfo altaExpedienteEniXml(TipoExpedienteEniFileInsideConDocumentos expedienteEniFile,
		...
		try {
			String info;
			ObjetoExpedienteInside objExp = insideUtilService.validateExpedientImport(expedienteEniFile.getExpedienteEniBytes());

			if (!insideUtilService.esLongitudIdentificadorValido(objExp.getIdentificador())) {
				throw new InsideWSException(InsideWsErrors.IDENTIFICADOR_DOCUMENTO_LONGITUD, null, "");
			}
			List<ObjetoDocumentoInside> documentos = insideUtilService
					.recoverDocumentXml(expedienteEniFile.getDocumentosEniFile());
			Collection<String> resultado = service.correspondencia(objExp, service.getListIdentifierDocsInside(documentos));
			if (resultado == null) {
				Map<String, Object> result = insideUtilService.crearNuevoExpediente(objExp, documentos, aplicacion, false);
				objExp = (ObjetoExpedienteInside) result.get("expediente");
				info = result.get("docsNoCreate") != null
						? InsideWsErrors.DOCS_SIN_ACTUALIZAR.getValue().getDescripcion() + result.get("docsNoCreate") : "";
			} else {
				throw new InsideWSException(InsideWsErrors.CORRESPONDENCIA_DOCUMENTOS_EXPEDIENTE, null, resultado.toString());
			}
	...
	public ExpedienteInsideInfo cambiaMetadatosExpediente(String identificador, TipoMetadatos metadatosEni,
		...
		try {
			...
			ObjetoExpedienteInside expediente = service.cambiaMetadatosExpediente(identificador, metadatos, aplicacion, false);
	...
	public DocumentoInsideInfo cambiaMetadatosDocumento(String identificador,
		...
		try {
			...
			ObjetoDocumentoInsideMetadatos metadatos = InsideConverterMetadatos.Documento
					.metadatosXmlToMetadatosInside(metadatosEni, metadatosAdicionales);
			ObjetoDocumentoInside documento = service.cambiaMetadatosDocumento(identificador, metadatos, aplicacion, false);
	...
	public ExpedienteInsideInfo modificaExpedienteEni(TipoExpediente expedienteEni, TipoMetadatosAdicionales metadatosAdicionales,
		...
		try {
			// Control Permisos
			servicePermisos.checkPermisos(expedienteEni.getMetadatosExp().getIdentificador(), null, ObjetoExpedienteInside.class,
					aplicacion);
			...
			ObjetoExpedienteInside expediente = InsideConverterExpediente.expedienteEniToInside(expedienteEni,
	...
	public ExpedienteInsideInfo altaCarpetaEnExpediente(String identificadorExpediente, TipoCarpetaIndizada carpetaIndizada,
			String ruta, String aplicacion) throws InsideWSException {
		try {
			...
			if (carpetaIndizada == null
					|| org.apache.commons.lang.StringUtils.isBlank(carpetaIndizada.getIdentificadorCarpeta())) {
			...
			ObjetoExpedienteInside expediente = checkNoRepeatIds(identificadorExpediente, carpetaIndizada);
			expediente = service.altaCarpeta(expediente, InsideConverterIndice.carpetaIndizadaConversionToInside(carpetaIndizada),
					ruta, aplicacion, false);
			return InsideConverterExpedienteInfo.fromExpediente(expediente);
		} catch (Exception e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	...
	private ObjetoExpedienteInside checkNoRepeatIds(String identificadorExpediente, TipoCarpetaIndizada carpetaIndizada)
			throws InsideWSException {
		try {
			ObjetoExpedienteInside expediente = service.getExpediente(identificadorExpediente, false);

			TipoExpediente tipoExpediente = InsideConverterExpediente.expedienteInsideToEni(expediente, null);
			...
			if (CollectionUtils.isNotEmpty(listRepeatIds)) {
				...
				for (String id : listRepeatIds)
					sb.append(id + ".");
				throw new InsideWSException(InsideWsErrors.IDENTIFICADOR_CARPETA_VACIO, null, sb.toString());
			}
			return expediente;
	...
	public void checkPermissionsAltaCarpeta(String identificadorExpediente, TipoCarpetaIndizada carpetaIndizada,
			String aplicacion) throws InSideServiceException {
		servicePermisos.checkPermisos(identificadorExpediente, null, ObjetoExpedienteInside.class, aplicacion);

		Map<String, String> listId = service
				.obtenerDocsExpTipo(carpetaIndizada.getDocumentoIndizadoOrExpedienteIndizadoOrCarpetaIndizada(), "/");
		Iterator it = listId.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry elemento = (Map.Entry) it.next();
			servicePermisos.checkPermisos((String) elemento.getKey(), null, ObjetoDocumentoInside.class, aplicacion);
		}
	}
	...
	public ExpedienteInsideInfo borrarCarpetaEnExpediente(String identificadorExpediente, String identificadorCarpeta,
		...
		try {
			...
			logger.debug("llamando a borrar carpeta para expediente " + identificadorExpediente + " con identificador de carpeta "
					+ identificadorCarpeta + " en ruta " + ruta);

			// Control Permisos
			servicePermisos.checkPermisos(identificadorExpediente, null, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.borrarCarpeta(identificadorExpediente, identificadorCarpeta, ruta);
			insideUtilService.validateExpedientImport(insideUtilService
					.generateExpXml(InsideConverterExpediente.expedienteInsideToConMAdicionales(expediente, null)));
	...
	@Override
	@Secured("ROLE_MODIFICA_EXPEDIENTE")
	public ExpedienteInsideInfo cambiaEstadoExpediente(String identificador, EnumeracionEstados estado, String aplicacion)
			throws InsideWSException {
		try {
			if (estado == null) {
			...
			ObjetoExpedienteInsideMetadatosEnumeracionEstados estadoInside = ObjetoExpedienteInsideMetadatosEnumeracionEstados
					.fromValue(estado.value());

			ObjetoExpedienteInside expediente = service.setEstadoExpediente(identificador, estadoInside, false, aplicacion,
					false);

			ExpedienteInsideInfo expedienteInfo = InsideConverterExpedienteInfo.fromExpediente(expediente);

			logger.info("Cambiado estado del Expediente con identificador " + identificador + " a " + estado.value());
			return expedienteInfo;
	...
	public ExpedienteInsideInfo importarExpediente(String identificador, String identificadorImportado, String ruta,
		...
		try {
			...
			ObjetoExpedienteInside expediente = service.importarExpediente(identificador, identificadorImportado, ruta,
					aplicacion, false);

			ExpedienteInsideInfo expedienteInfo = InsideConverterExpedienteInfo.fromExpediente(expediente);

			logger.info("importado Expediente con identificador " + identificadorImportado + " a expediente" + identificador);
			return expedienteInfo;
	...
	public ExpedienteInsideInfo vincularExpediente(String identificador, String identificadorVinculado, String ruta,
		...
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);
			servicePermisos.checkPermisos(identificadorVinculado, null, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.vincularExpediente(identificador, identificadorVinculado, ruta,
					aplicacion, false);

			ExpedienteInsideInfo expedienteInfo = InsideConverterExpedienteInfo.fromExpediente(expediente);

			logger.info("Vinculado Expediente con identificador " + identificadorVinculado + " a expediente" + identificador);
			return expedienteInfo;
	...
	@Override
	@Secured("ROLE_MODIFICA_EXPEDIENTE")
	public ExpedienteInsideInfo crearVistaAbiertaExpediente(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.createOpenViewExpedient(identificador, null, false, aplicacion, false);
	...
	@Override
	@Secured("ROLE_MODIFICA_EXPEDIENTE")
	public ExpedienteInsideInfo crearVistaCerradaExpediente(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.createClosedViewExpedient(identificador, null, false, aplicacion, false);
	...
	public ExpedienteInsideInfo vincularDocumentosEnExpediente(String identificador, List<String> identificadoresDocumentos,
		...
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);
			servicePermisos.checkPermisos(identificadoresDocumentos, null, ObjetoDocumentoInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.vincularDocumentosEnExpediente(identificador, identificadoresDocumentos,
					ruta, aplicacion, false);

			ExpedienteInsideInfo expedienteInfo = InsideConverterExpedienteInfo.fromExpediente(expediente);

			logger.info(
					"importados Documentos " + identificadoresDocumentos + " a expediente" + identificador + " en ruta " + ruta);
			return expedienteInfo;
	...
	public ExpedienteInsideInfo desvincularDocumentosEnExpediente(String identificador, List<String> identificadoresDocumentos,
		...
		try {
			...
			insideUtilService.validateExpedientImport((insideUtilService.getStringExpedienteMarshall(expediente)).getBytes());
			service.modificaExpedienteInside(expediente, aplicacion, false);
			ExpedienteInsideInfo expedienteInfo = InsideConverterExpedienteInfo.fromExpediente(expediente);

			logger.info("desvinculados Documentos " + identificadoresDocumentos + " a expediente" + identificador + " en ruta "
					+ ruta);
			return expedienteInfo;
	...
	public ExpedienteInsideInfo cambiarUbicacionEnExpediente(String identificadorExpediente,
			List<String> identificadoresElementos, String rutaOrigen, String rutaDestino, String aplicacion)
					throws InsideWSException {
		try {
			servicePermisos.checkPermisos(identificadorExpediente, null, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInside expediente = service.cambiarUbicacionEnExpediente(identificadorExpediente,
					identificadoresElementos, rutaOrigen, rutaDestino, aplicacion, false);
			insideUtilService.validateExpedientImport((insideUtilService.getStringExpedienteMarshall(expediente)).getBytes());
			service.modificaExpedienteInside(expediente, aplicacion, false);
			return InsideConverterExpedienteInfo.fromExpediente(expediente);
		} catch (Exception e) {
	...
	public TipoExpedienteInside getExpediente(String identificador, Integer version, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, version, ObjetoExpedienteInside.class, aplicacion);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public TipoExpediente getExpedienteEni(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);
			...
			TipoExpediente expedienteEni = InsideConverterExpediente.expedienteInsideToEni(expediente, null);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public TipoExpedienteEniFileInside getExpedienteEniFile(String identificador, Boolean visualizacionIndice, String aplicacion)
			throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);
			...
			TipoExpedienteEniFileInside expedienteEniFile = InsideConverterExpediente
					.expedienteEniToTipoExpedienteEniFileInside(expedienteEni, true);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public TipoIndice getIndiceExpediente(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public ExpedienteInsideMetadatos getExpedienteMetadatos(String identificador, Integer version, String aplicacion)
			throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, version, ObjetoExpedienteInside.class, aplicacion);

			ObjetoExpedienteInsideMetadatos metadatos = service.getMetadatosExpediente(identificador, version);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public List<TipoVersionInside> getVersionesExpediente(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoExpedienteInside.class, aplicacion);

			return InsideConverterVersion.versionesInsideToXml(service.getVersionesExpediente(identificador));
	...
	public DocumentoInsideInfo altaDocumentoOriginal(TipoDocumentoAltaInside documentoAlta, String aplicacion)
			throws InsideWSException {
		try {
			if (documentoAlta.getAsociadoaExpediente() != null
					&& StringUtils.hasText(documentoAlta.getAsociadoaExpediente().getIdentificadorExpediente())) {
				// Control Permisos si viene informado el expediente
				servicePermisos.checkPermisos(documentoAlta.getAsociadoaExpediente().getIdentificadorExpediente(), null,
			...
			DocumentoInsideInfo documentoInfo = InsideConverterDocumento.documentoInsideToDocumentoInfo(documentoInside);

			logger.info("Dado de alta Documento original con identificador " + documentoInside.getIdentificador());
	...
	@Override
	@Secured("ROLE_ALTA_DOCUMENTO")
	public DocumentoInsideInfo altaDocumentoDigitalizado(TipoDocumentoAltaInside documentoAlta, String aplicacion)
			throws InsideWSException {
		try {
			if (documentoAlta.getAsociadoaExpediente() != null
					&& StringUtils.hasText(documentoAlta.getAsociadoaExpediente().getIdentificadorExpediente())) {
			...
			logger.info("Dado de alta Documento digitalizado con identificador " + documentoInside.getIdentificador());
			return documentoInfo;
	...
	public DocumentoInsideInfo altaDocumentoCopiaParcial(TipoDocumentoAltaInside documentoAlta,
		...
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificadorDocumentoOrigen, null, ObjetoDocumentoInside.class, aplicacion);
			...
			ObjetoDocumentoInside documentoInside = this.altaDocumento(documentoAlta,
					ObjetoDocumentoInsideMetadatosEstadoElaboracionEnumeracion.EE_04, identificadorDocumentoOrigen, aplicacion);

			DocumentoInsideInfo documentoInfo = InsideConverterDocumento.documentoInsideToDocumentoInfo(documentoInside);

			logger.info("Dado de alta Documento con identificador " + documentoInside.getIdentificador()
					+ " proviniente de copia parcial de documento origen con identificador " + identificadorDocumentoOrigen);
			return documentoInfo;
	...
	public DocumentoInsideInfo altaDocumentoCambioFormato(TipoDocumentoAltaInside documentoAlta,
		...
		try {
			...
			ObjetoDocumentoInside documentoInside = this.altaDocumento(documentoAlta,
					ObjetoDocumentoInsideMetadatosEstadoElaboracionEnumeracion.EE_02, identificadorDocumentoOrigen, aplicacion);

			DocumentoInsideInfo documentoInfo = InsideConverterDocumento.documentoInsideToDocumentoInfo(documentoInside);

			logger.info("Dado de alta Documento con identificador " + documentoInside.getIdentificador()
					+ " proviniente de cambio de formato de documento origen con identificador " + identificadorDocumentoOrigen);
			return documentoInfo;
	...
	@Override
	@Secured("ROLE_ALTA_DOCUMENTO")
	public DocumentoInsideInfo altaDocumentoOtros(TipoDocumentoAltaInside documentoAlta, String aplicacion)
			throws InsideWSException {
		try {
			if (documentoAlta.getAsociadoaExpediente() != null
					&& StringUtils.hasText(documentoAlta.getAsociadoaExpediente().getIdentificadorExpediente())) {
	...
	public DocumentoInsideInfo altaDocumentoEni(TipoDocumento documentoEni, TipoDocumentoAsociadoaExpediente asociadoAExpediente,
			TipoMetadatosAdicionales metadatosAdicionales, boolean firmaServidor, String aplicacion) throws InsideWSException {
		try {
			checkVersionNTI(documentoEni.getMetadatos());

			logger.debug("Convirtiendo el documentoEni a Inside");

			if (asociadoAExpediente != null && StringUtils.hasText(asociadoAExpediente.getIdentificadorExpediente())) {
			...
			documento = insideUtilService.validateDocumentImport(insideUtilService.generateDocXml(docAdic));
			insideUtilService.checkNoExistIdDocumentInside(documento.getIdentificador(), true);

			if (asociadoAExpediente != null && StringUtils.hasText(asociadoAExpediente.getIdentificadorExpediente())) {
				documento = service.altaDocumento(documento, asociadoAExpediente.getIdentificadorExpediente(),
			...
			DocumentoInsideInfo documentoInfo = InsideConverterDocumento.documentoInsideToDocumentoInfo(documento);

			logger.info("Importado Documento desde ENI con identificador " + documento.getIdentificador());
	...
	@Override
	@Secured("ROLE_ALTA_DOCUMENTO")
	public DocumentoInsideInfo altaDocumentoEniXml(TipoDocumentoEniFileInside documentoEniFile, String aplicacion)
			throws InsideWSException {
		try {

			ObjetoDocumentoInside objDoc = insideUtilService.validateDocumentImport(documentoEniFile.getDocumentoEniBytes());
	...
	public DocumentoInsideInfo modificaDocumento(TipoDocumento documentoEni, TipoMetadatosAdicionales metadatosAdicionales,
		...
		try {
			// Control Permisos
			servicePermisos.checkPermisos(documentoEni.getMetadatos().getIdentificador(), null, ObjetoDocumentoInside.class,
					aplicacion);
			...
			ObjetoDocumentoInside documento = insideUtilService.validateDocumentImport(insideUtilService.generateDocXml(docAdic));
	...
	public TipoDocumentoInside getDocumento(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);
			...
			TipoDocumentoInside documentoXml = InsideConverterDocumento.documentoInsideToXml(documento, null);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public TipoDocumento getDocumentoEni(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public TipoDocumentoEniFileInside getDocumentoEniFile(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);
			...
			TipoDocumentoEniFileInside documentoEniFile = InsideConverterDocumento
					.documentoEniToTipoDocumentoEniFileInside(documentoEni, true);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public TipoContenido getDocumentoContenido(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);

			ObjetoDocumentoInsideContenido contenidoInside = service.getDocumentoContenido(identificador, null);

			TipoContenido contenido = InsideConverterDocumento.Contenido.contenidoInsideToEni(contenidoInside, null);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public DocumentoInsideMetadatos getMetadatosDocumento(String identificador, Integer version, String aplicacion)
			throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, version, ObjetoDocumentoInside.class, aplicacion);

			ObjetoDocumentoInsideMetadatos metadatosInside = service.getMetadatosDocumento(identificador);

			DocumentoInsideMetadatos metadatos = InsideConverterMetadatos.Documento.metadatosInsideToXml(metadatosInside);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public Integer getNumeroBytesDocumentoFirmado(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public Firmas getFirmasEniCSVDocumento(String identificador, String aplicacion) throws InsideWSException {
		try {
			// Control Permisos
			servicePermisos.checkPermisos(identificador, null, ObjetoDocumentoInside.class, aplicacion);

			return InsideConverterFirmas.firmasInsideToEni(service.getFirmasCSVDocumentoInside(identificador), true);
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public ExpedienteResultadoBusqueda buscarExpedientes(ConsultaWsInside consulta, int limite, int pagina, String aplicacion)
			throws InsideWSException {
		try {
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public DocumentoResultadoBusqueda buscarDocumentos(ConsultaWsInside consulta, int limite, int pagina, String aplicacion)
			throws InsideWSException {
		try {
	...
	public DocumentoEniFileInside convertirDocumentoAEni(TipoDocumentoConversionInside documentoConversion, byte[] bytesContenido,
			Boolean firmar, WSCredentialInside aplicacion) throws InsideWSException {
		try {
			return gInsideOperationWebService.convertirDocumentoAEni(documentoConversion, bytesContenido, firmar, aplicacion);
		} catch (Exception e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	...
	public ExpedienteEniFileInside convertirExpedienteAEni(TipoExpedienteConversionInside expedienteConversion,
			String contenidoFirma, WSCredentialInside aplicacion) throws InsideWSException {
		try {
			return gInsideOperationWebService.convertirExpedienteAEni(expedienteConversion, contenidoFirma, aplicacion);
		} catch (Exception e) {
			throw InsideExceptionConverter.convertToException(e);
		}
	}
	...
	private ObjetoDocumentoInside altaDocumento(TipoDocumentoAltaInside documentoAlta,
			ObjetoDocumentoInsideMetadatosEstadoElaboracionEnumeracion estadoElaboracion, String identificadorDocumentoOrigen,
			String aplicacion) throws InSideServiceException, InsideConverterException {
		logger.debug("Convirtiendo el documentoAlta a Inside");

		ObjetoDocumentoInside documentoInside = InsideConverterDocumento.documentoAltaXmlToInside(documentoAlta, infofirmaService,
				null);
		
		ObjetoDocumentoInsideMetadatosEstadoElaboracion estadoElaboracionCambioFormato = new ObjetoDocumentoInsideMetadatosEstadoElaboracion();
		...
		documentoInside = insideUtilService.validateDocumentImport(insideUtilService.generateDocXml(docAdic));
		...
		if (documentoAlta.getAsociadoaExpediente() != null
			...
			documentoInside = service.altaDocumento(documentoInside,
					documentoAlta.getAsociadoaExpediente().getIdentificadorExpediente(), carpeta, aplicacion, false);
		} else {
	...
	private void checkVersionNTI(es.mpt.dsic.inside.xml.eni.documento.metadatos.TipoMetadatos metadatosEni)
			throws InsideWSException {
		metadatosEni.setVersionNTI(checkVersionNTIDocumento(metadatosEni.getVersionNTI()));
	}

	/**
	 * Nos aseguramos que el valor del metadato versionNTI siempre sea la 1.0
	 * (de momento no soportamos otra cosa)
	 * 
	 * @param versionNTI
	 * @return
	 * @throws InsideWSException
	...
	@Override
	@Secured("ROLE_LEER_EXPEDIENTE")
	public List<ExpedienteInsideInfo> expedientesAsociadoDocumento(String identificador, String aplicacion)
			throws InsideWSException {
		try {
			...
			List<ObjetoExpedienteInside> expedientesInside = service.getExpedientesAsociadosDocumento(ObjetoDocumentoInside.class,
					identificador);
	...
	@Override
	@Secured("ROLE_LEER_DOCUMENTO")
	public TipoResultadoVisualizacionDocumentoInside visualizarDocumento(String identificador, String aplicacion)
			throws InsideWSException {
	...
	public RespuestaPdfExpediente getPdfExpediente(byte[] expedienteEni) throws InsideWSException {
		...
		try {
			...
			tipoExpedienteMA.setMetadatosAdicionales(
					InsideConverterMetadatos.metadatosAdicionalesInsideToXml(objExp.getMetadatos().getMetadatosAdicionales()));
			
			respuestaPdfExpediente = new RespuestaPdfExpediente();
			respuestaPdfExpediente.setPdfExpediente(tipoExpedienteMA.getExpediente().getVisualizacionIndice().getValorBinario());
