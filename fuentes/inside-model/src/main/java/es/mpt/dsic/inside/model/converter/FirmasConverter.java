...
public abstract class FirmasConverter {
  ...
  public static Firmas firmasInsideToEeutilClient(es.mpt.dsic.inside.xml.eni.firma.Firmas input,
  ...
  public static List<TipoFirmasElectronicas> tipoFirmasInsideToEeutilClient(
      List<es.mpt.dsic.inside.xml.eni.firma.TipoFirmasElectronicas> input,
      byte[] contenidoDocumento) {
    ...
    if (CollectionUtils.isNotEmpty(input)) {
      for (es.mpt.dsic.inside.xml.eni.firma.TipoFirmasElectronicas tipoFirma : input) {
        ...
        data.setContenidoFirma(
            contenidoFirmaInsideToEeutilClient(tipoFirma.getContenidoFirma(), contenidoDocumento));
  ...
  public static ContenidoFirma contenidoFirmaInsideToEeutilClient(
      es.mpt.dsic.inside.xml.eni.firma.TipoFirmasElectronicas.ContenidoFirma input,
      byte[] contenidoDocumento) {
    ContenidoFirma retorno = new ContenidoFirma();
    if (input.getCSV() != null) {
      retorno.setCSV(csvInsideToEeutilClient(input.getCSV()));
    }
    retorno.setFirmaConCertificado(firmaConCertificadoInsideToEeutilClient(
        input.getFirmaConCertificado(), contenidoDocumento));
    return retorno;
  }

  public static CSV csvInsideToEeutilClient(
      es.mpt.dsic.inside.xml.eni.firma.TipoFirmasElectronicas.ContenidoFirma.CSV input) {
    CSV retorno = new CSV();
  ...
  public static FirmaConCertificado firmaConCertificadoInsideToEeutilClient(
      es.mpt.dsic.inside.xml.eni.firma.TipoFirmasElectronicas.ContenidoFirma.FirmaConCertificado input,
      byte[] contenidoDocumento) {
  ...
  public static TipoFirma tipoFirmaInsideToEeutilClient(
      es.mpt.dsic.inside.xml.eni.firma.TipoFirma input) {
