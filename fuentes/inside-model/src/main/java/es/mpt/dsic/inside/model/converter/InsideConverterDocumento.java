...
public abstract class InsideConverterDocumento {
  ...
  private static final String EL_XML_INTRODUCIDO_NO_ES_UN_DOCUMENTO_ENI_VALIDO =
      "El xml introducido no es un Documento Eni válido ";
  protected static final Log logger = LogFactory.getLog(InsideConverterDocumento.class);

  public static ObjetoDocumentoInside documentoEniAndMetadatosToDocumentoInside(
      TipoDocumento documentoEni, TipoMetadatosAdicionales metadatosAdicionales)
    ...
    documentoInside.setMetadatos(InsideConverterMetadatos.Documento
        .metadatosEniToInside(documentoEni.getMetadatos(), metadatosAdicionales));

    return documentoInside;
  }

  public static ObjetoDocumentoInside documentoEniAndMetadatosMtomToDocumentoInside(
      TipoDocumentoMtom documentoEni, TipoMetadatosAdicionales metadatosAdicionales)
  ...
  public static ObjetoDocumentoInsideContenido documentoInsideToObjetoDocumentoInsideContenido(
      ObjetoDocumentoInside documentoInside, InfoFirmaService infoFirmaService,
    ...
    if (infoFirma != null) {
      if (infoFirma.getTipoContenidoFirmado().equals(TipoContenidoFirmado.TC_H)) {
        throw new InsideConverterException(
            "La firma es explícita. No se pueden obtener los bytes del documento", true);
      }
      ...
      try {
        documentoInsideContenido.setNombreFormato(
            InsideConverterUtils.getNombreFormatoByMime(infoFirma.getMimeContenidoFirmado()));
      } catch (InsideConverterUtilsException e) {
        throw new InsideConverterException("No se puede obtener el formato del contenido firmado.",
            e, true);
      }
    }
  ...
  public static byte[] getFirmaElectronica(ObjetoDocumentoInside documentoInside)
      throws InsideConverterException {
    byte[] contenido;
    if (documentoInside.getContenido().getValorBinario() != null) {
      ...
      while (i < documentoInside.getFirmas().size() && !found) {
        if (idReferencia
            .equalsIgnoreCase(documentoInside.getFirmas().get(i).getIdentificadorEnDocumento())) {
          contenidoFirma = (ContenidoFirmaCertificadoContenidoBinarioInside) documentoInside
              .getFirmas().get(i).getContenidoFirma();
          found = true;
        }
        i++;
      }
      ...
      if (!found) {
        throw new InsideConverterException(
            "No se ha encontrado la firma a la que hace referencia el documento.");
      }
  ...
  public static byte[] getFicheroFirmado(ObjetoDocumentoInside documentoInside)
      throws InsideConverterException {
    byte[] contenido;
    if (documentoInside.getContenido().getValorBinario() != null) {
      contenido = documentoInside.getContenido().getValorBinario();
  ...
  public static void checkContenidoNoEsFirma(ObjetoDocumentoInside documentoInside,
      ObjetoDocumentoInsideContenido documentoInsideContenido, boolean contenidoEnContenido,
      InfoFirmaServiceException e) throws InsideConverterException {
  ...
  public static ObjetoDocumentoInside documentoConversionToInside(
    ...
    ObjetoDocumentoInside documentoInside = new ObjetoDocumentoInside();
    documentoInside.setMetadatos(InsideConverterMetadatos.Documento
        .metadatosConversionToInside(documentoConversion.getMetadatosEni()));
    documentoInside.setIdentificador(documentoConversion.getMetadatosEni().getIdentificador());

    documentoInside = fillContenidoAndFirmasObjetoDocumentoInside(documentoInside,
        documentoConversion.getContenido(), contenido,
        documentoConversion.isFirmadoConCertificado(),
  ...
  public static ObjetoDocumentoInside documentoConversionToInside(
      TipoDocumentoConversionInside documentoConversion, ObjetoDocumentoInside documentoInside)
      throws InsideConverterException {
    documentoInside.setMetadatos(InsideConverterMetadatos.Documento
        .metadatosConversionToInside(documentoConversion.getMetadatosEni()));
    documentoInside.setIdentificador(documentoConversion.getMetadatosEni().getIdentificador());
  ...
  public static TipoDocumentoConversionInside documentoConversionToInside(
      ObjetoDocumentoInside docInside) throws InsideConverterException {
  ...
  private static Csv getCsvObjetoConversionInside(List<FirmaInside> firmas) {
    ...
    for (FirmaInside firma : firmas) {
      if ("TF01".equals(firma.getTipoFirma().value())) {
        ...
        csv.setRegulacionCSV(
            ((ContenidoFirmaCSVInside) firma.getContenidoFirma()).getRegulacionGeneracionCSV());
      }
    }
    return csv;
  }

  public static ObjetoDocumentoInside documentoAltaXmlToInside(
      TipoDocumentoAltaInside documentoAlta, InfoFirmaService infoFirmaService, byte[] contenido)
      throws InsideConverterException {

    ObjetoDocumentoInside documentoInside = new ObjetoDocumentoInside();

    documentoInside.setMetadatos(InsideConverterMetadatos.Documento.metadatosEniToInside(
        documentoAlta.getMetadatosEni(), documentoAlta.getMetadatosAdicionales()));
      ...
      if ("XADES DETACHED".equalsIgnoreCase(infoFirma.getTipoFirma().getTipoFirma())
        ...
        try {
          ...
          throw new InsideConverterException("Del documento: " + idDocumento
              + ". No se puede extraer el nombre del formato a partir del Mime: "
              + contenido.getMime(), e, true);
        }
        ...
        contenido
            .setReferencia("#" + documentoInside.getFirmas().get(0).getIdentificadorEnDocumento());

        // a partir del mime hay que traducirlo al formato aceptado
        try {
          contenido.setNombreFormato(
              InsideConverterUtils.getNombreFormatoByMime(infoFirma.getMimeContenidoFirmado()));
        } catch (InsideConverterUtilsException e) {
          throw new InsideConverterException("Del documento: " + idDocumento
              + ". No se puede extraer el nombre del formato a partir del Mime: "
              + infoFirma.getMimeContenidoFirmado(), e, true);
        }
  ...
  public static TipoDocumentoInsideConMAdicionales documentoInsideToConMAdicionales(
      ObjetoDocumentoInside documentoInside, byte[] bytesContenido)
      throws InsideConverterException {
    TipoDocumentoInsideConMAdicionales documentoConMAdicionales =
        new TipoDocumentoInsideConMAdicionales();
  ...
  public static TipoDocumentoInside documentoInsideToXml(ObjetoDocumentoInside documentoInside,
      byte[] bytesContenido) throws InsideConverterException {
  ...
  public static TipoDocumento documentoInsideToEni(ObjetoDocumentoInside documentoInside,
      byte[] bytesContenido) throws InsideConverterException {
    TipoDocumento documentoEni = new TipoDocumento();
    TipoContenido contenidoEni =
        Contenido.contenidoInsideToEni(documentoInside.getContenido(), bytesContenido);
    ...
    documentoEni.setMetadatos(
        InsideConverterMetadatos.Documento.metadatosInsideToEni(documentoInside.getMetadatos()));
    return documentoEni;
  }
  ...
  public static TipoDocumentoEniFileInside documentoEniToTipoDocumentoEniFileInside(
      TipoDocumento documentoEni, boolean cabeceraXml) throws InsideConverterException {
  ...
  public static class Contenido {
    ...
    public static ObjetoDocumentoInsideContenido contenidoEniToInside(
        TipoContenidoMtom contenidoEni) throws IOException {
      ...
      if (contenidoEni.getDatosXML() != null) {
        ...
        contenidoInside
            .setValorBinario(IOUtils.toByteArray(contenidoEni.getValorBinario().getInputStream()));
        contenidoInside
            .setMime(InsideConverterUtils.getMimeByNombreFormato(contenidoEni.getNombreFormato()));
      } else if (contenidoEni.getReferenciaFichero() != null) {
        contenidoInside.setReferencia(contenidoEni.getReferenciaFichero());
    ...
    public static TipoContenido contenidoInsideToEni(ObjetoDocumentoInsideContenido contenidoInside,
        byte[] bytesContenido) {
    ...
    public static void extraerContenidoFirma(ObjetoDocumentoInsideContenido contenidoInside,
  ...
  public static ObjetoDocumentoInside documentoVisualizacionToInside(
      TipoDocumentoVisualizacionInside documentoVisualizacion) throws InsideConverterException {
      ...
      try {
        ...
        try {
          ...
          if (nodoEni != null) {
            ...
            String nodoEniString =
                XMLUtils.documentoAdicionalWsToEni(docEniBinarioOTipo.getDocumentoEniBinario());
            e = (JAXBElement<?>) unmarshaller
                .unmarshal(new ByteArrayInputStream(IOUtils.toByteArray(nodoEniString)));
            docEni = (TipoDocumento) e.getValue();
          }
          ...
          throw new InsideConverterException(EL_XML_INTRODUCIDO_NO_ES_UN_DOCUMENTO_ENI_VALIDO, e2,
              true);
        }
      }
    } else {
    ...
    TipoMetadatosAdicionales metadatosAdicionales =
        documentoVisualizacion.getMetadatosAdicionales();
  ...
  public static ObjetoDocumentoInside tipoDocumentoValidacionToInside(
    ...
    try {
      ...
      element = (JAXBElement<?>) unmarshaller
          .unmarshal(new ByteArrayInputStream(documentoValidacion.getContenido()));
      ...
      try {
        ...
        if (nodoEni != null) {
          ...
          String nodoEniString =
              XMLUtils.documentoAdicionalWsToEni(documentoValidacion.getContenido());
          element = (JAXBElement<?>) unmarshaller
              .unmarshal(new ByteArrayInputStream(IOUtils.toByteArray(nodoEniString)));
          doc = (TipoDocumento) element.getValue();
        }
  ...
  public static ObjetoDocumentoInside tipoDocumentoValidacionToInsideToMtom(
   ...
   * 
   * @param firmaInside objeto FirmaInside
   * @param contenido Contenido del documento
   * @return contenido de la firma InsideConverterException error.
   */
  public static byte[] obtenerContenidoFirma(FirmaInside firmaInside, byte[] contenido)
      throws InsideConverterException {
    ...
    if (FirmaInsideTipoFirmaEnum.TF_02.name().equals(firmaInside.getTipoFirma().name())
      ...
      if (firmaInside.getContenidoFirma() instanceof ContenidoFirmaCertificadoReferenciaInside) {
        contenidoFirma = contenido;
          ...
          .getContenidoFirma() instanceof ContenidoFirmaCertificadoDsSignatureInside) {
        contenidoFirma =
            ((ContenidoFirmaCertificadoDsSignatureInside) firmaInside.getContenidoFirma())
                .getValorBinario();

      } else if (firmaInside
          .getContenidoFirma() instanceof ContenidoFirmaCertificadoAlmacenableInside) {
        contenidoFirma =
            ((ContenidoFirmaCertificadoAlmacenableInside) firmaInside.getContenidoFirma())
                .getValorBinario();
      ...
      if (firmaInside.getContenidoFirma() instanceof ContenidoFirmaCertificadoAlmacenableInside) {
        ContenidoFirmaCertificadoAlmacenableInside contenidoFirmaCertificado =
            (ContenidoFirmaCertificadoAlmacenableInside) firmaInside.getContenidoFirma();
  ...
  /**
   * Convierte el contenido en una firma para que sea válida para @firma. En el caso de la firma
   * XAdES Detached añadiendo las etiquetas <AFIRMA></AFIRMA> si no las tiene.
   * 
   * @param contenidoFirma Contenido de la firma
   * @return firma convertida.
   * @throws InsideConverterException error
   */
  public static byte[] convertirContenidoAFirma(byte[] contenidoFirma)
      throws InsideConverterException {
    byte[] firma = null;

    try {
      String contenidoBinarioStr =
          org.apache.commons.io.IOUtils.toString(new ByteArrayInputStream(contenidoFirma));
      if (contenidoBinarioStr.indexOf("Signature") > -1
          && contenidoBinarioStr.indexOf("SignedInfo") > -1) {
        ...
        if (contenidoBinarioStr.startsWith("<![CDATA[")) {
          ...
          contenidoBinarioStr =
              contenidoBinarioStr.substring(0, contenidoBinarioStr.lastIndexOf("]]>"));
        }
      ...
      throw new InsideConverterException(
          "Se ha producido un error al convertir el contenido a firma: ", e, true);
    }
  ...
  public static DocumentoEniConMAdicionales convertToEeutil(
      ObjetoDocumentoInside objetoDocumentoInside) throws InsideConverterException {
    DocumentoEniConMAdicionales retorno = new DocumentoEniConMAdicionales();

    TipoDocumentoInsideConMAdicionales docAdicionales = documentoInsideToConMAdicionales(
        objetoDocumentoInside, objetoDocumentoInside.getContenido().getValorBinario());

    es.mpt.dsic.eeutil.client.visDocExp.model.TipoDocumento tipoDocumento =
        TipoDocumentoConverter.tipoDocumentoInsideToEeutilClient(docAdicionales.getDocumento());
    es.mpt.dsic.eeutil.client.visDocExp.model.TipoMetadatosAdicionales adicionales =
        MetadatoAdicionalConverter
