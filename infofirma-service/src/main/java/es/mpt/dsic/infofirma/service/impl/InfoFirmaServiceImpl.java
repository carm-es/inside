...
public class InfoFirmaServiceImpl implements InfoFirmaService {
	...
	public void configureInfoFirma() {
		...
		try {
			if (!ACTIVO.contentEquals(infoFirmaActivo)) {
				...
				EeUtilServiceMtomImplService service1 = new EeUtilServiceMtomImplService(
						new URL(properties.getProperty("infofirma.url")));
				...
				logger.debug(String.format("Utilizando para infofirma idaplicacion/password : %s/%s",
						properties.getProperty("infofirma.idaplicacion"), properties.getProperty("infofirma.password")));
				activo = true;
			}
	 ...
	 * 
	 * @throws InfoFirmaServiceNoEsFirmaException
	 *             cuando el contenido aportado no se corresponde con una firma
	 *             electrónica en un formato de los admitidos.
	 * @throws InfoFirmaServiceException
	 *             cuando no se pueda obtener la información de la firma
	 */
	@Override
	public InfoFirmaElectronica getInfoFirma(FirmaElectronica firma, OpcionesInfoFirma opciones, byte[] bytesContenido)
			throws InfoFirmaServiceNoEsFirmaException, InfoFirmaServiceException {
		if (!activo) {
			throw new InfoFirmaServiceException("El WS de INFOFIRMA para la obtención de información de firmas no está activo");
		}

		if (firma == null || firma.getFirma() == null) {
			throw new InfoFirmaServiceException(new IllegalArgumentException("La firma no puede ser nula"));
		}
		...
		try {
			...
			InformacionFirma infoFirma = port.obtenerInformacionFirma(applicationLogin, dataHanlerFirma, opcionesWS,
					dataHanlerFirmaContenido);
			...
			if ("COD_0001".equalsIgnoreCase(e.getFaultInfo().getCodigo())
				...
				logger.warn("El servicio de obtención de información de firma está devolviendo un error: CODIGO: "
						+ e.getFaultInfo().getCodigo() + " DESCRIPCION: " + e.getFaultInfo().getDescripcion());
				throw new InfoFirmaServiceException("El servicio de obtención de información de firma está devolviendo un error");
			}

		} catch (Exception t) {
			logger.error("Excepción en la llamada al WS de obtención de Información de Firma", t);
			throw new InfoFirmaServiceException("Excepción en la llamada al WS de obtención de Información de Firma", t);
		}
	...
	@Override
	public ResultadoValidarCertificado getInfoCertificate(String certificate) throws InfoFirmaServiceException, InSideException {
		if (!activo) {
		...
		if (certificate == null || "".equals(certificate)) {
			throw new InfoFirmaServiceException(new IllegalArgumentException("El certificado no puede ser vacio"));
		}
	...
	@Override
	public ResultadoValidacionInfo validacionFirma(FirmaElectronica firma, String tipoFirma, DatosFirmados datosFirmados)
			throws InfoFirmaServiceException {
		...
		try {
			...
			validacionInfo = port.validacionFirma(applicationLogin, dataHanlerFirma, tipoFirma, dataHanlerDatosFirmados);
			return validacionInfo;
		} catch (InSideException e) {
			logger.debug("Error de validación de firma" + e);
			throw new InfoFirmaServiceException("El servicio de validación de firma está devolviendo un error");
		}
